...
/**
 * Base class for <code>BuiltInLibraryLoader</code> components that obtain content from a file on the file system or
...
public abstract class AbstractBuiltInLibraryLoader implements BuiltInLibraryLoader {
    ...
    /**
     * Returns an input source for the schema location that has been specified for the built-in library file.
     * 
     * @return LibraryInputSource&lt;InputStream&gt;
     * @throws LibraryLoaderException thrown if an error occurs while constructing the input source or the library file
     *         does not exist
     */
    protected LibraryInputSource<InputStream> getInputSource() throws LibraryLoaderException {
        ...
        try {
            ...
            throw new LibraryLoaderException(
                "Invalid library URL: " + libraryUrl + " (the namespace and/or library name needs to be modified)." );
        } catch (Exception e) {
            throw new LibraryLoaderException( "Unknown error loading built-in library: " + libraryDeclaration.getName(),
                e );
        }
        return inputSource;
    }
    ...
    /**
     * Returns the library URL as a concatenation of the namespace and name for the assigned schema declaration.
     * 
     * @return String
     */
    private String getLibraryUrl() {
        ...
        if (libraryDeclaration != null) {
            ...
            String location =
                libraryDeclaration.getLocation( CodeGeneratorFactory.XSD_TARGET_FORMAT ).replaceAll( "\\\\", "/" );
