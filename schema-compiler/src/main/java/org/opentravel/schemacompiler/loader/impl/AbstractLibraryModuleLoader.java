...
public abstract class AbstractLibraryModuleLoader implements LibraryModuleLoader<InputStream> {
    ...
    /**
     * Loads the content of the JAXB content from the specified input source. If a validation schema is provided, it
     * will be used to validate the XML content of the file.
     * 
    ...
    protected Object loadLibrary(LibraryInputSource<InputStream> inputSource, ValidationFindings validationFindings,
        JAXBContext jaxbContext, javax.xml.validation.Schema validationSchema)
        ...
        try (Reader reader = (inputStream == null) ? null : new InputStreamReader( inputStream )) {
            if (reader != null) {
                ...
                validationFindings.addFinding( FindingType.WARNING, new URLValidationSource( libraryUrl ),
                    LoaderConstants.WARNING_LIBRARY_NOT_FOUND, URLUtils.getShortRepresentation( libraryUrl ) );
            }
        } catch (IOException e) {
    ...
    public LibraryModuleInfo<Schema> loadSchema(LibraryInputSource<InputStream> inputSource,
        ...
        try (InputStream is = (inputSource == null) ? null : inputSource.getLibraryContent()) {
            ...
            if (is != null) {
                XMLStreamReader xmlsReader =
                    XMLInputFactory.newInstance().createXMLStreamReader( inputSource.getLibraryContent() );
                Map<String,String> namespacePrefixMappings = new HashMap<>();
                Unmarshaller unmarshaller = jaxbContext.createUnmarshaller();

                unmarshaller.setSchema( schemaValidationSchema );
                schema = (Schema) unmarshaller
                    .unmarshal( new PrefixMappingXMLStreamReader( xmlsReader, namespacePrefixMappings ) );
                ...
                validationFindings.addFinding( FindingType.WARNING, new URLValidationSource( schemaUrl ),
                    ...
                    (schemaUrl == null) ? MISSING_URL : URLUtils.getShortRepresentation( schemaUrl ) );
            }
            ...
            String urlString = (schemaUrl == null) ? MISSING_URL : URLUtils.getShortRepresentation( schemaUrl );

            validationFindings.addFinding( FindingType.ERROR, new URLValidationSource( schemaUrl ),
                LoaderConstants.ERROR_UNREADABLE_SCHEMA_CONTENT, urlString,
    ...
    private LibraryModuleInfo<Schema> buildModuleInfo(Schema schema) {
        ...
        return new LibraryModuleInfo<>( schema, null, schema.getTargetNamespace(), null, includes, imports );
    }

    /**
     * <code>XMLStreamReader</code> class used to examine XML parsing events during the unmarshalling process, and
     * capture the namespace/prefix mapping information from the root-level 'schema' element of the XML document being
     * parsed.
     * 
     * @author S. Livezey
     */
