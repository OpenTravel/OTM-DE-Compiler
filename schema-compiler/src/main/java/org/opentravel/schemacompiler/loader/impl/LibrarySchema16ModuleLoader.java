...
/**
 * Default implementation of the <code>LibraryModuleLoader</code> that loads and parses XML content from an input
 * stream. Unless configured otherwise, a <code>CatalogNamespaceResolver</code> is used to resolve library namespaces.
 * 
 * @author S. Livezey
 */
public class LibrarySchema16ModuleLoader extends AbstractLibraryModuleLoader {
    ...
    public LibraryModuleInfo<Object> loadLibrary(LibraryInputSource<InputStream> inputSource,
        ...
        try {
            ...
            if (jaxbLibrary != null) {
                ...
                jaxbLibrary
                    .setNamespace( (jaxbLibrary.getNamespace() == null) ? null : jaxbLibrary.getNamespace().trim() );
            ...
            validationFindings.addFinding( FindingType.ERROR, new URLValidationSource( libraryUrl ),
                LoaderConstants.ERROR_UNREADABLE_LIBRARY_CONTENT, urlString,
    ...
    /**
     * Attempts to load the library with XML validation enabled. On failure, a reload will be attempted with validation
     * disabled.
     * 
    ...
    private Library loadAndRetryOnFailure(LibraryInputSource<InputStream> inputSource, URL libraryUrl,
        ...
        try {
            jaxbLibrary =
                (Library) loadLibrary( inputSource, validationFindings, jaxbContext, libraryValidationSchema );
    ...
    /**
     * Attempts to reload the library without XML validation enabled if the original load failed. If the content is
     * still invalid with validation turned off, the file is completely unreadable. If that is the case, a JAXB
        ...
        try {
            ...
            if (jaxbLibrary != null) {
                validationFindings.addFinding( FindingType.WARNING, new URLValidationSource( libraryUrl ),
                    LoaderConstants.WARNING_CORRUPT_LIBRARY_CONTENT, urlString,
                    ExceptionUtils.getExceptionClass( originalException ).getSimpleName(),
                    ExceptionUtils.getExceptionMessage( originalException ) );
            }
    ...
    private LibraryModuleInfo<Object> buildModuleInfo(Library library, URL libraryUrl) {
        ...
        return new LibraryModuleInfo<>( library, library.getName(), library.getNamespace(), library.getVersionScheme(),
            includes, imports );
    }
    ...
    static {
        try {
            ...
            InputStream schemaStream =
                SchemaDeclarations.OTA2_LIBRARY_SCHEMA_1_6.getContent( CodeGeneratorFactory.XSD_TARGET_FORMAT );
