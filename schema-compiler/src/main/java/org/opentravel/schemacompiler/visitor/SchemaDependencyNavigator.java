...
public class SchemaDependencyNavigator extends AbstractNavigator<NamedEntity> {
	...
	protected void navigateCoreObject(TLCoreObject coreObject, TLAlias alias) {
		...
		if (canVisitCore || canVisitAlias) {
			...
			if (canVisitAlias)
				visitor.visitAlias( alias );
			TLAlias summaryAlias = (alias == null) ? null
					: AliasCodegenUtils.getFacetAlias( alias, TLFacetType.SUMMARY );
	...
	/**
	 * Navigates the contextual facet.  If the owner alias provided is non-null, the corresponding
	 * facet alias will be looked up prior to navigating the facet.
	 * 
		...
		if (actionFacet.getReferenceType() == TLReferenceType.NONE) {
			...
			for (TLContextualFacet choiceFacet : choicePayload.getChoiceFacets()) {
				do {
					...
					choiceFacet = (facetOwner instanceof TLContextualFacet) ? (TLContextualFacet) facetOwner
							: null;
	...
	private void navigateBusinessObjectRef(TLBusinessObject boRef, TLActionFacet actionFacet) {
		if (boRef != null) {
			TLFacet boFacet = ResourceCodegenUtils.getReferencedFacet( boRef,
	...
	private ClassSpecificAssignment<Object> navigateEntityFunction = new ClassSpecificAssignment<Object>()
