...
public class ContextUtils {
    ...
    /**
     * When a model element is moved from one library to another, it is necessary to scan its context ID references and
     * rename them to valid values in the target library. This mapping is accomplished by examining the application
     * context values for each <code>TLContext</code> in the target library. If an application context does not yet
     * exist in the target library, it is copied from the source library.
     * 
     * @param entity the entity whose context references are to be translated
     * @param sourceLibrary the source library where the original context references existed
     * @param targetLibrary the target library to which the references should be translated
     */
    public static void translateContextIdReferences(LibraryElement entity, TLLibrary sourceLibrary,
        ...
        for (TLContextReferrer cr : visitor.getContextReferrers()) {
            TLContext sourceContext = (cr.getContext() == null) ? null : sourceLibrary.getContext( cr.getContext() );
            ...
            if (sourceContext != null) {
                if (targetContextMap.containsKey( sourceContext.getApplicationContext() )) {
                    ...
                    targetContext.setContextId( getUniqueContextId( sourceContext.getContextId(), targetLibrary ) );
                    targetContext.setApplicationContext( sourceContext.getApplicationContext() );
                    targetLibrary.addContext( targetContext );
                    targetContextMap.put( targetContext.getApplicationContext(), targetContext.getContextId() );
                }
            }
        }
    }
    ...
    /**
     * During some import operations, it is sometimes not possible to know the correct context ID for a
     * <code>TLContextReferrer</code> prior to creating the object. In these cases, the import function may assign the
     * application context of the imported object (typically its target namespace) as the context ID. This method will
     * scan all context references in the given library element and replace all application context values with the
     * context ID values that are resolved from the owning library.
     * 
     * @param entity the entity whose application context references are to be resolved
     */
    public static void resolveApplicationContexts(LibraryElement entity) {
        if (entity.getOwningLibrary() instanceof TLLibrary) {
            ...
            for (TLContextReferrer cr : visitor.getContextReferrers()) {
                if ((cr.getContext() != null) && owningLibrary.getContext( cr.getContext() ) == null) {
     ...
     * 
     * @param preferredId the preferred context ID to use if one is not already defined in the target library
     * @param targetLibrary the target library where the context ID will be used
     * @return String
     */
