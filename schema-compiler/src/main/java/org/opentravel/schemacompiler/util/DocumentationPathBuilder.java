...
/**
 * Static utility methods that assist with the construction of path
 * strings for <code>TLDocumentationOwner</code> elements within an
 * OTM model.
 */
public class DocumentationPathBuilder {
	...
	private static class BuilderContext {
		...
		/**
		 * Adds the given path component to the beginning of the current
		 * documentation path.
		 * 
	...
	/**
	 * Provides the contract for pre-pending a documentation path
	...
	private interface PathBuilder<T extends ModelElement> {
		...
		/**
		 * Builds the next component of the documentation path using the current
		 * model element provided.  The next model element to process is returned
		 * by this method.
		 * 
	...
	private static class ServiceBuilder implements PathBuilder<TLService> {
		...
		/**
		 * @see org.opentravel.schemacompiler.util.DocumentationPathBuilder.PathBuilder#build(org.opentravel.schemacompiler.model.ModelElement, org.opentravel.schemacompiler.util.DocumentationPathBuilder.BuilderContext)
		 */
		@Override
	...
	private static class ResourceParentRefBuilder implements PathBuilder<TLResourceParentRef> {
		...
		public ModelElement build(TLResourceParentRef currentElement, BuilderContext context) {
			context.addPath( "|@PREF:" + currentElement.getParentResourceName() + "/" + currentElement.getParentParamGroupName() );
			return currentElement.getOwner();
		}
	...
	private static class ActionResponseBuilder implements PathBuilder<TLActionResponse> {
		...
		public ModelElement build(TLActionResponse currentElement, BuilderContext context) {
			context.addPath( "|@RS" + DocumentationPathResolver.getIntegerListIdentity(
					currentElement.getStatusCodes() ) );
			return currentElement.getOwner();
		}
