...
/**
 * Visitor that scans a model in order to construct a map that links each member of an inheritance hierarchy with a list
 * of all hierarchy members.
 * 
 * @author S. Livezey
 */
public class InheritanceHierarchyBuilder {
	
	private static final TLFacetType[] ISOLATED_HIERARCHIES = new TLFacetType[] { TLFacetType.QUERY,
			TLFacetType.REQUEST, TLFacetType.RESPONSE, TLFacetType.NOTIFICATION };
	private static final List<TLFacetType> isolatedHierarchies = Arrays.asList( ISOLATED_HIERARCHIES );
	 ...
	 * 
	 * @param model the model instance to be scanned
	 */
	...
	/**
	 * Constructs a map that associates each individual member of an inheritance hierarchy with a collection that
	 * contains all members of that hierarchy tree. The list of hierarchy members are sorted from the top-most hierarchy
	 * base member to the lowest (siblings are sorted in alphabetical order by local name).
	 * 
	 ...
	 * 
	 * @param member the hierarchy member for which to return the root
	 * @param extensionMap the pre-computed inheritance mappings
	 * @return NamedEntity
	 */
	private NamedEntity getHierarchyRoot(NamedEntity member, Map<NamedEntity,NamedEntity> extensionMap) {
		NamedEntity rootMember = member;
		
		if (isIsloatedHierarchyMember( member )) {
			TLAlias memberAlias = null;
			TLFacet memberFacet;
			NamedEntity candidateRoot;
			...
			candidateRoot = getIsolatedRootMember( memberFacet, memberAlias, extensionMap );
			
			if (candidateRoot != null) {
				rootMember = candidateRoot;
			}
	...
	/**
	 * Finds the hierarchy root for the given facet member.
	 * 
	 * @param memberFacet  the isolated member facet
	 * @param memberAlias  alias of the member facet (may be null)
	 * @param extensionMap the pre-computed inheritance mappings
	 * @return NamedEntity
	 */
	private NamedEntity getIsolatedRootMember(TLFacet memberFacet, TLAlias memberAlias,
			Map<NamedEntity,NamedEntity> extensionMap) {
		TLFacetOwner baseFacetOwner = (TLFacetOwner) extensionMap.get( memberFacet.getOwningEntity() );
		NamedEntity rootMember = null;
		
		while (baseFacetOwner != null) {
			String facetName = FacetCodegenUtils.getFacetName( memberFacet );
			TLFacet baseFacet = FacetCodegenUtils.getFacetOfType( baseFacetOwner, memberFacet.getFacetType(),
					facetName );
	...
	/**
	 * Returns true if the given member is to be considered a member of an isolated inheritance hierarchy. For EXAMPLE,
	 * query facets of business objects should not be considered substitutable with the business object's ID facet, but
	 * it may be considered a substitutable with query facets of the same name from an extended business object.
	 * 
	 * <p>
	 * NOTE: Under the current modeling scheme, isloated hierarchies are guranteed to be wholly composed of
	 * <code>TLFacet</code> instances or <code>TLAlias</code> objects whose owners are facets.
	 * 
	 * @param member the hierarchy member to analyze
	 */
			...
			if (extendedEntity instanceof TLAliasOwner) {
				...
				for (TLAlias extendedAlias : extendedAliasOwner.getAliases()) {
					...
					while ((baseAlias == null) && (baseOwner != null)) {
						...
						baseOwner = (TLAliasOwner) FacetCodegenUtils.getFacetOwnerExtension( (TLFacetOwner) baseOwner );
					}
					if (baseAlias != null) {
	...
	private class HierarchyComparator implements Comparator<NamedEntity> {
		...
		/**
		 * Constructor that supplies the extension relationships to use when comparing members of an inheritance
		 * hierarchy.
		 * 
		 * @param extensionMap the extension mapping information to use for entity comparisons
		 */
		 ...
		 * 
		 * @param member the member for which to return the hierarchy depth
		 * @return int
		 */
