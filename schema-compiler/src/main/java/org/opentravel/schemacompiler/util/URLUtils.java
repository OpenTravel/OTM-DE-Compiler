...
public class URLUtils {
	...
	public static File toFile(URL url) {
		if (!isFileURL(url)) {
			throw new IllegalArgumentException(
					"URL is not a local file reference: " + ((url == null) ? null : url.toExternalForm()));
		} else {
	...
	/**
	 * Returns a URL that is the relative parent location of the one passed to this method.  If the given
	 * URL is, itself, a top-level address (authority only - no path or file references), the original URL
	 * will be returned.  In all cases, the query string (if one exists) will be omitted from the URL that
	 * is returned.
	 * 
	...
	/**
	 * If the specified 'urlLocation' is a fully-qualified URL, that location is returned as a URL
	 * object.  If the 'urlLocation' is a relative URL, the fully-qualified URL is calculated uring the
	 * 'baseUrl' as the base <i>folder</i> location from which the absolute location is to be derived.
	 * 
	 * <p>NOTE: It is important that the 'baseUrl' passed to this method is not a file location.
	 * 
	...
	/**
	 * Returns the relative URL string that represents the path from the specified originating location
	 * to the indicated target location.
	 * 
	 ...
	 * @param originatingLocationIsFile  flag indicating whether the originating location provided represents
	 *								  the URL of a file or folder (for files, the last path component of the
	 *								  URL is disgarded before processing begins)
	 * @return String
	 */
	public static final String getRelativeURL(URL originatingLocation, URL targetLocation, boolean originatingLocationIsFile) {
		// Special Case: If the URL protocols or authorities do not match, the target location is the relative URL
	...
	/**
	 * Returns true if the protocol and authority components of the given URL match one another.  If the URL
	 * represents a windows-style file URL, the drive-letter is also checked.
	 * 
	...
	private static boolean isMatchingProtocolAndAuthority(URL url1, URL url2) {
		...
		boolean protocolMatches = ((protocol1 == null) || protocol1.equals("")) ?
				((protocol2 == null) || protocol2.equals("")) : protocol1.equals(protocol2);
		boolean authorityMatches = ((authority1 == null) || authority1.equals("")) ?
				((authority2 == null) || authority2.equals("")) : authority1.equals(authority2);
