...
public class JsonSchemaCodegenUtils {
	...
	/**
	 * Constructor that supplies the current code generation transformer
	 * context.
	 * 
	 ...
	 * 
	 * @param simpleInfo the pre-determined simple type info for the member
	 *            field (must not be null)
	 * @param jsonType the pre-determined JSON type of the member field (must
	 *            not be null)
	 * @return JsonSchema
	 */
	...
	/**
	 * Adds documentation to the given schema componnent that describes the
	 * original OTM type for an element or attribute.
	 * 
	 * @param docOwner the JSON schema component to which documentation should
	 *            be added
	 * @param fieldType the original type assigned to the attribute, element, or
	 *            VWA value
	 */
	...
	/**
	 * Returns the assigned type label that should be included in the JSON
	 * documentation for OTM simple types.
	 * 
	 * @param fieldType the type assigned to the attribute, element, or VWA
	 *            value
	 * @return String
	 */
	...
	/**
	 * Applies the OTM simple type constraints provided to the given JSON
	 * schema.
	 * 
	 * @param schema the JSON schema to which the type constraints will be
	 *            applied
	...
	/**
	 * Applies an additional line of documentation to the given schema
	 * component's list of descriptions. If then given doc-owner does not yet
	 * contain any documentation, it will be created automatically.
	 * 
	 * @param docOwner the schema componnent to which the supplemental
	...
	/**
	 * Returns a relative path reference to the JSON schema definition of the
	 * given named entity.
	 * 
	...
	/**
	 * Returns a relative path reference to the JSON schema definition of the
	 * given schema dependency. If the referenced type does not have an
	 * associated JSON definition, this method will return null.
	 * 
	 * @param referencedEntity the schema dependency for which to return a
	 *            reference
	 * @param referencingEntity the named entity which owns the reference
	 * @return String
	 */
	public String getSchemaReferencePath(SchemaDependency schemaDependency, NamedEntity referencingEntity) {
		JsonTypeNameBuilder typeNameBuilder = getTypeNameBuilder();
		String referencedFilename = schemaDependency.getSchemaDeclaration()
			.getFilename(CodeGeneratorFactory.JSON_SCHEMA_TARGET_FORMAT);
	...
	/**
	 * Returns a relative path reference to the XML schema definition of the
	 * given named entity.
	 * 
	 * @param referencedEntity the named entity for which to return a reference
	 * @return String
	...
	/**
	 * If the context has been configured for single-file swagger generation,
	 * this method will return the shared <code>JsonTypeNameBuilder</code>.
	...
	/**
	 * Shared method that constructs the JSON structures for the
	 * 'x-otm-annotations' element of a schema. If no annotations are required,
	 * this method will return with no action.
	 * 
	 * @param targetJson the target JSON document to which the annotation
	 *            element will be applied
	 * @param docOwner the JSON documentation owner from which to obtain the
	...
	public static void createOtmAnnotations(JsonObject targetJson, JsonDocumentationOwner docOwner) {
		...
		if ((documentation != null) || !equivalentItems.isEmpty() || !exampleItems.isEmpty()) {
			...
			hasOtmAnnotation = addExampleAndEquivalentAnnotations(exampleItems, equivalentItems, otmAnnotations)
					|| hasOtmAnnotation;
	...
	/**
	 * Since the 'description' field is supported by the JSON schema spec, we
	 * will move that property from the 'x-otm-documentation' element to the
	 * main schema properties.
	 * 
	 * @param jsonDoc the JSON object containing the OTM model documentation
	 *            fields
	 * @param targetJson the target JSON schema to which the description will be
	 *            assigned
	 */
	 ...
	 * @param otmAnnotations the JSON schema object representing the OTM
	 *            annotations
	 * @return boolean
	 */
	...
	/**
	 * Recursively processes the given JSON document, removing all properties
	 * whose names begin with 'x-otm-'.
	 * 
