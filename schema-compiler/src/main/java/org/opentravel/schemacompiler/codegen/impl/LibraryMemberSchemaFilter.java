...
/**
 * Code generation filter that only allows generation of artifacts derived directly from a single library member.
 * 
 * @author S. Livezey
 */
public class LibraryMemberSchemaFilter implements CodeGenerationFilter {
     ...
     * @param libraryFilter code generation filter that contains information about the libraries upon which the member's
     *        schema will depend
     */
    ...
    public boolean processLibrary(AbstractLibrary library) {
        return builtInLibraries.contains( library ) || libraryFilter.processLibrary( library );
    }
