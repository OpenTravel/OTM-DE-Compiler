 ...
 *
 */
public class OperationInfoWriter extends AbstractInfoWriter<ServiceDocumentationBuilder> {

    /**
     * @param writer the writer for which to create an info-writer
     * @param member the service builder to which the operation belongs
     */
    public OperationInfoWriter(SubWriterHolderWriter writer, ServiceDocumentationBuilder member) {
        super( writer, member );
     ...
     * (non-Javadoc)
     * 
     * @see org.opentravel.schemacompiler.codegen.documentation.html.writers.AbstractInfoWriter
    ...
    private Content getInfo(OperationDocumentationBuilder fdb) {
        ...
        if (doc != null) {
            DocumentationInfoWriter docWriter = new DocumentationInfoWriter( writer, doc );
    ...
    protected String getInfoTableSummary() {
        ...
        return config.getText( "doclet.Operation_Table_Summary", config.getText( "doclet.Operation_Summary" ),
