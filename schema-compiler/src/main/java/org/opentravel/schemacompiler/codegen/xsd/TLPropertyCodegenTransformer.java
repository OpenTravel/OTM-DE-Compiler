...
/**
 * Performs the translation from <code>TLProperty</code> objects to the JAXB nodes used to produce
 * the schema output.
 * 
 * @author S. Livezey
 */
public class TLPropertyCodegenTransformer extends
    ...
    public TopLevelElement transform(TLProperty source) {
        TopLevelElement element = source.isReference() ? transformReferenceProperty(source)
    ...
    private TopLevelElement transformReferenceProperty(TLProperty source) {
        ...
        element.setType(new QName(XMLConstants.W3C_XML_SCHEMA_NS_URI,
