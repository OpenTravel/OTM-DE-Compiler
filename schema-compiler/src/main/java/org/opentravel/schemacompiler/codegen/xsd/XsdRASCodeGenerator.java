...
/**
 * Code generator implementation used to generate XML schema documents for RAS services from
 * meta-model business objects.
 * 
 * <p>
 * The following context variable(s) are required when invoking this code generation module:
 * <ul>
 * <li><code>schemacompiler.OutputFolder</code> - the folder where generated files are normally
 * stored</li>
 * <li><code>schemacompiler.RASOutputFolder</code> - the folder where generated RAS files are to be
 * stored; if omitted, this will be in a /ras sub-folder of the root output folder</li>
 * </ul>
 * 
 * @author S. Livezey
 */
public class XsdRASCodeGenerator extends AbstractXsdCodeGenerator<TLBusinessObject> {
     ...
     */
    @Override
    protected Marshaller getMarshaller(TLBusinessObject source, org.w3._2001.xmlschema.Schema schema)
     ...
     */
    @Override
    protected Object transformSourceObjectToJaxb(TLBusinessObject source,
            CodeGenerationContext context) throws CodeGenerationException {
        String targetNamespace = source.getNamespace();
        Schema schema = createSchema(targetNamespace,
                ((TLLibrary) source.getOwningLibrary()).getVersion());
        ...
        schemaAnnotation.getAppinfoOrDocumentation().add(
                XsdCodegenUtils.getServiceAppInfo(source, context));
        ...
        incl.setSchemaLocation(new LibraryTrimmedFilenameBuilder(source).buildFilename(
                source.getOwningLibrary(), "xsd"));
        ...
        ComplexType createRQType = createType(source.getDetailFacet(), RASOperationType.CREATE,
                true, false);
        ComplexType createRSType = createType(source.getIdFacet(), RASOperationType.CREATE, false,
                false);
        ComplexType updateRQType = createType(source.getSummaryFacet(), RASOperationType.UPDATE,
                true, false);
        ComplexType updateRSType = createType(source.getSummaryFacet(), RASOperationType.UPDATE,
                false, false);
        ComplexType deleteRQType = createType(source.getIdFacet(), RASOperationType.DELETE, true,
                false);
     ...
     * 
     * @param type
     *            the complex type for which to create an element
     * @param targetNamespace
     *            the target namespace of the schema
     ...
     * 
     * @param sourceFacet
     *            the business object facet that will define the type's content
     * @param operationType
     *            the type of operation for which the type will be created
     * @param isRequest
     *            flag indicating RQ/RS type name (true = request; false = response)
     * @param hasFacetTypeAttribute
     *            flag indicating whether a 'facetType' attribute should be included in the type
     *            definition
     * @return ComplexType
     */
    private ComplexType createType(TLFacet sourceFacet, RASOperationType operationType,
            boolean isRequest, boolean hasFacetTypeAttribute) {
        ...
        if (hasFacetTypeAttribute) {
            ...
            doc.getContent().add(
                    "Specifies the facet type of the element(s) to be returned in the response.");
            annotation.getAppinfoOrDocumentation().add(doc);

            ftAttr.setName("responseType");
            ftAttr.setType(new QName(sourceFacet.getNamespace(), targetFacet.getOwningEntity()
                    .getLocalName() + "_FacetType"));
     ...
     * 
     * @param source
     *            the business object for which the RAS schema is being generated
     * @return SimpleType
     */
    ...
    private JAXBElement<NoFixedFacet> createFacetEnumValue(TLFacet facet) {
        ...
        enumValue.setValue(facet.getOwningEntity().getLocalName() + "_"
     ...
     *            the preferred facet to reference
     * @return TLFacet
     */
