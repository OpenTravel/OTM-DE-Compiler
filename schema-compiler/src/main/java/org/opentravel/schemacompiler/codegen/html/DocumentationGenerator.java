...
public class DocumentationGenerator implements CodeGenerator<TLModel> {
    ...
    @Override
    public Collection<File> generateOutput(TLModel model, CodeGenerationContext context)
        throws ValidationException, CodeGenerationException {
        Configuration config = Configuration.getInstance();
        File destination = new File( context.getValue( CodeGenerationContext.CK_OUTPUT_FOLDER ) );
        if (!destination.exists()) {
            destination.mkdirs();
        }
        config.setDestDirName( destination.getPath() + File.separator );
        String title = context.getValue( CodeGenerationContext.CK_PROJECT_FILENAME );
        if (title != null) {
    ...
    @Override
    public void setFilenameBuilder(CodeGenerationFilenameBuilder<TLModel> filenameBuilder) {
