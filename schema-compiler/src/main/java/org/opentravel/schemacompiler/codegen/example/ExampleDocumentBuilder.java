...
/**
 * Builder component that is capable of producing XML example output for model entities in a variety of formats (e.g.
 * DOM, text, and streaming output).
 * 
 * @author S. Livezey
 */
public class ExampleDocumentBuilder extends ExampleBuilder<Document> {

    private static final String XML_HEADER_CONTENT =
    ...
    /**
     * Assigns the location of the XML schema (XSD) file that should be used to validate the content from the specified
     * namespace. Schema locations will only be included in the example XML output if they are bound to one or more
     * elements in the resulting XML document.
     * 
     * @param library the library for which to assign a schema location
     * @param schemaLocation the relative schema location for the example document
    ...
    /**
     * Returns the schema location of the given library. If a schema location has not been added for the requested
     * library, this method will return null.
     * 
    ...
    /**
     * Returns the schema location of the specified namespace. If a schema location has not been added for the
     * namespace, this method will return null.
     * 
    ...
    /**
     * Returns true if an 'xsi:schemaLocation' attribute should be added to the root element of the example for the
     * given entity.
     * 
     * @return boolean
     */
    private boolean schemaLocationRequired() {
        return PropertyCodegenUtils.hasGlobalElement( modelElement ) || (modelElement instanceof TLExtensionPointFacet);
    }
