...
/**
 * Code generation delegate for facet used to separate logic and break up the complexity
 * of the facet artifact generation process.
 * 
...
public abstract class FacetCodegenDelegate<F extends TLAbstractFacet> {
	...
	protected void addCompileTimeDependency(SchemaDependency dependency) {
		if (transformerContext != null) {
			...
			if (codeGenerator instanceof AbstractJaxbCodeGenerator) {
				((AbstractJaxbCodeGenerator<?>) codeGenerator).addCompileTimeDependency(dependency.getSchemaDeclaration());
			}
		}
	}
	...
	protected TransformerFactory<CodeGenerationTransformerContext> getTransformerFactory() {
		return (TransformerFactory<CodeGenerationTransformerContext>) transformerContext.getTransformerFactory();
	}
	
	/**
	 * Generates the code artifacts of the facet.  Typically, the artifacts produced for each
	 * facet include a JAXB type and a global element definition.  Sub-classes may extend this method
	 * to add additional artifacts as required.
	 * 
	 ...
	 * 
	 * @param elementType  the local name of the element type (namespace is assumed to be tns)
	 * @return Annotated
	 */
	 ...
	 * that is returned by this method will always belong to the same owner as the given source facet.
	 * 
	 * @return F
	 */
	...
	/**
	 * Returns the substitution group for the source facet.  By default, the substitution group
	 * for an element is null (not defined); sub-classes may override to assign a substitution
	 * group to the element(s) that are generated for the facet. 
	 * 
	 * @param facetAlias  the alias of the source facet element being created (may be null)
	 * @return QName
	 */
