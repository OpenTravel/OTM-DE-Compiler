...
public class XsdCodegenUtils {
    ...
    /**
     * Returns the name of the global XML schema element associated with the given property type, or null if the type
     * cannot be referenced by element (e.g. simple types).
     * 
    ...
    public static QName getGlobalElementName(NamedEntity modelEntity) {
        ...
        if (isSimpleCoreObject( modelEntity )) {
            elementName = getSimpleCoreGlobalElementName( modelEntity );

        } else if (modelEntity instanceof TLContextualFacet) {
            elementName = getContextualFacetElementName( (TLContextualFacet) modelEntity );

        } else if ((modelEntity instanceof TLAbstractFacet) && !(modelEntity instanceof TLSimpleFacet)) {
            elementName = getAbstractFacetElementName( (TLAbstractFacet) modelEntity );

        } else if (modelEntity instanceof TLAlias) {
            elementName = getAliasElementName( (TLAlias) modelEntity );

        } else if (modelEntity instanceof TLActionFacet) {
            elementName = getActionFacetElementName( (TLActionFacet) modelEntity );

        } else if ((modelEntity instanceof TLCoreObject) || (modelEntity instanceof TLChoiceObject)
    ...
    /**
     * Returns the global element name of the given simple core entity (or alias). This method accounts for the fact a
     * direct reference to the summary facet is the same as a reference to the core itself.
     * 
     * @param simpleCoreEntity the simple core entity for which to return the global element name
     * @return QName
     */
    private static QName getSimpleCoreGlobalElementName(NamedEntity simpleCoreEntity) {
        QName elementName;

        if (simpleCoreEntity instanceof TLAlias) {
            TLAlias alias = (TLAlias) simpleCoreEntity;

            if (alias.getOwningEntity() instanceof TLFacet) {
                simpleCoreEntity = AliasCodegenUtils.getOwnerAlias( alias );
            }
        } else if (simpleCoreEntity instanceof TLFacet) {
            simpleCoreEntity = ((TLFacet) simpleCoreEntity).getOwningEntity();
        }
    ...
    /**
     * Returns the global element name for the given contextual facet.
     * 
     * @param facet the contextual facet for which to return the global element name
     * @return QName
     */
    private static QName getContextualFacetElementName(TLContextualFacet facet) {
        ...
        if (owner != null) {
            ...
            elementName = new QName( facet.getNamespace(), elementLocalName );
        }
        return elementName;
    }

    /**
     * Returns the global element name for the given facet. This method presumes that the given facet is not a simple
     * facet type.
     * 
     * @param facet the facet for which to return the global element name
     * @return QName
     */
    private static QName getAbstractFacetElementName(TLAbstractFacet facet) {
     ...
     * 
     * @param alias the alias for which to return the global element name
     * @return QName
     */
    private static QName getAliasElementName(TLAlias alias) {
        ...
        return new QName( alias.getNamespace(), elementLocalName );
    }

    /**
     * Returns the global element name for the given action facet.
     * 
     * @param actionFacet the entity for which to return the global element name
     * @return QName
     */
    private static QName getActionFacetElementName(TLActionFacet actionFacet) {
    ...
    /**
     * Returns the name of the substitutable element for the given facet. Substitutable elements are defined in the
     * schema, but are not referenced by properties.
     * 
    ...
    /**
     * Returns the name of the substitutable element for the given facet alias. Substitutable elements are defined in
     * the schema, but are not referenced by properties.
     * 
    ...
    /**
     * Returns the name of the global XML schema element associated with head of the entity's substitution, group or
     * null if the type does not represent the head of a substitution group.
     * 
     * @param modelEntity the model entity for which to return the element name
     * @return QName
     */
    public static QName getSubstitutionGroupElementName(NamedEntity modelEntity) {
        ...
        boolean isTopLevelFacetOwner =
            (nonAliasEntity instanceof TLBusinessObject) || (nonAliasEntity instanceof TLCoreObject)
                || (nonAliasEntity instanceof TLChoiceObject) || (nonAliasEntity instanceof TLActionFacet);

        if (isTopLevelFacetOwner) {
            QName globalElementName = getGlobalElementName( modelEntity );

            if ((nonAliasEntity instanceof TLChoiceObject) || (nonAliasEntity instanceof TLActionFacet)) {
                ...
                referenceElementName =
                    new QName( globalElementName.getNamespaceURI(), globalElementName.getLocalPart() + "SubGrp" );
            }
        }
    ...
    /**
     * In cases where the payload of an action facet maps to a core or choice object, this method will return a
     * facet-specific element name. If the owner of the payload facet does not match the action facet's base payload
        ...
        if (payloadFacet == null) {
            ...
            if (elementName != null) {
                switch (payloadFacet.getFacetType()) {
                    case DETAIL:
                    case CHOICE:
                        facetSuffix = payloadFacet.getFacetType()
                            .getIdentityName( FacetCodegenUtils.getFacetName( payloadFacet ) );
                        break;
                    default:
                        break;
                }
                ...
                if (facetSuffix != null) {
                    elementName = new QName( elementName.getNamespaceURI(), elementName.getLocalPart() + facetSuffix );
                }
            }
        }
        return elementName;
    ...
    /**
     * Returns true if the given core object is considered "simple" (or an alias of a simple core). Simple cores define
     * summary facet members, but no published or inherited detail facet members.
     * 
     ...
     * 
     * @param modelEntity the model entity for which to return the element name
     * @param referencingProperty the property that is referencing the given model entity
     * @return String
     */
    ...
    private static String getFacetTypeName(TLAbstractFacet facet) {
        ...
        if (facet.getOwningEntity() instanceof TLComplexTypeBase) {
            String complexTypeName = getComplexFacetTypeName( facet, typeName );

            typeName = (complexTypeName == null) ? typeName : complexTypeName;
    ...
    /**
     * Returns the type name for the given facet. This method presumes that the given facet's owner is a subclass of
     * <code>TLComplexTypeBase</code>.
     * 
     * @param facet the facet for which to return the type name
     * @param preferredTypeName the preferred name for the type that should be returned if another one cannot be
     *        identified
     * @return String
     */
    private static String getComplexFacetTypeName(TLAbstractFacet facet, String preferredTypeName) {
        String typeName = preferredTypeName;
        ...
        } else if ((facet.getOwningEntity() instanceof TLBusinessObject) && (facet instanceof TLFacet)) {
            if (facet.getFacetType() == TLFacetType.SUMMARY) {
                typeName = facet.getOwningEntity().getLocalName();
    ...
    protected static String getTypeFacetSuffix(TLContextualFacet facet) {
        ...
        while (facet instanceof TLContextualFacet) {
            ...
            if ((facetName != null) && (facetName.length() > 0) && (facet instanceof TLContextualFacet)) {
    ...
    /**
     * Returns the name of the update indicator to be included in an update facet for the given optional attribute.
     * 
    ...
    /**
     * Returns the name of the update indicator to be included in an update facet for the given optional element.
     * 
    ...
    /**
     * Returns the 'AppInfo' child element of the given JAXB object. If an 'AppInfo' element is not yet defined, one
    ...
    /**
     * If required, adds an 'AppInfo' element to the JAXB entity provided with all of the required sub-elements to
     * document its use within the compiler meta-model.
     * 
     * @param modelEntity the model entity that contains all relevant information to be included in the 'AppInfo'
     *        element
    ...
    /**
     * Adds equivalent elements to the given app-info schema element from the EXAMPLE owner provided. If the 'appInfo'
     * parameter passed to this method is null, a new one will be created automatically.
     * 
    ...
    /**
     * Adds EXAMPLE elements to the given app-info schema element from the EXAMPLE owner provided. If the 'appInfo'
     * parameter passed to this method is null, a new one will be created automatically.
     * 
    ...
    /**
     * Returns the <code>OTA2Entity</code> app-info entry that describes the given named entity. If the entity does not
     * have a corresponding app-info description, this method will return null.
     * 
     * @param entity the named entity to process
     * @return OTA2Entity
     */
