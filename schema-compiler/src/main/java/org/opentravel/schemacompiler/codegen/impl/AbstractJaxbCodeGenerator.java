...
/**
 * <code>CodeGenerator</code> base class that handles schema output generation using JAXB bindings as a mechanism for
 * producing the output content.
 * 
 * @param <S> the source type for which output content will be generated
 * @author S. Livezey
 */
public abstract class AbstractJaxbCodeGenerator<S extends ModelElement> extends AbstractCodeGenerator<S> {
    ...
    protected void copyCompileTimeDependencies(CodeGenerationContext context) throws CodeGenerationException {
        try {
            for (SchemaDeclaration schemaDeclaration : getCompileTimeDependencies()) {
                ...
                File outputFile =
                    new File( outputFolder, schemaDeclaration.getFilename( CodeGeneratorFactory.XSD_TARGET_FORMAT ) );
    ...
    /**
     * Performs the translation from meta-model element to a JAXB object that will be used to generate the output
     * content.
     * 
    ...
    /**
     * Returns the XML schema associated with the given JAXB object. For schema code generators, the JAXB object
     * provided will typically be the schema instance. For WSDL code generators, the JAXB schema will be nested inside
     * the types section of the WSDL document.
     * 
