 ...
 * an external representation (e.g. XSD, WSDL, etc.).
 * 
 * <p>
 * NOTE: Code generators assume that the model content has been determined to be error-free prior to
 * calling the 'generateOutput()' method. The code generator itself does not perform any validation
 * checking.
 * 
 * @param <S>
...
public interface CodeGenerator<S> {
     ...
     * provided. When calling this method, the code generator itself is responsible for determining
     * the target output location for generated content.
     * 
     * @param source
     ...
     *             thrown if problems are detected during validation that prevent schema generation
     *             from proceeding
    ...
    /**
     * Assigns the code generation filter to use during processing. If one is not specified, the
     * code generator should use a default filter implementation.
     * 
     * @param filter
     *            the code generation filter to assign
     */
