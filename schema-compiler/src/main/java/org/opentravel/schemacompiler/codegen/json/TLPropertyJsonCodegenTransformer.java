...
/**
 * Performs the translation from <code>TLProperty</code> objects to the JSON schema elements used to produce the output.
 */
public class TLPropertyJsonCodegenTransformer
    extends AbstractJsonSchemaTransformer<TLProperty,JsonSchemaNamedReference> {
    ...
    public JsonSchemaNamedReference transform(TLProperty source) {
        return source.isReference() ? transformReferenceProperty( source ) : transformValueProperty( source );
    }
    ...
    /**
     * Performs the transformation of the property as the JSON equivalent of an IDREF(S) element.
     * 
     * @param source the source object being transformed
     * @return JsonSchemaNamedReference
     */
    private JsonSchemaNamedReference transformReferenceProperty(TLProperty source) {
        ...
        if (isMultipleReference) { // Array of references
            ...
            entityInfo.setEntityType(
                JsonType.JSON_ARRAY.getSchemaType() + "[" + JsonType.JSON_STRING.getSchemaType() + "]" );
        }
    ...
    private void setPropertyType(JsonSchemaReference schemaRef, TLPropertyType propertyType, TLProperty source) {
        ...
        if ((jsonType != null) && !(source.getType() instanceof TLValueWithAttributes)
            ...
            doc.setDescriptions(
                "Legacy XML schema reference - {" + propertyType.getNamespace() + "}" + propertyType.getLocalName() );
    ...
    /**
     * Calculate the max-occurs based on the source property or the property type (in the case of non-simple list
     * facets).
     * 
