...
public abstract class AbstractInfoWriter<T> implements InfoWriter {
     ...
     * @param source the source object for the new info-writer
     */
     ...
     * 
     * @param memberTree the content to which the summary should be added
     */
     ...
     * 
     * @param content the content to add the icons too.
     * @param targetId the id to collapse
     * @param triggerId the icon id, used to flip between open/closed.
     */
    protected void addCollapseTrigger(Content content, String targetId, String triggerId) {
        HtmlTree openSpan = HtmlTree.span( HtmlTree.EMPTY );
        openSpan.addAttr( HtmlAttr.CLASS, HtmlStyle.TOGGLE_BUTTON + " " + HtmlStyle.IMG_OPEN );
     ...
     * 
     * @param tree the html tree to collapse.
     * @param id the html tree id. Used by a collapse trigger.
     */
    ...
    protected Content getTableTree() {
        Content content = caption == null ? HtmlTree.EMPTY : writer.getTableCaption( caption );
        Content table = HtmlTree.table( HtmlStyle.OVERVIEW_SUMMARY, 0, 3, 0, getInfoTableSummary(), content );
        table.addContent( writer.getSummaryTableHeader( getInfoTableHeader(), "col" ) );
        return table;
    }
    ...
    protected Content getInfoLabel() {
        return HtmlTree.heading( HtmlConstants.SUMMARY_HEADING, title );
    }
