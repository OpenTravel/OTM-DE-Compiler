...
/**
 * Performs the translation from <code>TLActionResponse</code> objects to the Swagger model objects used to produce the
 * output.
 */
public class TLActionResponseSwaggerTransformer
    ...
    /**
     * Constructs a new Swagger response for the indicated status code. If the status code is null, a default response
     * will be created.
     * 
    ...
    private SwaggerResponse createResponse(TLActionResponse source, Integer statusCode) {
        ...
        if (payloadType != null) {
            if (containsSupportedType( source.getMimeTypes(), TLMimeType.TEXT_JSON, TLMimeType.APPLICATION_JSON )) {
                response.setSchema( new JsonSchemaReference( jsonUtils.getSchemaReferencePath( payloadType, null ) ) );
            }
            if (containsSupportedType( source.getMimeTypes(), TLMimeType.TEXT_XML, TLMimeType.APPLICATION_XML )) {
                response.setXmlSchema( new SwaggerXmlSchemaRef( jsonUtils.getXmlSchemaReferencePath( payloadType ) ) );
            }
