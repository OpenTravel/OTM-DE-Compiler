...
/**
 * Code generator for built-in XML schemas referenced in a library meta-model. The behavior of this code generator is to
 * simply copy the content of the file from its source location in the local classpath to the proper output location.
 * 
 * @author S. Livezey
 */
public class XsdBuiltInCodeGenerator extends AbstractCodeGenerator<BuiltInLibrary> {
    ...
    protected void generateXsdLibraryOutput(BuiltInLibrary source, CodeGenerationContext context)
        ...
        if (!source.getNamespace().equals( XMLConstants.W3C_XML_SCHEMA_NS_URI )) {
            ...
            try (BufferedReader reader = new BufferedReader( new InputStreamReader(
                source.getSchemaDeclaration().getContent( CodeGeneratorFactory.XSD_TARGET_FORMAT ) ) )) {
    ...
    private static class XsdUserBuiltInCodeGenerator extends AbstractXsdCodeGenerator<BuiltInLibrary> {
         ...
         */
        @Override
        protected Marshaller getMarshaller(BuiltInLibrary source, org.w3._2001.xmlschema.Schema schema)
