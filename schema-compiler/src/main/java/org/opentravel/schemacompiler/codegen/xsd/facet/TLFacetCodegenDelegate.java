...
public abstract class TLFacetCodegenDelegate extends FacetCodegenDelegate<TLFacet> {
	...
	public FacetCodegenElements generateElements() {
		...
		if (hasContent()) {
			...
			for (TLAlias facetAlias : sourceFacet.getAliases()) {
				...
				addAliasElements( facetAlias, doDefaultElement,
						doSubstitutionGroupElement, doNonSubstitutableElement, codegenElements );
			}
		}
	 ...
	 */
	private void addAliasElements(TLAlias facetAlias, boolean doDefaultElement,
			boolean doSubstitutionGroupElement, boolean doNonSubstitutableElement,
		...
		if (owner instanceof TLDocumentationOwner) {
			...
			if (doc != null) {
				ObjectTransformer<TLDocumentation,Annotation,CodeGenerationTransformerContext> docTransformer = getTransformerFactory()
					.getTransformer( doc, Annotation.class );
	...
	protected Annotated createType() {
		...
		boolean hasSequence = (globalExtensionPointElement != null) || !elementList.isEmpty()
				|| hasElementIndicators( indicatorList );
