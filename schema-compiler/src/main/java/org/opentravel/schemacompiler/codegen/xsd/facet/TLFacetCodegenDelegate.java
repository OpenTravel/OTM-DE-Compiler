...
public abstract class TLFacetCodegenDelegate extends FacetCodegenDelegate<TLFacet> {
    ...
    public FacetCodegenElements generateElements() {
        ...
        if (doDefaultElement && hasContent()) {
            codegenElements.addFacetElement( FacetCodegenUtils.getTopLevelOwner( getSourceFacet() ),
                createElement( null ) );
        }
        ...
        if (hasContent()) {
            ...
            if (doSubstitutionGroupElement) {
                codegenElements.addSubstitutionGroupElement( facetOwner, createSubstitutionGroupElement( null ) );
            }
            ...
            for (TLAlias facetAlias : sourceFacet.getAliases()) {
                ...
                addAliasElements( facetAlias, doDefaultElement, doSubstitutionGroupElement, doNonSubstitutableElement,
                    codegenElements );
            }
        }
    ...
    /**
     * Adds all required XML schema elements for the given facet alias.
     * 
     * @param facetAlias the facet alias for which to generate XML schema elements
     * @param codegenElements the XML schema elements being generated
     */
    private void addAliasElements(TLAlias facetAlias, boolean doDefaultElement, boolean doSubstitutionGroupElement,
        boolean doNonSubstitutableElement, FacetCodegenElements codegenElements) {
        ...
        if (ownerAlias != null) {
            ...
            if (doNonSubstitutableElement) {
                codegenElements.addFacetElement( topLevelOwnerAlias, createNonSubstitutableElement( facetAlias ) );
            }
        }
    }
    ...
    public boolean hasExtensionPoint() {
        ...
        if (getExtensionPointElement() != null) {
            isExtensionPointPublished = FacetCodegenUtils.isExtensible( getSourceFacet().getOwningEntity() );

            if (!isExtensionPointPublished) {
                TLFacet sourceFacet = getSourceFacet();
                TLFacetOwner ownerExtension = FacetCodegenUtils.getFacetOwnerExtension( sourceFacet.getOwningEntity() );
        ...
        if (baseFacet == null) {
            InheritedFacetSearch facetSearch = new InheritedFacetSearch( getSourceFacet() ) {
                public boolean isMatchingCandidate(TLFacet candidateFacet) {
                    TLFacetCodegenDelegate candidateFacetDelegate =
                        (TLFacetCodegenDelegate) new FacetCodegenDelegateFactory( transformerContext )
                            .getDelegate( candidateFacet );
    ...
    /**
     * Returns true if the given facet (not necessarily the source facet) declares or inherits fields (attributes,
     * elements, or indicators) from another extended facet.
     * 
    ...
    /**
     * Returns the global type name of the local base facet for the source facet. By default, this method simply returns
     * the type name of the value returned by the 'getLocalBaseFacet()' method. Sub-classes may override if the facet's
     * schema type should extend from a type that is not a member of the compiler's model.
     * 
     * @return QName
     */
    protected QName getLocalBaseFacetTypeName() {
        ...
        return (baseFacet == null) ? null
            : new QName( baseFacet.getNamespace(), XsdCodegenUtils.getGlobalTypeName( baseFacet ) );
    }

    /**
     * Returns true if this delegate should call the base class to generate a default global element (returns true by
     * default).
     * 
     * @return boolean
     */
    ...
    /**
     * Returns true if root substitution group element for the source facet owner should be generated using the source
     * facet.
     * 
     * <p>
     * NOTE: By default, this method returns false. Sub-classes should override to implement facet-specific logic
     * indicating when the reference element should be created.
     * 
     * @return boolean
     */
    ...
    /**
     * Returns the element that will serve as the head of the substitution group for the source facet's owning entity.
     * 
     * @param ownerAlias the alias of the facet owner for which the element is being constructed (may be null)
     * @return Element
     */
    protected Element createSubstitutionGroupElement(TLAlias ownerAlias) {
        ...
        if (ownerAlias != null) {
            element.setName( XsdCodegenUtils.getSubstitutionGroupElementName( ownerAlias ).getLocalPart() );
        } else {
            element.setName( XsdCodegenUtils.getSubstitutionGroupElementName( owner ).getLocalPart() );
        }

        element.setAbstract( true );
        element.setType( new QName( sourceFacet.getNamespace(), XsdCodegenUtils.getGlobalTypeName( sourceFacet ) ) );
    ...
    /**
     * Returns a non-substitutable element for the source facet. Although it is termed "non-substitutable", all global
     * elements are members of the root "...SubGrp" element at a minimum.
     * 
     * @param facetAlias the alias of the facet for which the element is being constructed (may be null)
     * @return Element
     */
     ...
     * 
     * <p>
     * NOTE: By default, this method returns the results of the 'getElementName()' that is used to construct names for
     * the substitutable elements. Sub-classes that return true from the 'hasNonSubstitutableElement()' method should
     * override this method to supply a unique element name for the non-substitutable element.
     * 
    ...
    public String getElementName(TLAlias facetAlias) {
        ...
        if (facetAlias == null) {
            elementName = XsdCodegenUtils.getSubstitutableElementName( getSourceFacet() ).getLocalPart();
        } else {
    ...
    /**
     * If the source facet should support an extension point element, this method will return the qualified name of the
     * global extension point element to use in the type's definition. If extensions are not supported for the facet,
    ...
    protected QName getSubstitutionGroup(TLAlias facetAlias) {
        ...
        if (facetAlias != null) {
            subGrp = XsdCodegenUtils
                .getSubstitutionGroupElementName( AliasCodegenUtils.getTopLevelOwnerAlias( facetAlias ) );
        } else {
            subGrp = XsdCodegenUtils
                .getSubstitutionGroupElementName( FacetCodegenUtils.getTopLevelOwner( getSourceFacet() ) );
        }
        return subGrp;
    }

    /**
     * Returns the list of attributes to be generated for the source facet. By default, this method returns the
     * attributes declared by the source facet and inherited from like-typed facets of the extended facet owner(s).
     * Sub-classes may override to alter (or add to) the contents of the resulting list.
     * 
    ...
    /**
     * Returns the list of elements (properties) to be generated for the source facet. By default, this method returns
     * the elements declared by the source facet and inherited from like-typed facets of the extended facet owner(s)..
     * Sub-classes may override to alter (or add to) the contents of the resulting list.
     * 
    ...
    /**
     * Returns the list of indicators to be generated for the source facet. By default, this method returns the
     * indicators declared by the source facet and inherited from like-typed facets of the extended facet owner(s).
     * Sub-classes may override to alter (or add to) the contents of the resulting list.
     * 
    ...
    protected Annotated createType() {
        ...
        boolean hasSequence =
            (globalExtensionPointElement != null) || !elementList.isEmpty() || hasElementIndicators( indicatorList );
    ...
    /**
     * Common method used to populate a list of the facet's JAXB attributes using the attributes and indicators of the
     * source facet.
     * 
    ...
    protected List<Annotated> createJaxbAttributes(List<TLAttribute> attributeList, List<TLIndicator> indicatorList) {
        ...
        for (TLAttribute attribute : getAttributes()) {
            jaxbAttributes.addAll( attributeTransformer.transform( attribute ).getArtifactsOfType( Attribute.class ) );
        }
    ...
    /**
     * Common method used to populate a list of the facet's JAXB elements using the properties and indicators of the
     * source facet.
     * 
     ...
     * 
     * @param entity the model entity from which to obtain documentation
     * @return Annotation
     */
    ...
    /**
     * Returns the specified facet type from the core object, unless that facet is empty. In those cases, the facet
     * hierarchy will be traversed upwards until a non-empty candidate is found.
     * 
     * @param coreObject the core object for which to return a facet instance
     * @param facetType the preferred type of facet to return
    ...
    /**
     * Returns the specified facet type from the business object, unless that facet is empty. In those cases, the facet
     * hierarchy will be traversed upwards until a non-empty candidate is found.
     * 
     * @param businessObject the business object for which to return a facet instance
     * @param facetType the preferred type of facet to return
     * @return TLPropertyType
     */
    protected TLPropertyType getPreferredFacet(TLBusinessObject businessObject, TLFacetType facetType) {
        Deque<TLAbstractFacet> facetHierarchy = new LinkedList<>();
    ...
    /**
     * Returns the specified facet type from the facet hierarchy provided, unless that facet is empty. In those cases,
     * the facet hierarchy will be traversed upwards until a non-empty candidate is found.
     * 
     * @param facetHierarchy the facet hierarchy for the core/business object
     * @param facetType the preferred type of facet to return
     * @return TLPropertyType
     */
    ...
    /**
     * Returns true if the given list of indicators has at least one that should be declared as an XSD element.
     * 
    ...
    /**
     * Returns the code generation filter (if any) that is associated with the transformation context.
     * 
    ...
    /**
     * Helper class that searches the inheritance hierarchy of a facet, and returns a corresponding facet from the
     * next-higher owner if some (abstract) criteria are met.
     * 
     * @author S. Livezey
     */
    protected abstract class InheritedFacetSearch {
        ...
        /**
         * Searches the source facet's inheritance hierarchy, and returns the corresponding facet from the next-higher
         * owner if the test condition is met.
         * 
         * @return TLFacet
         */
        public TLFacet getInheritedFacet() {
            TLFacetOwner extendedOwner = FacetCodegenUtils.getFacetOwnerExtension( sourceFacet.getOwningEntity() );
            TLFacet inheritedFacet = null;
            TLFacet firstCandidate = null;

            while ((inheritedFacet == null) && (extendedOwner != null)) {
                TLFacet candidateFacet = FacetCodegenUtils.getFacetOfType( extendedOwner, sourceFacet.getFacetType(),
                    FacetCodegenUtils.getFacetName( sourceFacet ) );

                if (firstCandidate == null) {
                    if (candidateFacet == null) {
                        firstCandidate = getCandidateGhostFacet( extendedOwner );
        ...
        /**
         * We should never need to do this for facet types such as ID's that are the root of their substitution group,
         * and are guranteed (by validation rules) to exist. For facet types such as Request, Response, Query, etc., the
         * root of the substitution group may need to be presumed to exist if it is inherited from an extended facet
         * owner. In these cases, we will create a "ghost facet" to represent the missing base facet during code
         * generation.
         * 
         * @param extendedOwner the facet owner for which to create a candidate ghost facet
         * @return TLFacet
         */
        private TLFacet getCandidateGhostFacet(TLFacetOwner extendedOwner) {
            TLFacet firstCandidate;
         ...
         * 
         * @param candidateFacet the inherited candidate facet to analyze
         * @return boolean
         */
