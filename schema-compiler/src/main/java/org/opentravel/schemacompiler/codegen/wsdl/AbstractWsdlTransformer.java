...
/**
 * Base class for all <code>ObjectTransformer</code> implementations that are part of the WSDL code
...
public abstract class AbstractWsdlTransformer<S, T> extends AbstractCodegenTransformer<S, T> {
    ...
    /**
     * Returns the <code>CodeGenerationWsdlBindings</code> component that will be used to generate
     * binding elements for the WSDL document.
     * 
    ...
    /**
     * Creates the types section of the WSDL document with imports for each of the namespaces and
     * schemas required to define the RAS service messages.
     * 
     * @param source
     *            the business object being transformed
     * @return TTypes
     */
    protected TTypes createTypes(LibraryMember source,
        ...
        schemaAnnotation.getAppinfoOrDocumentation().add(
                XsdCodegenUtils.getServiceAppInfo(source, context.getCodegenContext()));
        ...
        if (source instanceof TLService) {
            for (TLService dependentService : getDependentServices((TLService) source)) {
                if (dependentService.getNamespace().equals(targetNamespace)) {
                    schema.getIncludeOrImportOrRedefine().add(
                            newInclude(filenameBuilder.buildFilename(dependentService, "xsd")));

                } else {
                    schema.getIncludeOrImportOrRedefine().add(
                            newImport(dependentService.getNamespace(),
            ...
            schema.getIncludeOrImportOrRedefine().add(
                    newInclude(filenameBuilder.buildFilename(source, "xsd")));
        }
        ...
        schema.getIncludeOrImportOrRedefine()
                .add(newImport(SchemaDeclarations.OTA2_APPINFO_SCHEMA));
     ...
     * 
     * @param libraryMember
     *            the library member for which to return a target namespace
     * @return String
     */
    protected String getTargetNamespace(LibraryMember libraryMember) {
        ...
        try {
            ...
            VersionScheme vScheme = VersionSchemeFactory.getInstance().getVersionScheme(
                    owningLibrary.getVersionScheme());
    ...
    /**
     * Returns a list of all operations that are defined by or inherited from previous versions of
     * the given source service.
     * 
     * <p>
     * NOTE: This method assumed that the given service is the latest version of the owning library
     * that contains service/operation content.
     * 
     * @param source
     *            the source service for which to return operations
    ...
    /**
     * Returns a list of dependent services (including the one provided) that must be included or
     * imported due to inherited operations from previous versions of the service.
     * 
     * @param service
     *            the service for which to return dependencies
    ...
    /**
     * Factory method that constructs a new <code>Include</code> instance using the schema location
     * provided.
     * 
     * @param schemaLocation
     *            the schema location of the file to import
     * @return Include
     */
    ...
    /**
     * Factory method that constructs a new <code>Import</code> instance using the information
     * provided.
     * 
     * @param schemaDeclaration
     *            the schema declaration to be imported
     * @param schemaLocation
     *            the schema location of the file to import
     * @return Import
     */
    ...
    /**
     * Creates the WSDL bindings and service elements using the information provided. This operation
     * delegates the actual content creation to the 'codeGenerationWsdlBindings' component defined
     * in the compiler's application context file. If a service endpoint URL is not specified in the
     * code generation context, the service component will not be generated.
     * 
     * @param definitions
     *            the top-level WSDL document element to which the bindings and service should be
     *            added
     * @param portType
     *            the port-type for which the bindings should be created
     * @param wsdlMessages
     *            the list of all messages defined in the WSDL document
     * @param cgContext
     *            the current code generation context
     */
    protected void addBindingAndService(TDefinitions definitions, TPortType portType,
            List<TMessage> wsdlMessages, CodeGenerationContext cgContext) {
        String serviceEndpointUrl = cgContext
                .getValue(CodeGenerationContext.CK_SERVICE_ENDPOINT_URL);

        if ((wsdlBindings != null) && (serviceEndpointUrl != null)
                && (serviceEndpointUrl.length() > 0)) {
            TBinding binding = wsdlBindings.createBinding(portType,
                    definitions.getTargetNamespace(), wsdlMessages);

            if (binding != null) {
                TService service = wsdlBindings.createService(binding,
                        definitions.getTargetNamespace(), serviceEndpointUrl);

                for (TBindingOperation operation : binding.getOperation()) {
                    TOperation portTypeOperation = getPortTypeOperation(portType,
                            operation.getName());

                    if (portTypeOperation != null) {
                        operation.setDocumentation(cloneDocumentation(portTypeOperation
                                .getDocumentation()));
                    }
                }
     ...
     * 
     * @param doc
     *            the documentation element to clone
     ...
     * 
     * @param portType
     *            the port type from which to retrieve the operation
     * @param operationName
     *            the name of the port type operation to return
     * @return TOperation
     */
