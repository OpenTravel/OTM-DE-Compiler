...
/**
 * Performs the translation from <code>TLExtensionPointFacet</code> objects to the JAXB nodes used to produce the schema
 * output.
 * 
 * @author S. Livezey
 */
public class TLExtensionPointFacetCodegenTransformer
    extends AbstractXsdTransformer<TLExtensionPointFacet,CodegenArtifacts> {
    ...
    protected ComplexType createType(TLExtensionPointFacet source) {
        ...
        if (sequence != null) {
            ...
            for (TLProperty element : elementList) {
                sequence.getParticle()
                    .add( jaxbObjectFactory.createElement( elementTransformer.transform( element ) ) );
            }
            for (TLIndicator indicator : indicatorList) {
                if (indicator.isPublishAsElement()) {
                    sequence.getParticle().add( jaxbObjectFactory
                        .createElement( (TopLevelElement) indicatorTransformer.transform( indicator ) ) );
                }
            }
        }
        ...
        for (TLAttribute attribute : attributeList) {
            type.getAttributeOrAttributeGroup()
                .addAll( attributeTransformer.transform( attribute ).getArtifactsOfType( Attribute.class ) );
        }
        for (TLIndicator indicator : indicatorList) {
            if (!indicator.isPublishAsElement()) {
