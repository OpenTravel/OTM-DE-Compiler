...
/**
 * Navigator that traverses model elements in order to produce visitor events that will
 * allow the construction of an example data set for the element(s) that are visited.
 * 
 * @author S. Livezey
 */
public class ExampleNavigator {
	...
	public void navigateListFacet(TLListFacet listFacet) {
		try {
			...
			if (canVisit(listFacet)) {
				...
				if ((itemFacet instanceof TLFacet) && (facetOwner.getRoleEnumeration().getRoles().size() > 0)) {
	...
	protected boolean canVisit(Object obj) {
		...
		if (visitationAllowed) {
			if (entityStack.peek() != obj) {
				throw new IllegalStateException("Cannot perform a visitation check on an entity before calling 'incrementRecursionCount()'.");
			}
	 ...
	 * 
	 * <p>NOTE: Since simple types are capable of directly generating examples, no
	 * depth-first traversal is performed by this method.
	 * 
	 * @param target  the named entity for which an example is to be generated
	 */
	...
	/**
	 * Recursively navigates the members (attributes, elements, and indicators) of the given facet.  The
	 * navigated members that are inherited from higher-level members of the same owner, as well as members
	 * that are inherited from extended core/business objects.
	 * 
	...
	protected void navigateFacetMembers(TLFacet facet) {
		...
		for (TLProperty element : PropertyCodegenUtils.getInheritedProperties(facet)) {
			...
			if (currentFacet.getFacetType() != previousFacetType) {
				...
				// Ignore the last element in the facet hierarchy list since it is always the current
				...
				for (int i = 0; i < (facetHierarchy.size() - 1); i++) {
					...
					if (!processedExtensionPointTypes.contains( hFacet.getFacetType() )) {
						List<TLExtensionPointFacet> facetExtensions = facetExtensionsByType.get( hFacet.getFacetType() );
		...
		// Wrap up by checking for any extension points for the current facet (take into account that
	 ...
	 * 
	 * @param facet  the facets whose extension points are to be navigated
		...
		try {
			...
			if (canVisit(element)) {
				if (element.isReference()) {
					...
					int repeatCount = (element.getRepeat() < 0) ? maxRepeatCount : Math.max(1, Math.min(maxRepeatCount, element.getRepeat()));
					TLPropertyType propertyType = element.getType();
					
					// If the property type is a core object, select the appropriate level of detail based
	...
	protected void navigateIndicator(TLIndicator indicator) {
		try {
			...
			if (canVisit(indicator)) {
				boolean isAttribute = !indicator.isPublishAsElement() || (indicator.getOwner() instanceof TLValueWithAttributes);
	...
	/**
	 * Returns the facet that should be used for the purposes of producing example data in the generated
	 * output.  Depending upon the level of detail selected in the <code>ExampleGeneratorOptions</code>,
	 * the facet that is returned may or may not be the one that is passed to this method.
	 * 
	 * @param preferredFacet  the facet with the preferred amount of detail (per the current model configuration)
	...
	/**
	 * Returns the list facet that should be used for the purposes of producing example data in the generated
	 * output.  Depending upon the level of detail selected in the <code>ExampleGeneratorOptions</code>,
	 * the list facet that is returned may or may not be the one that is passed to this method.
	 * 
	...
	/**
	 * Returns the non-empty facet from the owner with the minimum amount of detail, but with
	...
	/**
	 * Returns the entity with the specified name type name from the schema-for-schemas
	 * built-in of the given model.
	 * 
	...
	/**
	 * Returns the extension points from the model that reference the given entity facet.  The resulting
	 * map is indexed by the facet-type to which each <code>TLExtensionPointFacet</code> is associated.
	 * The lists of extension point facets include those items that reference extended entities of the
	 * facet's owner.
	 * 
		...
		if ((extensionPointRegistry == null) && (model != null)) {
			...
			for (TLLibrary library : model.getUserDefinedLibraries()) {
				for (TLExtensionPointFacet xpFacet : library.getExtensionPointFacetTypes()) {
					...
					NamedEntity extendedFacet = (extension == null) ? null : extension.getExtendsEntity();
					
					if (extendedFacet instanceof TLFacet) {
						TLFacet extendedTLFacet = (TLFacet) extendedFacet;
						List<TLExtensionPointFacet> extensionPoints = extensionPointRegistry.get( extendedTLFacet );
			...
			for (TLFacet hFacet : facetHierarchy) {
				...
				while ((hFacet != null) && (facetOwner != null)) {
					...
					hFacet = (facetOwner == null) ? null : FacetCodegenUtils.getFacetOfType(
							facetOwner, hFacet.getFacetType(), hFacet.getContext(), hFacet.getLabel() );
				}
			}
		}
		return result;
