...
public class PropertyInfoWriter
    extends AbstractFieldInfoWriter<FacetDocumentationBuilder,PropertyDocumentationBuilder> {

    /**
     * @param writer the writer for which to create an info-writer
     * @param owner the owner of the new info-writer
     */
    public PropertyInfoWriter(SubWriterHolderWriter writer, FacetDocumentationBuilder owner) {
        super( writer, owner );
    ...
    protected void addInheritedInfoSummary(Content summaryTree) {
        ...
        while (parentFacet != null) {
            if (!parentFacet.getProperties().isEmpty()) {
                addInheritedInfoHeader( parentFacet, summaryTree, "doclet.Properties_Inherited_From" );
     ...
     */
    @Override
    protected void addInheritedInfoAnchor(FacetDocumentationBuilder parent, Content inheritedTree) {
        inheritedTree
            .addContent( writer.getMarkerAnchor( "properties_inherited_from_object_" + parent.getQualifiedName() ) );
    }
    ...
    protected String getInfoTableSummary() {
        ...
        return config.getText( "doclet.Property_Table_Summary", config.getText( "doclet.Property_Summary" ),
    ...
    protected String getDetailInfoTableSummary() {
        ...
        return config.getText( "doclet.Property_Detail_Table_Summary",
            config.getText( "doclet.Property_Detail_Summary" ), config.getText( "doclet.properties" ) );
    }
