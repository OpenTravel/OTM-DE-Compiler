...
/**
 * Code generator for built-in JSON schemas referenced in a library meta-model. The behavior of this code generator is
 * to simply copy the content of the file from its source location in the local classpath to the proper output location.
 */
public class JsonSchemaBuiltInCodeGenerator extends AbstractCodeGenerator<BuiltInLibrary> {
    ...
    protected void generateJsonLibraryOutput(BuiltInLibrary source, CodeGenerationContext context)
        ...
        if (!source.getNamespace().equals( XMLConstants.W3C_XML_SCHEMA_NS_URI )) {
            try (InputStream is =
                source.getSchemaDeclaration().getContent( CodeGeneratorFactory.JSON_SCHEMA_TARGET_FORMAT )) {
                if (is != null) {
    ...
    protected boolean canGenerateOutput(BuiltInLibrary source, CodeGenerationContext context) {
        ...
        return super.canGenerateOutput( source, context ) && ((filter == null) || filter.processLibrary( source ));
    }
    ...
    /**
     * Handles the generation of XSD output for user-defined (<code>TLLibrary</code>) built-ins on behalf of the owning
     * built-in code generator.
     * 
     * @author S. Livezey
     */
    private class JsonSchemaUserBuiltInCodeGenerator extends AbstractJsonSchemaCodeGenerator<BuiltInLibrary> {
        ...
        /**
         * @see org.opentravel.schemacompiler.codegen.json.AbstractJsonSchemaCodeGenerator#getOutputFile(org.opentravel.schemacompiler.model.TLModelElement,
         *      org.opentravel.schemacompiler.codegen.CodeGenerationContext)
         */
        @Override
        protected File getOutputFile(BuiltInLibrary source, CodeGenerationContext context) {
