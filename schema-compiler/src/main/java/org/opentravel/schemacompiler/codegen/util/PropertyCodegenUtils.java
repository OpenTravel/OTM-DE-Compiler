...
public class PropertyCodegenUtils {
    ...
    /**
     * Returns the XML element name for a property of the given type. This method presumes that the property type has
     * already been determined to be a complex type with a managed name (or an alias of one).
     * 
    ...
    /**
     * Adds the local properties from the given facet to the list of inherited properties being constructed.
     * 
    ...
    /**
     * Adds the given property to the list if it meets all of the criteria for being considered a local property that is
     * not eclipsed by lower-level substitution group properties.
     * 
    ...
    /**
     * Returns the inheritance root fo the given property type. This method presumes that the property type has already
     * been determined to be a facet or facet alias.
     * 
    ...
    /**
     * Returns the inheritance root fo the given property type. This method presumes that the property type has already
     * been determined NOT to be a facet or facet alias.
     * 
     * @param propertyType the property type for which to return the inheritance root
     * @return NamedEntity
     */
    private static NamedEntity getNonFacetTypeInheritanceRoot(TLPropertyType propertyType) {
        ...
        if (facetOwner != null) {
            ...
            while (parentEntity != null) {
                ...
                inheritanceRoot =
                    (ownerAlias == null) ? facetOwner : ((TLAliasOwner) facetOwner).getAlias( ownerAlias.getName() );
                parentEntity = FacetCodegenUtils.getFacetOwnerExtension( parentEntity );
            }
    ...
    public static boolean isReferenceProperty(TLProperty property) {
        ...
        if (property.isReference()) {
            ...
            if ((propertyType != null) && XMLConstants.W3C_XML_SCHEMA_NS_URI.equals( propertyType.getNamespace() )) {
                result =
                    propertyType.getLocalName().equals( "IDREF" ) || propertyType.getLocalName().equals( "IDREFS" );
            }
        }
        return result;
    }
    ...
    protected static TLAbstractFacet[] getAlternateFacets(TLAbstractFacet referencedFacet) {
        ...
        if (referencedFacet instanceof TLListFacet) {
            results =
                getAlternateFacets( (TLCoreObject) referencedFacet.getOwningEntity(), (TLListFacet) referencedFacet );
    ...
    protected static TLAbstractFacet[] getAlternateFacets(TLFacet facet, TLAbstractFacet referencedFacet) {
        ...
        if (facet.getOwningEntity() instanceof TLBusinessObject) {
            if (referencedFacet.getOwningEntity() instanceof TLBusinessObject) {
                results =
                    getAlternateFacets( facet, (TLBusinessObject) referencedFacet.getOwningEntity(), referencedFacet );

            } else if (referencedFacet.getOwningEntity() instanceof TLCoreObject) {
                results =
                    getAlternateFacets( facet, (TLCoreObject) referencedFacet.getOwningEntity(), referencedFacet );
            }
            ...
            if (referencedFacet.getOwningEntity() instanceof TLBusinessObject) {
                results = getAlternateFacets( (TLBusinessObject) referencedFacet.getOwningEntity(), referencedFacet );
     ...
     * 
     * @param referencedOwner the owner of the referenced facet
     * @param referencedFacet the referenced facet for which an alternate facet should be returned
     * @return TLAbstractFacet[]
     */
     ...
     * Simple dispatch method used to decompose the problem from the primary 'getAlternateFacets()' method.
     * 
     * @param originatingFacet the facet from which the reference originated
     * @param referencedOwner the owner of the referenced facet
     * @param referencedFacet the referenced facet for which an alternate facet should be returned
     * @return TLAbstractFacet[]
     */
        ...
        switch (originatingFacet.getFacetType()) {
            case ID:
                switch (referencedFacet.getFacetType()) {
                    ...
                    case SUMMARY:
                        results =
                            new TLAbstractFacet[] {referencedOwner.getIdFacet(), referencedOwner.getDetailFacet()};
                        break;
                    case DETAIL:
                    case CUSTOM:
                        results =
                            new TLAbstractFacet[] {referencedOwner.getSummaryFacet(), referencedOwner.getIdFacet()};
                        break;
                    default:
                        break;
                }
            ...
            case QUERY:
                switch (referencedFacet.getFacetType()) {
                    ...
                    case SUMMARY:
                        results =
                            new TLAbstractFacet[] {referencedOwner.getDetailFacet(), referencedOwner.getIdFacet()};
                        break;
                    case DETAIL:
                        results =
                            new TLAbstractFacet[] {referencedOwner.getSummaryFacet(), referencedOwner.getIdFacet()};
        ...
        switch (originatingFacet.getFacetType()) {
            case ID:
                switch (referencedFacet.getFacetType()) {
                    case SIMPLE:
                        results =
                            new TLAbstractFacet[] {referencedOwner.getSummaryFacet(), referencedOwner.getDetailFacet()};
                        break;
                    case SUMMARY:
                        results =
                            new TLAbstractFacet[] {referencedOwner.getSimpleFacet(), referencedOwner.getDetailFacet()};
                        break;
                    case DETAIL:
                        results =
                            new TLAbstractFacet[] {referencedOwner.getSummaryFacet(), referencedOwner.getSimpleFacet()};
                        break;
                    default:
                        break;
                }
            ...
            case SUMMARY:
                switch (referencedFacet.getFacetType()) {
                    ...
                    case SUMMARY:
                        results =
                            new TLAbstractFacet[] {referencedOwner.getDetailFacet(), referencedOwner.getSimpleFacet()};
                        break;
                    case DETAIL:
                        results =
                            new TLAbstractFacet[] {referencedOwner.getSummaryFacet(), referencedOwner.getSimpleFacet()};
     ...
     * @return TLAbstractFacet[]
     */
    protected static TLAbstractFacet[] getAlternateFacets(TLCoreObject referencedOwner,
        TLAbstractFacet referencedFacet) {
        TLAbstractFacet[] results = new TLAbstractFacet[0];
