...
public abstract class AbstractXsdTransformer<S,T> extends AbstractCodegenTransformer<S,T> {
	...
	/**
	 * Identify imports for any compile-time dependencies that would not otherwise be covered by
	 * the list of explicitly imported namespaces.
	 * 
	...
	private void addImpliedBuiltIns(Schema schema, AbstractLibrary sourceLibrary, List<AbstractLibrary> importList,
		...
		for (SchemaDeclaration schemaDependency : getCompileTimeDependencies()) {
			...
			String schemaLocation = builtInFolder
	...
	/**
	 * Removes redundant imports from the given list that would be covered by includes within
	 * the dependent files.
	 * 
	...
	/**
	 * Add import statements for all of the import dependencies in the given list to the
	 * schema provided.
	 * 
	...
	private void addImportStatements(Schema schema, List<AbstractLibrary> importList, String builtInFolder,
		...
		addImport( schema, SchemaDeclarations.OTA2_APPINFO_SCHEMA.getNamespace(),
				builtInFolder + SchemaDeclarations.OTA2_APPINFO_SCHEMA.getFilename( CodeGeneratorFactory.XSD_TARGET_FORMAT ),
				false );
	}
	...
	private void addImport(Schema schema, String namespace, String defaultSchemaLocation,
		...
		ImportSchemaLocations importLocations = ((AbstractXsdCodeGenerator<?>) context.getCodeGenerator())
			.getImportSchemaLocations();
	...
	private List<AbstractLibrary> getBuiltInIncludes(BuiltInLibrary builtIn, ApplicationContext appContext) {
		...
		for (String builtInDependency : builtIn.getSchemaDeclaration().getDependencies()) {
			...
			if (dependency.getNamespace().equals( builtIn.getNamespace() )) {
				AbstractLibrary dependentLib = builtIn.getOwningModel().getLibrary( dependency.getNamespace(),
						dependency.getName() );
	...
	/**
	 * Find all of the libraries identified as compile-time dependencies that are assigned to
	 * the source library's local namespace.
	 * 
