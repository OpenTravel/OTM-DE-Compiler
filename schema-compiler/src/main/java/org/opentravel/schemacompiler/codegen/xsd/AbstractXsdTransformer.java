...
/**
 * Base class for all <code>ObjectTransformer</code> implementations that are part of the XML schema
...
public abstract class AbstractXsdTransformer<S, T> extends AbstractCodegenTransformer<S, T> {
     ...
     * 
     * @param schema
     *            the target XML schema that will receive the imports
     * @param sourceLibrary
     *            the source library for which the include directives are being created
     * @param filenameBuilder
     *            the filename builder to use for all schema locations
     * @param importFilter
     *            filter that only allows imported dependencies from the specified source library
     */
    protected void addImports(Schema schema, AbstractLibrary sourceLibrary,
        ...
        // Now, filter out redundant imports that would be covered by includes within the dependent
        // files
        ...
        for (AbstractLibrary importedLibrary : importArray) {
            ...
            while (redundantCandidates.hasNext()) {
                ...
                if (isIncludedBy(importedLibrary, redundantCandidate, includeDependencies)) {
                    includeDependencies.remove(redundantCandidate); // protects against omissions
                                                                    // when circular includes are
                                                                    // defined
        ...
        for (AbstractLibrary importedLib : importList) {
            ...
            if ((importedLib instanceof XSDLibrary) && (importFilter != null)
                ...
                addImport(schema, namespace,
                        importFolder
                                + new LegacySchemaExtensionFilenameBuilder<AbstractLibrary>(
                                        filenameBuilder).buildFilename(importedLib, "xsd"), true);
            } else {
                addImport(schema, namespace,
                        importFolder + filenameBuilder.buildFilename(importedLib, "xsd"), false);
            }
        }
     ...
     * 
     * @param schema
     *            the target XML schema that will receive the import
     * @param namespace
     *            the namespace to be imported
     * @param defaultSchemaLocation
     *            the schema location of the file to import if one has not been specified in the
     *            <code>ImportSchemaLocations</code>
     * @param isLegacyExtensionImport
     *            indicates that this is an import of a legacy extension schema
     */
    private void addImport(Schema schema, String namespace, String defaultSchemaLocation,
            boolean isLegacyExtensionImport) {
        ImportSchemaLocations importLocations = ((AbstractXsdCodeGenerator<?>) context
                .getCodeGenerator()).getImportSchemaLocations();
     ...
     * 
     * @param libraryList
     *            the list of libraries to include in the dependency analysis
        ...
        for (AbstractLibrary library : libraryList) {
            ...
            if (library instanceof BuiltInLibrary) {
                ...
                for (String builtInDependency : builtIn.getSchemaDeclaration().getDependencies()) {
                    SchemaDeclaration dependency = (SchemaDeclaration) appContext
                            .getBean(builtInDependency);

                    // The dependency is an include if both namespaces are the same
                    if (dependency.getNamespace().equals(builtIn.getNamespace())) {
                        AbstractLibrary dependentLib = library.getOwningModel().getLibrary(
                                dependency.getNamespace(), dependency.getName());
     ...
     * 
     * @param sourceLibrary
     *            the source library of the potential include dependency
     * @param includeCandidate
     *            the candidate target library of the include
     * @param includeDependencies
     *            the pre-calculated include dependencies
     * @return boolean
     */
    ...
    /**
     * Returns the list of URL's for all of the included libraries. NOTE: Built-in libraries cannot
     * be processed by this routine.
     * 
     * @param library
     *            the user-defined (.otm) or legacy (.xsd) library
     * @return List<URL>
     */
     ...
     * 
     * @param schema
     *            the target XML schema that will receive the includes
     * @param sourceLibrary
     *            the source library for which the include directives are being created
     * @param filenameBuilder
     *            the filename builder to use for all schema locations
     * @param includeFilter
     *            filter that only allows included dependencies from the specified source library
     */
    protected void addIncludes(Schema schema, AbstractLibrary sourceLibrary,
        ...
        // of includes.
        includeList.addAll(sourceLibrary.getOwningModel().getLibrariesForNamespace(
        ...
        for (AbstractLibrary includedLib : includeList) {
            ...
            if ((includeFilter == null) || includeFilter.processLibrary(includedLib)
                ...
                addInclude(schema,
                        includeFolder + filenameBuilder.buildFilename(includedLib, "xsd"));
            }
            if ((includedLib instanceof XSDLibrary) && (includeFilter != null)
                ...
                addInclude(schema,
                        includeFolder
                                + new LegacySchemaExtensionFilenameBuilder<AbstractLibrary>(
                                        filenameBuilder).buildFilename(includedLib, "xsd"));
            }
        }
    }
     ...
     * 
     * @param schema
     *            the target XML schema that will receive the include
     * @param schemaLocation
     *            the schema location of the file to include
     */
    ...
    /**
     * Correlates the global element definitions for the given source object and returns the
     * consolidated list of generated schema artifacts.
     * 
     * @param source
     *            the source entity for which the artifacts were generated
     * @param codegenElements
     *            the schema elements that were generated for the source entity
     * @param codegenArtifacts
     *            the non-element schema artifacts that were generated for the source entity
