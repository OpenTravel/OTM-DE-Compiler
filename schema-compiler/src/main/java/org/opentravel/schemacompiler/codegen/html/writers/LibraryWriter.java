...
public class LibraryWriter extends SubWriterHolderWriter implements LibrarySummaryWriter {
    ...
    /**
     * Constructor to construct LibraryWriter object and to generate "library-summary.html" file in the respective
     * library directory.
     *
     * @param configuration the configuration of the doclet.
     * @param library Library under consideration.
     * @param prev Previous library in the sorted array.
     * @param next Next library in the sorted array.
     * @throws IOException thrown if an error occurs during writer initialization
     */
    public LibraryWriter(Configuration configuration, LibraryDocumentationBuilder library,
        LibraryDocumentationBuilder prev, LibraryDocumentationBuilder next) throws IOException {
        super( configuration, DirectoryManager.getDirectoryPath( library.getName() ), OUTPUT_FILE_NAME,
            DirectoryManager.getRelativePath( library.getName() ) );
        this.prev = prev;
        this.next = next;
    ...
    public Content getHeader() {
        ...
        Content tHeading = HtmlTree.heading( HtmlConstants.TITLE_HEADING, true, HtmlStyle.TITLE, libraryLabel );
        tHeading.addContent( getSpace() );
        Content libraryHead = new RawHtml( library.getName() );
        tHeading.addContent( libraryHead );
        div.addContent( tHeading );
        tHeading = HtmlTree.heading( HtmlConstants.TITLE_HEADING, true, HtmlStyle.TITLE, namespaceLabel );
        tHeading.addContent( getSpace() );
    ...
    public void addNamespaceDescription(Content packageContentTree) {
        ...
        if (doc != null) {
            packageContentTree.addContent( getMarkerAnchor( "library_description" ) );
            Content h2Content =
                new StringContent( configuration.getText( "doclet.Library_Description", library.getName() ) );
            packageContentTree.addContent( HtmlTree.heading( HtmlConstants.PACKAGE_HEADING, true, h2Content ) );
    ...
    public Content getNavLinkPrevious() {
        ...
        if (prev == null) {
            ...
            String path = DirectoryManager.getRelativePath( library.getName(), prev.getName() );
            li = HtmlTree.li( getHyperLink( path + OUTPUT_FILE_NAME, "", prevLibraryLabel, "", "" ) );
        }
        return li;
    }
    ...
    public Content getNavLinkNext() {
        ...
        if (next == null) {
            ...
            String path = DirectoryManager.getRelativePath( library.getName(), next.getName() );
            li = HtmlTree.li( getHyperLink( path + OUTPUT_FILE_NAME, "", nextLibraryLabel, "", "" ) );
        }
        return li;
    }
