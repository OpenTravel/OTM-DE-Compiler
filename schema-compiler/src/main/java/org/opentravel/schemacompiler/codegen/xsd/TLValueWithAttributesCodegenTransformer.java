...
/**
 * Performs the translation from <code>TLValueWithAttributes</code> objects to the JAXB nodes used to produce the schema
 * output.
 * 
 * @author S. Livezey
 */
public class TLValueWithAttributesCodegenTransformer
    extends AbstractXsdTransformer<TLValueWithAttributes,CodegenArtifacts> {
    ...
    public CodegenArtifacts transform(TLValueWithAttributes source) {
        ...
        for (TLAttribute modelAttribute : PropertyCodegenUtils.getInheritedAttributes( source )) {
            extType.getAttributeOrAttributeGroup()
                .addAll( attributeTransformer.transform( modelAttribute ).getArtifactsOfType( Attribute.class ) );
        }
