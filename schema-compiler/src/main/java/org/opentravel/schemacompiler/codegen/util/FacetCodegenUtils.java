...
public class FacetCodegenUtils {
    ...
    /**
     * For contextual facets, this method will return the value of the 'name' field. For non-contextual facets, null
     * will be returned.
     * 
    ...
    /**
     * Returns all of the facets from the specified owner of the requested type. For non-contextual facet types (e.g.
     * summary or ID), the list that is returned will contain a single element. For contextual facet types (e.g. query
     * or custom) a list of zero or more matching facets will be returned, regardless of their context or label values.
     * 
    ...
    /**
     * Returns a facet of the specified type from the given owner. If no such facet is available from the owner, this
     * method will return null.
     * 
     * <p>
     * NOTE: This method assumes null values for the facet's context and label when looking up contextual facets.
     * 
     ...
     */
    @Deprecated
    public static TLFacet getFacetOfType(TLFacetOwner owner, TLFacetType facetType, String facetContext,
        String facetLabel) {
     ...
     * @return TLFacet
     */
    private static TLFacet getChoiceObjectFacetOfType(TLChoiceObject owner, TLFacetType facetType, String facetName) {
        TLFacet memberFacet;
        switch (facetType) {
            case SHARED:
    ...
    /**
     * Returns the contextual facet from the list with the specified name. If no such facet exists, this method will
     * return null.
     * 
    ...
    /**
     * Returns true if the given facet owner is an extensible business object, core, or operation entity.
     * 
    ...
    /**
     * Returns the <code>TLFacetOwner</code> instance that is extended by the facet owner that is passed to this method.
     * If the owner does not extend another model entity, this method will return null.
     * 
    ...
    /**
     * Returns the contextual facet that conforms with the specified path relative to the given owner. If no such facet
    ...
    /**
     * Returns the inheritance hierarchy for facets with the same owner. For EXAMPLE, passing the detail facet of a
     * business object to this method would result in a list containing the ID, summary, and detail facets from the
     * original facet's owner. The hierarchy that is returned is sorted from the highest level of the inheritance
     * hierarchy to the lowest. At a minimum, the facet that is passed to this method is guranteed to be a member (and
     * the last element of) of the resulting list.
     * 
     ...
     * 
     * @param entity the complex-type entity for which to return available facets
     * @return List&lt;TLFacet&gt;
     */
    public static List<TLFacet> getAvailableFacets(TLComplexTypeBase entity) {
        ...
        if (entity instanceof TLBusinessObject) {
            ...
            addContextualFacets( findGhostFacets( businessObject, TLFacetType.CUSTOM ), facetList,
            ...
            addContextualFacets( findGhostFacets( choiceObject, TLFacetType.CHOICE ), facetList,
                new HashSet<TLContextualFacet>() );
    ...
    /**
     * Returns the available facet aliases for the given parent object alias. The alias passed to this method must be a
     * direct alias of a core, choice, or business object (i.e. not a facet alias).
     * 
     ...
     */
    private static void addContextualFacets(List<TLContextualFacet> facetsToAdd, List<TLFacet> contextualFacets,
        ...
        for (TLContextualFacet facet : facetsToAdd) {
            if (!visitedFacets.contains( facet )) {
                ...
                addContextualFacets( FacetCodegenUtils.findGhostFacets( facet, facet.getFacetType() ), contextualFacets,
                    visitedFacets );
            }
        }
    }
    ...
    /**
     * If the given facet declares or inherits fields, this method will add it to the list provided.
     * 
    ...
    /**
     * Returns a list of "ghost facets" for the given owner. A ghost facet occurs when a contextual facet (e.g. custom
     * or query) with a certain context/label combination is declared on an ancestor entity that is extended by the
     * given 'facetOwner' but no corresponding entity is declared on the 'facetOwner' itself. In these cases, we must
     * compile XSD artifacts for an inherited "ghost facet", even though it was not explicitly declared in the model.
     * 
    ...
    public static List<TLContextualFacet> findGhostFacets(TLFacetOwner facetOwner, TLFacetType facetType) {
        ...
        for (TLContextualFacet inheritedFacet : inheritedFacets) {
            TLContextualFacet declaredFacet =
                (TLContextualFacet) getFacetOfType( facetOwner, facetType, inheritedFacet.getName() );
    ...
    /**
     * Returns a list of "ghost facets" for the given resource. A ghost facet occurs when an action facet with a certain
     * name is declared on an ancestor resource that is extended by the given 'resource' but no corresponding entity is
     * declared on the 'resource' itself. In these cases, we must compile XSD artifacts for an inherited "ghost facet",
     * even though it was not explicitly declared in the model.
     * 
    ...
    /**
     * Returns the list of all non-local ghost facets that should be included in the code generation output for the
     * given library. The list of facets returned by this method includes any nested ghost facets that might exist in
     * the facet hierarchy.
     * 
    ...
    public static List<TLContextualFacet> findNonLocalGhostFacets(TLLibrary library) {
        ...
        for (TLFacetOwner originalOwner : topLevelOwners) {
            findNonLocalGhostFacets( originalOwner, library, nonLocalFacets, extensionRegistry,
                new HashSet<TLFacetOwner>() );
        }
        return nonLocalFacets;
    }

    /**
     * Recursive routine that navigate through all of the extending entities to search for ghost facets that should be
     * generated in our starting library.
     * 
    ...
    /**
     * Recursively collects all non-local ghost facets for the given owner that should be generated in the original
     * library.
     * 
    ...
    /**
     * Constructs a map of all extension relationships within the given OTM model. The values in the resulting map
     * represent all entities which extend the facet-owner key.
     * 
        ...
        ModelElementVisitor visitor = new ModelElementVisitorAdapter() {
            ...
            public boolean visitExtension(TLExtension extension) {
                ...
                if ((extendingEntity instanceof TLFacetOwner) && (extendedEntity instanceof TLFacetOwner)) {
                    registry.computeIfAbsent( (TLFacetOwner) extendedEntity, e -> new ArrayList<>() )
                        .add( (TLFacetOwner) extendingEntity );
                }
                return true;
            }
        };
    ...
    /**
     * Returns the type of operation based on the configuration of the request, response, and/or notification facets.
     * 
