...
/**
 * Performs the translation from <code>TLActionFacet</code> objects to the JAXB nodes used to produce the schema output.
 */
public class TLActionFacetCodegenTransformer extends AbstractXsdTransformer<TLActionFacet,CodegenArtifacts> {
    ...
    private CodegenArtifacts generateEmptyWrapper(TLActionFacet source) {
        ...
        sequence.getParticle().add( jaxbObjectFactory.createElement( elementTransformer.transform( boElement ) ) );
    ...
    /**
     * Returns a <code>FacetCodegenDelegate</code> to generate schema artifacts for the given facet.
     * 
     * @param source the source action facet for which artifacts are being generated
