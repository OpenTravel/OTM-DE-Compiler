...
/**
 * Performs the translation from <code>TLExtensionPointFacet</code> objects to the JSON schema elements used to produce
 * the output.
 */
public class TLExtensionPointFacetJsonCodegenTransformer
    extends AbstractJsonSchemaTransformer<TLExtensionPointFacet,CodegenArtifacts> {
    ...
    public CodegenArtifacts transform(TLExtensionPointFacet source) {
        ...
        for (TLAttribute attribute : attributeList) {
            defSchema.getProperties().addAll(
                attributeTransformer.transform( attribute ).getArtifactsOfType( JsonSchemaNamedReference.class ) );
        }
        for (TLIndicator indicator : indicatorList) {
            if (!indicator.isPublishAsElement()) {
                defSchema.getProperties().add( indicatorTransformer.transform( indicator ) );
