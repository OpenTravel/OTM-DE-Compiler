...
public final class MinorVersionHelper extends AbstractVersionHelper {
		...
		if ((baseNamespace != null) && (library.getOwningModel() != null)
			...
			if (versionScheme.isPatchVersion( library.getNamespace() )) {
				...
				do {
					...
					priorMinorVersionNS = versionScheme.setVersionIdentifier( library.getNamespace(),
							priorMinorVersion );
	...
	public <V extends Versioned> List<V> getLaterMinorVersions(V versionedEntity) throws VersionSchemeException {
		...
		if (versionedEntity instanceof LibraryElement) {
			...
			if (versionedMember.getOwningLibrary() instanceof TLLibrary) {
				List<TLLibrary> minorVersionLibraries = getLaterMinorVersions(
						(TLLibrary) versionedMember.getOwningLibrary() );
				
				for (TLLibrary minorVersionLib : minorVersionLibraries) {
					NamedEntity minorVersionEntity = handler.retrieveExistingVersion( versionedEntity,
							minorVersionLib );
	...
	/**
	 * Returns all versioned entities of the given library that are the same type as the
	 * entity provided.
	 * 
	...
	/**
	 * Adds the entity from the given list to the major version family if it is a
	 * version of the original versioned entity.
	 * 
	...
	public TLLibrary createNewMinorVersion(TLLibrary library, File libraryFile)
		...
		try {
			VersionScheme versionScheme = VersionSchemeFactory.getInstance()
				.getVersionScheme( library.getVersionScheme() );
			...
			TLLibrary latestMinorVersion = minorVersions.isEmpty() ? library
					: minorVersions.get( minorVersions.size() - 1 );
