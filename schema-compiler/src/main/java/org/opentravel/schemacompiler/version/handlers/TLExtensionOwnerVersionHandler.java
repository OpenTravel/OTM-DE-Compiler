...
public abstract class TLExtensionOwnerVersionHandler<V extends TLVersionedExtensionOwner> extends VersionHandler<V> {
	...
	protected NamedEntity getBaseExtension(V entity) throws VersionSchemeException {
		...
		if (entity != null) {
			...
			V earliestMinorVersion = priorMinorVersions.isEmpty() ? null
					: priorMinorVersions.get( priorMinorVersions.size() - 1 );
	...
	/**
	 * If the source contextual facet is local, the fields and content are rolled up to the
	 * target facet.  If the target facet is null, a new one will be created and assigned to
	 * the target facet owner provided.
	 * 
	 ...
	 */
	protected <T extends TLFacetOwner> void rollupLocalContextualFacet(TLContextualFacet sourceFacet, TLContextualFacet targetFacet,
			T targetFacetOwner, Map<String,TLFacet> sourceFacets, Map<String,TLFacet> targetFacets,
			VersionHandlerMergeUtils mergeUtils, Assignment<T,TLContextualFacet> facetAssignment) {
		if (sourceFacet.isLocalFacet()) {
			if (targetFacet == null) {
				targetFacet = new TLContextualFacet();
				targetFacet.setName( sourceFacet.getName() );
