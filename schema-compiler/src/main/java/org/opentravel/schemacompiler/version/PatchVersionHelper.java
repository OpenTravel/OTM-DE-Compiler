...
/**
 * Helper methods used to construct new patch versions of <code>TLLibrary</code> instances and their
 * entity members.
 * 
 * @author S. Livezey
 */
public final class PatchVersionHelper extends AbstractVersionHelper {
    ...
    /**
     * Returns the list of all extension point facets that reference the given versioned entity from
     * a library whose version is a patch of the given entity.
     * 
     * @param versionedEntity
     *            the versioned entity for which to return patches
            ...
            if (versionedMember.getOwningLibrary() instanceof TLLibrary) {
                ...
                if (!patchVersionLibraries.isEmpty()) {
                    ...
                    for (TLLibrary patchVersionLib : patchVersionLibraries) {
                        for (TLExtensionPointFacet xpFacet : patchVersionLib
                                .getExtensionPointFacetTypes()) {
    ...
    /**
     * Returns the libraries in the current model that are eligible to accept new patches for the
     * given versioned entity facet.
     * 
     * @param versionedEntityFacet
     *            the versioned entity facet for which to return eligible patch libraries
    ...
    /**
     * Creates a new patch version of the given library. The library that is passed to this method
     * must be a major or minor version intance. The resulting patch number of the resulting patch
     * library will be one greater than the latest known patch for the library that is passed to
     * this method.
     * 
     * <p>
     ...
     * 
     * @param library
     *            the library for which to construct a new patch version
     * @return TLLibrary
     * @throws VersionSchemeException
     ...
     */
    public TLLibrary createNewPatchVersion(TLLibrary library) throws VersionSchemeException,
    ...
    public TLLibrary createNewPatchVersion(TLLibrary library, File libraryFile)
        ...
        try {
            ...
            TLLibrary latestPatch = patchVersions.isEmpty() ? null : patchVersions
                    .get(patchVersions.size() - 1);
    ...
    /**
     * Creates a new <code>TLExtensionPointFacet</code> that is a patch for the given versioned
     * entity. The contents of the patch returned by this method will be empty.
     * 
     * @param versionedEntity
     *            the versioned entity for which to create a patch
     * @param facetToPatch
     *            the facet to be patched from the versioned entity
     * @param targetLibraryVersion
     *            the target library that is assigned to the desired patch version number
