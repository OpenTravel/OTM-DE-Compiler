...
/**
 * Comparator used to sort <code>Versioned</code> objects according to their version
 * identifier.
 * 
 * @author S. Livezey
 */
public class OTA2VersionComparator implements Comparator<Versioned> {
	...
	/**
	 * Returns a <code>VersionIdentifier</code> instance that can be used to perform comparisons
	 * of the given version ID.
	 * 
	...
	private class VersionIdentifier implements Comparable<VersionIdentifier> {
		...
		public VersionIdentifier(String versionIdentifier) {
			this.versionParts = (versionIdentifier == null) ?
					new int[] { 0, 0, 0 } : OTA2VersionScheme.splitVersionIdentifier(versionIdentifier);
		}
