...
/**
 * Base class for all helper functions that are specific to a particular
 * <code>Versioned</code> entity type.
 * 
...
public abstract class VersionHandler<V extends Versioned> {
	...
	public V getVersionExtension(V entity) throws VersionSchemeException {
        ...
        if ((extendedEntity != null)
                && extendedEntity.getClass().equals(entity.getClass())
	...
	/**
	 * Returns the entity that is extended by the given one, or null if no extension
	 * is present.
	 * 
	...
	/**
	 * Returns the base extension of the given entity.  The base extension is
	 * the extended type from the earliest minor version in the entity's major
	 * version chain.  For this reason, the base extension will never be a prior
	 * minor version of the entity.
	 * 
    ...
    /**
     * Constructs a new version of the given entity that is an exact copy except that it does not
     * contain any child members (e.g. for a business object, the facets will  not contain any
     * attributes, elements, or indicators.  The new version returned by this method is configured
    ...
    /**
     * Returns a newer version of the given entity from the target library.  If a new version
     * of the entity does not yet exist in the library, a new version instance is created
     * automatically.
     * 
     * @param origVersion  the original version from which to construct the new version instance
     * @param targetLibrary  the library to which the new version should be assigned
    ...
    /**
     * Returns an existing version of the given entity from the target library.  If
     * a version with the original version's name is not defined in the library, this
    ...
    /**
     * Rolls up the contents of the given minor entity version into a major version of the
     * same entity in the library provided.  If a major version of the entity does not yet
     * exist in the library, one will be created automatically.
     * 
     ...
     * essentially a merge of the contents (i.e. attributes, properties, indicators, etc.) from
     * the minor version.
     * 
