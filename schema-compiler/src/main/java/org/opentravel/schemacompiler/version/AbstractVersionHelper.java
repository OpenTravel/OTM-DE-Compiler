...
public abstract class AbstractVersionHelper {
	...
	/**
	 * Adds the candidate library to the list of patch versions if it is an applicable patch
	...
	Versioned getPatchedVersion(TLExtensionPointFacet xpFacet) throws VersionSchemeException {
		NamedEntity extendedEntity = (xpFacet.getExtension() == null) ? null
		...
		if (extendedEntity instanceof TLFacet) {
			...
			if (extendedFacetOwner instanceof Versioned) {
				VersionScheme versionScheme = VersionSchemeFactory.getInstance()
					.getVersionScheme( xpFacet.getVersionScheme() );
	...
	private void loadLaterVersions(List<RepositoryItem> laterVersions, List<TLLibrary> existingLaterVersions,
		...
		if (!laterVersions.isEmpty()) {
			for (RepositoryItem item : laterVersions) {
				try {
					ProjectItem pItem = activeProject.getProjectManager().addManagedProjectItem( item,
							activeProject );
		...
		for (TLExtensionPointFacet patch : patchLibrary.getExtensionPointFacetTypes()) {
			...
			if (patchedEntity != null) {
				try {
					...
					Versioned newEntityVersion = handler.createOrRetrieveNewVersion( patchedEntity,
							minorVersionLibrary );
	...
	/**
	 * Returns the contextual facet that should be the ultimate target for the patch
	 * rollup.
	 * 
	...
	boolean isDuplicateOfPublishedLibrary(TLLibrary library) throws VersionSchemeException {
		...
		if (activeProject != null) {
			...
			if ((versionScheme != null) && (item != null) && ((item.getState() == RepositoryItemState.UNMANAGED)
				...
				try {
					String managedFilename = versionScheme.getDefaultFileHint( library.getNamespace(),
							library.getName() );
	...
	File getDefaultLibraryFileLocation(TLLibrary newVersionLibrary, TLLibrary originalLibraryVersion)
		...
		String newVersionFilename = versionScheme.getDefaultFileHint( newVersionLibrary.getNamespace(),
				newVersionLibrary.getName() );
