...
/**
 * Base class for version helpers used to retrieve related versions of a library or entity from a model or to create new
 * versions.
 * 
 * @author S. Livezey
 */
public abstract class AbstractVersionHelper {
     ...
     * are created or opened by this helper, they will be automatically added to the active project that is passed to
     * this constructor.
     * 
    ...
    /**
     * Returns the version chain for the given library. The resulting list includes all dependent versions starting with
     * the one provided and ending with the major version that started the chain.
     * 
    ...
    public List<TLLibrary> getVersionChain(TLLibrary library) throws VersionSchemeException {
        ...
        if (library.getOwningModel() == null) {
            throw new IllegalStateException( NO_OWNING_MODEL + library.getLibraryUrl().toExternalForm() );
        }
    ...
    /**
     * Returns the library from the model with the specified name and namespace, or null if no such library exists.
     * 
    ...
    List<TLLibrary> getLaterPatchVersions(TLLibrary library) throws VersionSchemeException {
        ...
        for (TLLibrary lib : library.getOwningModel().getUserDefinedLibraries()) {
            ...
            addIfPatchVersion( lib, library, minorVersionNS, patchVersionList, versionScheme );
        }
    ...
    /**
     * Adds the candidate library to the list of patch versions if it is an applicable patch for the original library.
     * 
     * @param patchCandidate the candidate library to test and possibly add
     * @param origLibrary the original library to test against
     * @param minorVersionNS the minor version namespace of the original library
     * @param patchVersionList the list to which a patch version will be added
     * @param versionScheme the version scheme of the libraries
     */
    @SuppressWarnings("squid:S2692") // False Sonar finding - code works as intended
    private void addIfPatchVersion(TLLibrary patchCandidate, TLLibrary origLibrary, String minorVersionNS,
        List<TLLibrary> patchVersionList, VersionScheme versionScheme) {
        if (minorVersionNS != null) { // original library was a major or minor version
            String adjustedVersion = versionScheme
                .decrementMinorVersion( versionScheme.incrementMinorVersion( patchCandidate.getVersion() ) );
            String adjustedLibNS = versionScheme.setVersionIdentifier( patchCandidate.getNamespace(), adjustedVersion );

            if (adjustedLibNS.equals( minorVersionNS )) {
                patchVersionList.add( patchCandidate );
            }
        } else { // original library was a patch version
            List<String> versionChain = versionScheme.getMajorVersionChain( patchCandidate.getNamespace() );

            // If our original library's namespace occurs later in the version chain
            // than our current one, it must be a later minor version
            if (versionChain.indexOf( origLibrary.getNamespace() ) > 0) {
                patchVersionList.add( patchCandidate );
            }
        }
    }
    ...
    /**
     * If the given <code>TLExtensionPointFacet</code> is a version patch, the <code>Versioned</code> entity that is
     * being patched will be returned.
     * 
    ...
    Versioned getPatchedVersion(TLExtensionPointFacet xpFacet) throws VersionSchemeException {
        NamedEntity extendedEntity =
            (xpFacet.getExtension() == null) ? null : xpFacet.getExtension().getExtendsEntity();
        ...
        if (extendedEntity instanceof TLFacet) {
            ...
            if (extendedFacetOwner instanceof Versioned) {
                VersionScheme versionScheme =
                    VersionSchemeFactory.getInstance().getVersionScheme( xpFacet.getVersionScheme() );
    ...
    /**
     * Searches all known remote repositories for later minor versions of the given library. If later minor versions are
     * identified that are not currently available in the model, they are automatically loaded. The newly-loaded
     * libraries are added to the list of existing minor versions, and their project items are returned in the resulting
     * list.
     * 
    ...
    List<ProjectItem> importLaterMinorVersionsFromRepository(TLLibrary library,
        ...
        loadLaterVersions( laterMinorVersions, existingLaterMinorVersions, importedVersions, versionScheme );

        return importedVersions;
    }

    /**
     * Searches all known remote repositories for later patch versions of the given library. If later patch versions are
     * identified that are not currently available in the model, they are automatically loaded. The newly-loaded
     * libraries are added to the list of existing patch versions, and their project items are returned in the resulting
     * list.
     * 
    ...
    List<ProjectItem> importLaterPatchVersionsFromRepository(TLLibrary library,
        ...
        if (activeProject != null) {
            ...
            // versions
            loadLaterVersions( laterPatchVersions, existingLaterPatchVersions, importedVersions, versionScheme );
        }
        return importedVersions;
    }

    /**
     * Loads the list of later versions into the active project.
     * 
     * @param laterVersions the list of later versions to be loaded
     * @param existingLaterVersions the list of later versions that has been loaded
     * @param importedVersions the list of imported library versions from the project
     * @param versionScheme the version scheme of the libraries to load
     */
    private void loadLaterVersions(List<RepositoryItem> laterVersions, List<TLLibrary> existingLaterVersions,
        List<ProjectItem> importedVersions, VersionScheme versionScheme) {
        if (!laterVersions.isEmpty()) {
            for (RepositoryItem item : laterVersions) {
                try {
                    ProjectItem pItem = activeProject.getProjectManager().addManagedProjectItem( item, activeProject );

                    if (pItem != null) {
                        importedVersions.add( pItem );
                        existingLaterVersions.add( (TLLibrary) pItem.getContent() );
                    }
                } catch (Exception e) {
                    // No action - just skip this repository item and move on to the next one
                }
            }
            Collections.sort( existingLaterVersions, new LibraryVersionComparator( versionScheme, true ) );
        }
    }

    /**
     * Returns a list of repository items that represent all versions of the given library that exist in a remote
     * repository that have not been loaded into the active project.
     * 
    ...
    private List<RepositoryItem> findAllManagedVersionsNotLoaded(TLLibrary library, VersionScheme versionScheme) {
        ...
        // The repository manager instance represents the local repository
        repositoriesToSearch.addAll( repositoryManager.listRemoteRepositories() );
        repositoriesToSearch.add( repositoryManager );
            ...
            try {
                ...
                for (RepositoryItem item : repository.listItems( baseNamespace, TLLibraryStatus.DRAFT, false )) {
                    ...
                    if (!itemKeys.contains( itemKey ) && item.getBaseNamespace().equals( baseNamespace )) {
                        addIfNotLoaded( item, library, newVersions, versionScheme, repositoryManager );
    ...
    /**
     * Adds the given repository item to the list of new versions if it represents a prior version library that has not
     * yet been loaded to the active project.
     * 
     * @param item the repository item to check and (possibly) add
     * @param library the original library version
     * @param newVersions the list to which the new version item might be added
     * @param versionScheme the version scheme of the library
     * @param repositoryManager the repository manager that owns the given item
     * @throws RepositoryException thrown if an error occurs while accessing a remote repository
     */
    private void addIfNotLoaded(RepositoryItem item, TLLibrary library, List<RepositoryItem> newVersions,
        VersionScheme versionScheme, RepositoryManager repositoryManager) throws RepositoryException {
    ...
    /**
     * Returns a unique key to identify the given repository item. This is necessary just in case multiple copies of the
     * same item exist in different remote repositories (unlikely, but possible).
     * 
            ...
            if (patchedEntity != null) {
                try {
                    ...
                    Versioned newEntityVersion =
                        handler.createOrRetrieveNewVersion( patchedEntity, minorVersionLibrary );
    ...
    /**
     * Constructs a new empty <code>TLLibrary</code> instance using the given one as a basis. The only fields that are
     * not initialized on the new library are it's namespace, prefix, and URL assignments. In addition to those initial
     * field values, all of the contexts in the given library are cloned for the new instance.
     * 
    ...
    /**
     * Rolls up the contents of the given patch version into the new major/minor version. A roll-up is essentially a
     * merge of the attributes, properties, and indicators from the facets of the patch version.
     * 
    ...
    void rollupPatchVersion(Versioned majorOrMinorVersionTarget, TLExtensionPointFacet patchVersion,
        ...
        if (!skipValidation) {
            validatePatchVersionRollup( patchVersion, majorOrMinorVersionTarget );
        }
        ...
        if (targetFacet != null) {
            ...
            mergeUtils.mergeAttributes( (TLAttributeOwner) targetFacet, patchVersion.getAttributes(),
                referenceHandler );
    ...
    /**
     * Verifies that the given extension point facet patch can be rolled up.
     * 
     * @param patchVersion the patch version of the entity whose items will be the source of the roll-up
     * @param majorOrMinorVersionTarget the major/minor version of the entity that will receive any rolled up items
     * @throws VersionSchemeException thrown if the given major version is not a later major version and/or the patch
     ...
     *         in either the source or target entity
     */
    private void validatePatchVersionRollup(TLExtensionPointFacet patchVersion, Versioned majorOrMinorVersionTarget)
    ...
    /**
     * Replicates all folders that exist in the source library for the target library. Only the folders themselves are
     * replicated by this routine, no members are assigned within the structure.
     * 
    ...
    /**
     * Recursive routine to replicate all folders that exist in the source folder owner for the target owner.
     * 
    ...
    /**
     * Assigns the target entity to the same folder path in its owning library as the source entity occupies in its
     * owner. If the source entity is not assigned to a folder or the target folder does not exist, this method will
     * return without action. If the target folder is successfully assigned, this method will return true; false
     * otherwise.
     * 
    ...
    boolean assignTargetFolder(LibraryMember sourceEntity, LibraryMember targetEntity) {
        ...
        if (!sourcePath.isEmpty() && targetPath.isEmpty()) {
            ...
            for (String folderName : sourcePath) {
                ...
                if (targetFolder == null) {
                    break; // give up - the target folder does not exist
                }
            }
    ...
    /**
     * Returns the folder path for the entity within its owning library. If the entity is not assigned to a folder, the
     * folder path list will remain empty after this method call.
     * 
    ...
    /**
     * Adds the given library to the active project. If no active project is assigned, this method will take no action.
     * 
    ...
    /**
     * Returns true if the given library is unmanaged (i.e. not under repository control), <u>and</u> a managaged
     * library with the same base namespace, version, and library name has already been published to a repository.
     * 
            ...
            if ((versionScheme != null) && (item != null) && ((item.getState() == RepositoryItemState.UNMANAGED)
                ...
                try {
                    String managedFilename =
                        versionScheme.getDefaultFileHint( library.getNamespace(), library.getName() );
                    RepositoryItem rItem = projectManager.getRepositoryManager()
                        .getRepositoryItem( library.getBaseNamespace(), managedFilename, library.getVersion() );
    ...
    /**
     * Returns true if the given library is a work-in-process (WIP) copy of a managed repository item.
     * 
     * @param library the library to analyze
     * @return boolean
     */
    ...
    /**
     * Returns true if the two libraries have the same name, are assigned to the same base namespace, and use the same
     * version scheme.
     * 
    ...
    boolean isVersionCandidateMatch(TLLibrary library1, TLLibrary library2) {
        return (library1.getName() != null) && (library1.getNamespace() != null)
            && (library1.getVersionScheme() != null) && library1.getName().equals( library2.getName() )
    ...
    TLLibrary getOwningLibrary(Versioned versionedEntity) {
        ...
        if (versionedEntity == null) {
            ...
            throw new IllegalArgumentException(
                "The versioned entity type is not a library member: " + versionedEntity.getClass().getSimpleName() );
        }
        return library;
    }

    /**
     * Calculates the folder location and filename of the new library. This method assumes that the folder location of
     * the new file will be the same as that of the original library file.
     * 
    ...
    File getDefaultLibraryFileLocation(TLLibrary newVersionLibrary, TLLibrary originalLibraryVersion)
        ...
        String newVersionFilename =
            versionScheme.getDefaultFileHint( newVersionLibrary.getNamespace(), newVersionLibrary.getName() );
        File originalFileLocation = URLUtils.toFile( originalLibraryVersion.getLibraryUrl() );
        File targetFolder;

        if ((activeProject != null)
            && activeProject.getProjectManager().isRepositoryUrl( originalLibraryVersion.getLibraryUrl() )) {
    ...
    /**
     * Implements the <code>Versioned</code> interface for the purpose of comparing the versions of libraries and/or
     * repository items.
     */
