...
public class LibraryCrcCalculator {
    ...
    /**
     * Returns true if a CRC value is required for the given library. CRC's are required for all libraries that are
     * assigned a status later than 'DRAFT' in the library status lifecycle.
     * 
    ...
    public static boolean isCrcRequired(TLLibrary library) {
        ...
        if (library != null) {
            ...
            crcRequired = (status != null) && (status.getRank() > TLLibraryStatus.DRAFT.getRank());
        }
        return crcRequired;
    }

    /**
     * Returns the CRC value for the given JAXB library if one is define. If a CRC is not specified in the library
     * content, this method will return null.
     * 
     ...
     */
    public static void recalculateLibraryCrc(File libraryFile)
        throws LibraryLoaderException, LibrarySaveException, IOException {
        if (!libraryFile.exists()) {
            throw new FileNotFoundException(
                "The specified library file does not exist: " + libraryFile.getAbsolutePath() );
        }
    ...
    /**
     * Visitor implementation that compiles a concatenated string value that represents the contents of the library for
     * use with the CRC calculation.
     * 
     * @author S. Livezey
     */
    private static class CrcVisitor extends ModelElementVisitorAdapter {
        ...
        public boolean visitUserDefinedLibrary(TLLibrary library) {
            ...
            crcData.append( "0" ).append( '|' ); // backward-compatibility for the deleted 'patchLevel' property
