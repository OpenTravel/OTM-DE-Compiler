...
/**
 * Registry that has the ability to identify protected namespaces and determine which users have
 * write access to libraries that are assigned to those namespaces.
 * 
 * @author S. Livezey
 */
public abstract class ProtectedNamespaceRegistry {
    ...
    public static ProtectedNamespaceRegistry getInstance() {
        return (ProtectedNamespaceRegistry) SchemaCompilerApplicationContext.getContext().getBean(
     ...
     *            the namespace URI to check
     * @param credentials
     *            the user credentials to use for security checks
     * @return boolean
     * @throws SchemaCompilerSecurityException
     *             thrown if an error occurs during the access check
     */
    public boolean hasWriteAccess(String namespace, ProtectedNamespaceCredentials credentials)
            throws SchemaCompilerSecurityException {
    ...
    /**
     * Returns the protected namespace group from the registry that matches the given namespace, or
     * null if no namespace matches were discovered.
     * 
     * @param namespace
     *            the namespace URI for which to retrieve the group
    ...
    private boolean isMatchingNamespace(String groupNS, String testNS) {
        // Normalize the namespace components by forcing a trailing '/' at the end of both
        // namespaces
        if (!groupNS.endsWith("/")) {
            groupNS += "/";
        }
