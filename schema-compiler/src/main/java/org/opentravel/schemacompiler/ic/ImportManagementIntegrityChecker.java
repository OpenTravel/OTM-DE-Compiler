 ...
 * @author S. Livezey
 */
public abstract class ImportManagementIntegrityChecker<E extends ModelEvent<S>, S>
    extends AbstractIntegrityChecker<E,S> {

    /**
     * Verifies the imports and includes of the originating library, ensuring that imports or includes are defined for
     * all referenced libraries. Any existing imports/includes that are no longer required are deleted by this method.
     * 
     * @param originatingLibrary the originating library whose imports and includes will be verified
     */
     ...
     * 
     * <p>
     * NOTE: The 'libraryUrlOverride' map that is passed to this method allows the caller to override the current
     * location of zero or more libraries in the current model. This is sometimes necessary when the locations of these
     * libraries are about to change (e.g. just before one or more libraries are published to a repository).
     * 
     * @param originatingLibrary the originating library whose imports and includes will be verified
     * @param libraryUrlOverrides map that allows the caller to override the current URL location of various libraries
     */
    public static void verifyReferencedLibraries(TLLibrary originatingLibrary,
        ...
        for (AbstractLibrary referencedLib : referencedLibraries) {
            if (referencedLib.getNamespace() != null) {
                if (referencedLib.getNamespace().equals( originatingLibrary.getNamespace() )) {
                    ...
                    importList.add( getReferenceUrl( originatingLibrary, referencedLib, libraryUrlOverrides ) );
                }
            }
        }
        ...
        addMissingIncludesAndImports( originatingLibrary, referencedLibraries, libraryUrlOverrides );
    }

    /**
     * Check each library in the list of references provided and add any missing imports or includes that do not
     * currently exist.
     * 
     * @param originatingLibrary the originating library whose imports and includes will be verified
     * @param referencedLibraries the list of libraries referenced by the originating one
     * @param libraryUrlOverrides map that allows the caller to override the current URL location of various libraries
     */
    private static void addMissingIncludesAndImports(TLLibrary originatingLibrary,
        List<AbstractLibrary> referencedLibraries, Map<AbstractLibrary,URL> libraryUrlOverrides) {
        for (AbstractLibrary referencedLib : referencedLibraries) {
            if (referencedLib.getNamespace() != null) {
                if (referencedLib.getNamespace().equals( originatingLibrary.getNamespace() )) {
    ...
    /**
     * Verifies that the owning library defines a namespace import (and file hint) for the specified reference library.
     * If one does not exist, it is added automatically.
     * 
     * @param owningLibrary the owning library whose imports are to be verified
     * @param referencedLibrary the referenced library that requires an import declaration
     * @param libraryUrlOverrides map that allows the caller to override the current URL location of various libraries
     */
    private static void verifyLibraryImport(TLLibrary owningLibrary, AbstractLibrary referencedLibrary,
        Map<AbstractLibrary,URL> libraryUrlOverrides) {
        ...
        if (referencedNamespace != null) {
            ...
            if (importReference == null) { // Add a new namespace for this import
                importReference =
                    new TLNamespaceImport( getUniquePrefix( owningLibrary, referencedLibrary ), referencedNamespace );
            ...
            if (!(referencedLibrary instanceof BuiltInLibrary)) {
                ...
                addFileHint( relativeUrl, importReference );
            }
        }
    }

    /**
     * Adds the file hint provided to the given import reference if such a hint does not already exist.
     * 
     * @param fileHintUrl the relative URL to the library being imported
     * @param importReference the import declaration to which the file hint will be added
     */
    private static void addFileHint(String fileHintUrl, TLNamespaceImport importReference) {
        boolean hasFileHint = false;

        for (String fileHint : importReference.getFileHints()) {
            if (fileHintUrl.equals( fileHint )) {
                hasFileHint = true;
                break;
            }
        }
        if (!hasFileHint) {
            importReference.getFileHints().add( fileHintUrl );
        }
    }

    /**
     * Verifies that the owning library defines an include (and file hint) for the specified reference library. If one
     * does not exist, it is added automatically.
     * 
     * @param owningLibrary the owning library whose includes are to be verified
     * @param referencedLibrary the referenced library that requires an include declaration
     * @param libraryUrlOverrides map that allows the caller to override the current URL location of various libraries
     */
    private static void verifyLibraryInclude(TLLibrary owningLibrary, AbstractLibrary referencedLibrary,
        Map<AbstractLibrary,URL> libraryUrlOverrides) {
        String relativeUrl = getReferenceUrl( owningLibrary, referencedLibrary, libraryUrlOverrides );
    ...
    /**
     * Removes all namespace imports and file hints that do not reference one of the given namespaces and libraries.
     * 
     * @param owningLibrary the owning library whose imports are to be processed
     * @param importedFileHints the map of imported namespaces to the relative URL's of the libararies assigned to those
     *        namespaces
     */
    private static void removeUnneededImports(TLLibrary owningLibrary, Map<String,List<String>> importedFileHints) {
        List<TLNamespaceImport> importList = new ArrayList<>( owningLibrary.getNamespaceImports() );
     ...
     * 
     * @param owningLibrary the owning library whose includes are to be processed
     * @param includedLibs the list of included libraries assigned to the same namespace as the owning library
     * @param libraryUrlOverrides map that allows the caller to override the current URL location of various libraries
     */
    private static void removeUnneededIncludes(TLLibrary owningLibrary, List<AbstractLibrary> includedLibs,
        Map<AbstractLibrary,URL> libraryUrlOverrides) {
        ...
        for (AbstractLibrary includedLib : includedLibs) {
            requiredIncludeUrls.add( getReferenceUrl( owningLibrary, includedLib, libraryUrlOverrides ) );
        }
    ...
    /**
     * Returns a list of all libraries whose entities are referenced by entities in the given user-defined library.
     * 
    ...
    /**
     * Constructs a URL that can be used by the originating library to reference the target library as an import or
     * include location. If the referenced library is repository-managed, the resulting URL will be an OTM repository
     * URI. If the referenced library is unmanaged, the URL will be a relative file path.
     * 
     * @param originatingLibrary the library whose location is the originating point of the relative reference
     * @param referencedLibrary the library that is being imported or included by the originator
     * @param libraryUrlOverrides map that allows the caller to override the current URL location of various libraries
     * @return String
     */
    private static String getReferenceUrl(TLLibrary originatingLibrary, AbstractLibrary referencedLibrary,
        Map<AbstractLibrary,URL> libraryUrlOverrides) {
        ProjectManager projectManager = ProjectManager.getProjectManager( originatingLibrary.getOwningModel() );
        ...
        if (referenceUrl == null) {
            referenceUrl = URLUtils.getRelativeURL( originatingLibraryUrl, referencedLibraryUrl, true );
        }
        return referenceUrl;
    }

    /**
     * Calculates a unique prefix to identify the reference library's namespace within the owning library.
     * 
     * @param owningLibrary the owning library to which the namespace import will be added
     * @param referencedLibrary the referenced library for which to create a prefix
     * @return String
     */
    ...
    private static String getBasePrefix(String prefix) {
        ...
        while ((basePrefix.length() > 0) && Character.isDigit( basePrefix.charAt( basePrefix.length() - 1 ) )) {
    ...
    /**
     * Returns true if the given prefix is not used for any of the namespace imports defined in the given library.
     * 
     * @param prefix the prefix to search for
     * @param library the library whose imports are to be searched
     * @return boolean
     */
    ...
    /**
     * Visitor that collects a list of all libraries whose entities are referenced by members of the referencing
     * library.
     * 
     * @author S. Livezey
     */
    private static class ReferencedLibraryVisitor extends ModelElementVisitorAdapter {
         ...
         * 
         * @param referencingLibrary the referencing library to be analyzed
         */
        ...
        /**
         * Ensures that the given library has been added to the list (as long as it is not the owning library instance).
         * 
         * @param referencedLibrary the referenced library to add
         */
