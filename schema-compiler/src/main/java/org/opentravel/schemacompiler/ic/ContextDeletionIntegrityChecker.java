...
/**
 * Integrity checker component that automatically deletes any associated <code>TLContextReferrer</code> elements from a
 * library's members when a <code>TLContext</code> declaration is deleted from the library. In the case of a contextual
 * facet, the context will simply be set to null instead of deleting the facet.
 * 
 * @author S. Livezey
 */
public class ContextDeletionIntegrityChecker
    extends AbstractIntegrityChecker<OwnershipEvent<TLLibrary,TLContext>,TLLibrary> {
    ...
    /**
     * Returns true if another context with the same ID as the deleted one still exists after the given one has been
     * removed.
     * 
