 ...
 * 
 * <p>Complex type definitions can have implicit associations with <code>XSDElement</code>
 * entities.  Assuming both definitions are in the same namespace, and element that references
 * an <code>XSDComplexType</code> is assumed to be an alias of the type.  If the alias has
 * the same name as the complex type, it is considered to be the identity alias.  A complex
 * type can have multiple aliases, but only one identity alias (if one is defined at all).
 * 
 * @author S. Livezey
 */
