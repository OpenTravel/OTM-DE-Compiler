 ...
 * methods used to manipulate the list of children. This class is intended to be utilized as an
 * internal component of the model and should not be called or referenced beyond that scope.
 * 
 * @param <C>
 *            the type of child entity being managed by this component
 * @param <O>
 *            the type of the parent entity that owns and manages the underlying list
 * @author S. Livezey
 */
abstract class ChildEntityListManager<C, O> {
     ...
     */
    public ChildEntityListManager(O owner, ModelEventType addEventType,
    ...
    /**
     * Adds a <code>DerivedChildEntityListManager</code> whose list of derived child entities should
     * be synchronized with the list of children managed by this component instance.
     * 
     * @param derivedListManager
     *            the manager for the list of derived child entities
     */
     ...
     * 
     * @param derivedListManager
     *            the manager for the list of derived child entities to remove
     */
     ...
     * 
     * @param child
     *            the child entity whose name should be returned
     * @return String
     */
     ...
     * 
     * @param child
     *            the child entity to which the owner will be assigned
     * @param owner
     *            the owning entity to assign (may be null)
     */
     ...
     * 
     * @param owner
     *            the owning entity that will broadcast the event
     * @param event
     ...
     * 
     * @param index
     *            the index at which the given child should be added
     * @param child
     *            the child to add
    ...
    public void addChild(int index, C child) {
        if ((child != null) && !children.contains(child)) {
            ...
            publishEvent(owner, new ModelEventBuilder(addEventType, owner).setAffectedItem(child)
                    .buildEvent());
    ...
    public void removeChild(C child) {
        if (children.contains(child)) {
            ...
            publishEvent(owner, new ModelEventBuilder(removeEventType, owner)
                    .setAffectedItem(child).buildEvent());
    ...
    /**
     * Removes all children from the current list. Events will be emitted for each child that is
     * removed.
     */
    ...
    /**
     * Moves this child up by one position in the list. If the child is not owned by this object or
    ...
    /**
     * Moves this child down by one position in the list. If the child is not owned by this object
