...
public class TLModel implements Validatable {
	...
	public void addLibrary(AbstractLibrary library) {
		if ((library != null) && !libraryList.contains( library )) {
			...
			publishEvent( new ModelEventBuilder( ModelEventType.LIBRARY_ADDED, this ).setAffectedItem( library )
				.buildEvent() );
		}
	}
	...
	public void removeLibrary(AbstractLibrary library) {
		if (libraryList.contains( library )) {
			...
			publishEvent( new ModelEventBuilder( ModelEventType.LIBRARY_REMOVED, this ).setAffectedItem( library )
				.buildEvent() );
		}
	}
	...
	public void moveToLibrary(LibraryMember member, TLLibrary destLibrary) {
		...
		try {
			...
			OwnershipEvent<TLLibrary,LibraryMember> removeEvent = new OwnershipEvent<>( ModelEventType.MEMBER_REMOVED,
					sourceLibrary, member );
			OwnershipEvent<TLLibrary,LibraryMember> addEvent = new OwnershipEvent<>( ModelEventType.MEMBER_ADDED,
					destLibrary, member );
