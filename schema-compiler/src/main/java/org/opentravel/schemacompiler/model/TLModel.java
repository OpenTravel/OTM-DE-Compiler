...
public class TLModel implements Validatable {
     ...
     *            the library namespace to check
     * @return boolean
     */
    ...
    /**
     * Returns true if a library has been registered with the given name that is assigned to the
     * indicated namespace.
     * 
     * @param libraryNamespace
     *            the library namespace to check
    ...
    public void addLibrary(AbstractLibrary library) {
        if ((library != null) && !libraryList.contains(library)) {
            checkDuplicateLibrary(library.getNamespace(), library.getName(),
                    library.getLibraryUrl(), null);
            ...
            publishEvent(new ModelEventBuilder(ModelEventType.LIBRARY_ADDED, this).setAffectedItem(
                    library).buildEvent());
        }
    }
    ...
    public void removeLibrary(AbstractLibrary library) {
        if (libraryList.contains(library)) {
            ...
            publishEvent(new ModelEventBuilder(ModelEventType.LIBRARY_REMOVED, this)
                    .setAffectedItem(library).buildEvent());
        }
    }
     ...
     * Also, any references to the moved entity are updated to reflect the new library to which the
     * member is assigned.
     * 
     * @param member
     *            the library member to move
     * @param destLibrary
     *            the destination library to which the member is to be assigned
     */
    public void moveToLibrary(LibraryMember member, TLLibrary destLibrary) {
        ...
        if (!(member.getOwningLibrary() instanceof TLLibrary)) {
            throw new IllegalArgumentException(
                    "Only members of user-defined libraries can be moved.");
        }
        if (destLibrary == null) {
        ...
        if (!destLibrary.isValidMember(member)) {
            throw new IllegalArgumentException(
                    "The entity is not a valid member of the destination library.");
        }
    ...
    /**
     * Clears all non-built-in libraries from the model. Any registered listeners are unaffected by
     * this operation, but no events are emitted by the removal of libraries from the model by this
     * operation.
     */
     ...
     * 
     * @param namespace
     *            the namespace of the library to return
     * @param libraryName
     ...
     * 
     * @param libraryUrl
     *            the resource URL of the library to return
     * @return AbstractLibrary
     */
    ...
    /**
     * Returns the libraries that are assigned to the specified namespace within this model
     * instance.
     * 
     * @param namespace
     *            the namespace from which to return libraries
     * @return List<AbstractLibrary>
     */
    ...
    /**
     * Assigns the flag value that indicates whenter registered listeners are to be notified of
     * model events.
     * 
     ...
     * of this model. If a conflict is discovered, this method will throw an
     * <code>IllegalArgumentException</code>. If the library is already a member of this model, all
     * of the other members will be checked. This is useful in the event of changes to the name,
     * namespace, or URL of the library.
     * 
     * <p>
     * NOTE: This method should be called before the library is added to the internal 'libraryList'
     * of the model or, in the case of updates to existing libraries, before the fields of the
     * library are actually modified.
     * 
     * @param proposedNamespace
     *            the proposed namespace value for the library to be added or modified
     * @param proposedName
     *            the proposed name value for the library to be added or modified
     * @param proposedUrl
     *            the proposed resource URL for the library to be added or modified
     * @param existingLibrary
     *            the existing library member instance (may be null)
     * @throws IllegalArgumentException
     *             thrown if a name/namespace conflict is discovered
     */
    protected void checkDuplicateLibrary(String proposedNamespace, String proposedName,
            URL proposedUrl, AbstractLibrary existingLibrary) {
        for (AbstractLibrary memberLib : libraryList) {
            ...
            boolean nameMatch = (proposedName == null) ? (memberLib.getName() == null)
                    : proposedName.equals(memberLib.getName());
