...
public class TLExtensionCompileValidator extends TLValidatorBase<TLExtension> {
    ...
    protected ValidationFindings validateFields(TLExtension target) {
        ...
        if (target.getExtendsEntity() != null) {
            ...
            // one as well.
            if (isExtendableEntity(extendsEntity)
                    && !isExtendableEntity((NamedEntity) extensionOwner)) {
    ...
    /**
     * Returns true if the given entity has been marked as 'extendable' (or marked as
     * not-notExtendable, as the case may be).
     * 
