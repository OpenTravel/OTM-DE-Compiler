	...
	protected ValidationFindings validateFields(TLParameter target) {
		...
		if (fieldRef == null) {
			...
			if ((fieldRefName == null) || fieldRefName.equals("")) {
				builder.addFinding(FindingType.ERROR, FIELD_REF, TLValidationBuilder.MISSING_NAMED_ENTITY_REFERENCE);
			} else {
				builder.addFinding(FindingType.ERROR, FIELD_REF, TLValidationBuilder.UNRESOLVED_NAMED_ENTITY_REFERENCE,
						fieldRefName);
			}
			...
			if (facetRef != null) {
				List<TLMemberField<TLMemberFieldOwner>> eligibleFields = ResourceCodegenUtils
					.getEligibleParameterFields(facetRef);
				
				if (!eligibleFields.contains(fieldRef)) {
					builder.addFinding(FindingType.ERROR, FIELD_REF, ERROR_INELIGIBLE_FIELD_REF,
