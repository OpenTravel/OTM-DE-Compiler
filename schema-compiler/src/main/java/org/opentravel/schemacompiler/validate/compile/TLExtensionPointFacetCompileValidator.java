...
public class TLExtensionPointFacetCompileValidator extends TLExtensionPointFacetBaseValidator {
	...
	protected ValidationFindings validateFields(TLExtensionPointFacet target) {
		...
		if (extension == null) {
			builder.addFinding( FindingType.ERROR, EXTENDS_ENTITY, TLValidationBuilder.ERROR_NULL_VALUE );
		}
			...
			if ((extension != null) && (extension.getExtendsEntity() != null)
				...
				if (vScheme != null) {
					if (vScheme.isPatchVersion( owningLibrary.getNamespace() )) {
						validatePatchVersionRules( owningLibrary, extension, builder );
						
					} else { // major or minor version library
						if (helper.getPatchedVersion( target ) != null) {
							// Error if the patched entity is assigned to the same base namespace
							builder.addFinding( FindingType.ERROR, EXTENDS_ENTITY, ERROR_ILLEGAL_PATCH1 );
						}
					}
				}
			}
	...
	/**
	 * @param owningLibrary  the library that owns the extension point facet being validated
	 * @param extension  the extension that provides the object being extended/patched
	 * @param builder  validation builder where errors and warnings will be reported
	 * @throws VersionSchemeException  thrown if the version scheme of the owning library is not valid
	 */
	private void validatePatchVersionRules(TLLibrary owningLibrary, TLExtension extension, TLValidationBuilder builder)
		...
		if (!isLegalPatch) {
			builder.addFinding( FindingType.ERROR, EXTENDS_ENTITY, ERROR_ILLEGAL_PATCH2,
