...
public class TLValueWithAttributesCompileValidator extends TLValueWithAttributesBaseValidator {
    ...
    protected ValidationFindings validateFields(TLValueWithAttributes target) {
        ...
        if ((target.getParentType() == null) && (target.getParentTypeName() != null)) {
            builder.addFinding( FindingType.ERROR, PARENT_TYPE, TLValidationBuilder.UNRESOLVED_NAMED_ENTITY_REFERENCE,
        ...
        checkExtensionAttributeConflicts( inheritedMembers, builder );

        // Check to see if any duplicate attributes (assumed to be inherited from VWA attributes)
        // have the same name but different type assignments.
        ...
        for (TLAttribute attribute : attributesWithDuplicates) {
            ...
            if (existingType == null) { // First time we have seen an attribute with this name
                ...
                builder.addFinding( FindingType.ERROR, ATTRIBUTES, ERROR_INHERITANCE_TYPE_CONFLICT,
                    attribute.getName() );
    ...
    /**
     * Check to see if any of the declared attribute/indicator names conflict with the 'Extension' attributes created
     * for the open enumeration attributes.
     * 
     * @param inheritedMembers the list of all inherited attributes and indicators for the VWA
     * @param builder validation builder where errors and warnings will be reported
     */
    private void checkExtensionAttributeConflicts(List<TLModelElement> inheritedMembers, TLValidationBuilder builder) {
        ...
        for (String openEnumAttrName : openEnumerationAttributes) {
            if (attributeNames.contains( openEnumAttrName + "Extension" )) {
                builder.addFinding( FindingType.ERROR, ATTRIBUTES, ERROR_ILLEGAL_EXTENSION_ATTRIBUTE,
                    openEnumAttrName );
                break;
            }
        }
    ...
    /**
     * Returns true if the given attribute's type is an open enumeration or a VWA whose base type is an open
     * enumeration.
     * 
    ...
    /**
     * Returns true if the parent type of the VWA is a open (or role) enumeration. In the case where the given VWA's
     * parent type is another VWA, this method will recursively determine if the final parent type is an open
     * enumeration.
     * 
    ...
    /**
     * Recursive method that checks whether the parent type of the VWA is an open enumeration, while protecting from
    ...
    /**
     * Returns true if the given list of VWA members contains an attribute with the name "extension". If the parent type
     * is an open/role enumeration, the name will interfere with the implied attribute used for un-declared enumeration
     * values.
     * 
     * <p>
     * NOTE: Indicators with the name "extension" are not considered, since the compiler will automatically append the
     * "...Ind" suffix during schema generation.
     * 
