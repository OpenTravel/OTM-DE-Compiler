...
public abstract class TLValidatorBase<T extends Validatable> implements Validator<T> {
	...
	protected <V extends Versioned> V getExtendedVersion(V versionedEntity) {
		...
		if ((versionedEntity != null) && (candidateVersion != null)
			...
			String candidateName = (candidateVersion instanceof TLOperation)
					? ((TLOperation) candidateVersion).getName()
	...
	protected SchemaNameValidationRegistry getSchemaNameRegistry(TLModel model) {
		SchemaNameValidationRegistry schemaNameRegistry = (SchemaNameValidationRegistry) getContextCacheEntry(
				SchemaNameValidationRegistry.class.getName() );
	...
	private void checkMajorVersionNamingConflicts(NamedEntity entity, ValidationBuilder<?> builder) {
		...
		Collection<Versioned> versionFamily = (entity instanceof Versioned)
				? getMajorVersionFamily( (Versioned) entity )
				: new ArrayList<>();
	...
	private <V extends Versioned> Collection<V> getMajorVersionFamily(V versionedEntity) {
		Map<Versioned,Collection<V>> minorVersionFamilyMappings = (Map<Versioned,Collection<V>>) getContextCacheEntry(
				"majorVersionFamilyMappings" );
	...
	private String getMajorVersionNamespace(TLLibrary library) {
		Map<String,String> majorVersionNamespaceMappings = (Map<String,String>) getContextCacheEntry(
				"majorVersionNamespaceMappings", HashMap.class );
