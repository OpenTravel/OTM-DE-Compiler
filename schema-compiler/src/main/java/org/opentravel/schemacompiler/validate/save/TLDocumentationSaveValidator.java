...
public class TLDocumentationSaveValidator extends TLValidatorBase<TLDocumentation> {
    ...
    protected ValidationFindings validateFields(TLDocumentation target) {
        ...
        builder.setProperty("DESCRIPTION", target.getDescription())
                .setFindingType(FindingType.WARNING).assertMaximumLength(MAX_DESCRIPTION_LENGTH);

        builder.setProperty("deprecations", target.getDeprecations())
                .setFindingType(FindingType.WARNING).assertContainsNoNullElements();

        builder.setProperty("references", target.getReferences())
                .setFindingType(FindingType.WARNING).assertContainsNoNullElements();
        ...
        for (TLAdditionalDocumentationItem otherDoc : target.getOtherDocs()) {
            if (otherDoc != null) {
                builder.setProperty("otherDocs.context", otherDoc.getContext())
