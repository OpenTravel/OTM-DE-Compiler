...
/**
 * Checks a facet or VWA for any elements (or indicators rendered as elements) with names that will cause UPA violations
 * in the resulting schemas. Note that only violations related to substitution groups will be reported by this checker
 * since simple (non-substitution group) UPA violations will be caught by the <code>DuplicateNameChecker</code>.
 * 
 * @author S. Livezey
 */
public class UPAViolationChecker {
    ...
    private UPAViolationChecker(TLModelElement fieldOwner) {
        ...
        for (int checkIdx = 1; checkIdx < elementSequence.size(); checkIdx++) {
            ...
            boolean hasSubstitutionGroup =
                (checkElement instanceof TLProperty) && isSubstitutionGroupElement( (TLProperty) checkElement );
            ...
            hasSubstitutionGroup =
                hasPrecedingElementConflict( elementSequence, precedingNames, checkIdx, hasSubstitutionGroup );
    ...
    /**
     * Returns true if the name of the given indicator or element causes a UPA violation.
     * 
     * @param indicator the indicator to check
     * @return boolean
     */
    ...
    private Set<QName> getPossibleElementNames(TLIndicator indicator) {
        ...
        if ((indicator != null) && (indicator.getOwningLibrary() != null)) {
            elementNames.add(
                new QName( indicator.getOwningLibrary().getNamespace(), fieldNameResolver.getIdentity( indicator ) ) );
        }
        return elementNames;
    }
    ...
    private Set<QName> getPossibleElementNames(TLProperty element) {
        ...
        if ((element != null) && (element.getOwningLibrary() != null)) {
            if (isSubstitutionGroupElement( element )) {
                ...
                if (fieldName != null) {
                    elementNames.add( new QName( element.getOwningLibrary().getNamespace(), fieldName ) );
                }
            }
        }
        return elementNames;
    ...
    /**
     * If the given facet contains (or inherits) any content its name will be added to the list provided. If an owner
     * alias is specified, the element name for the corresponding facet alias will be added.
     * 
