...
public class TLAttributeCompileValidator extends TLAttributeBaseValidator {
	...
	protected ValidationFindings validateFields(TLAttribute target) {
		...
		builder.setEntityReferenceProperty("type", target.getType(), target.getTypeName()).setFindingType(FindingType.ERROR)
			.assertNotNull()
			.assertValidEntityReference(TLClosedEnumeration.class, TLOpenEnumeration.class, TLValueWithAttributes.class,
					TLSimple.class, TLCoreObject.class, TLSimpleFacet.class, TLListFacet.class, XSDSimpleType.class)
		...
		if (dupBuilder.isEmpty() && !(target.getAttributeOwner() instanceof TLValueWithAttributes)) { // Does not apply to VWA's
			List<TLModelElement> inheritedMembersOfOwner = getInheritedMembersOfOwner(target);
			
			if (inheritedMembersOfOwner != null) {
				dupBuilder.setProperty("name-upa", inheritedMembersOfOwner).setFindingType(FindingType.ERROR)
					.assertNoDuplicates(new FacetMemberIdentityResolver());
			}
		}
		...
		if (target.getType() instanceof TLCoreObject) {
			...
			if (!coreObject.getSimpleFacet().declaresContent()) {
				builder.addFinding(FindingType.ERROR, "type", ERROR_NON_SIMPLE_CORE_AS_ATTRIBUTE, coreObject.getLocalName());
			}
		}
	...
	/**
	 * Returns the list of attributes, properties, and indicators defined by the given attribute's owner.
	 * 
	...
	/**
	 * Returns the list of inherited attributes, properties, and indicators defined by the given attribute's owner.
	 * If the given attribute is not owned by a facet or VWA, this method will return null (indicating that the
	 * concept of inheritance does not apply to the owner).
	 * 
	 * @param target  the target attribute being validated
	 * @return List<TLModelElement>
	 */
	...
	private List<TLModelElement> getInheritedMembersOfOwner(TLAttribute target) {
		TLAttributeOwner attrOwner = target.getAttributeOwner();
		String cacheKey = attrOwner.getNamespace() + ":" + attrOwner.getLocalName() + ":inheritedMembers";
		List<TLModelElement> members = (List<TLModelElement>) getContextCacheEntry(cacheKey);
