...
public class TLAttributeCompileValidator extends TLAttributeBaseValidator {
    ...
    protected ValidationFindings validateFields(TLAttribute target) {
        ...
        // List facets can only be referenced if the core object defines one or
        // more roles (applies to standard attributes and references)
    ...
    private void validateReferenceType(TLAttribute target, TLValidationBuilder builder) {
        ...
        builder.setEntityReferenceProperty("refType", target.getType(), target.getTypeName())
        		.setFindingType(FindingType.ERROR)
        		.assertValidEntityReference(TLValueWithAttributes.class, TLFacet.class,
        				TLListFacet.class, TLCoreObject.class, TLChoiceObject.class,
        				TLBusinessObject.class, TLAlias.class, XSDComplexType.class,
        				XSDElement.class);
        
        if ((attributeType != null) && target.isReference()
        		&& !TLPropertyCompileValidator.hasID(attributeType)) {
            builder.addFinding(FindingType.ERROR, "refType", ERROR_ILLEGAL_REFERENCE,
                    target.getTypeName());
        }
