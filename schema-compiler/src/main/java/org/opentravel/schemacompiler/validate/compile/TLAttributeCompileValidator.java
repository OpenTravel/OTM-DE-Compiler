...
public class TLAttributeCompileValidator extends TLAttributeBaseValidator {
    ...
    /**
     * List facets can only be referenced if the core object defines one or more roles (applies to both standard
     * attributes and references).
     * 
    ...
    private void validateStandardType(TLAttribute target, TLValidationBuilder builder) {
        ...
        builder.setEntityReferenceProperty( "type", attributeType, target.getTypeName() ).assertValidEntityReference(
            TLClosedEnumeration.class, TLOpenEnumeration.class, TLRoleEnumeration.class, TLValueWithAttributes.class,
    ...
    /**
     * If the given attribute's type assignment is an xsd:ID, make sure it is contained in the top-level facet if the
