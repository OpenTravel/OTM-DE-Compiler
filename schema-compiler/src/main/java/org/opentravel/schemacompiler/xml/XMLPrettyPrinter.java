...
/**
 * Encapsulates the logic required to format an XML output stream using the DOM load-and-save (LS)
 * utilities. In addition to the line-spacing logic that is implemented by this class, all XML
 * output is also converted to UTF-8 encoding when content is written to the file system.
 * 
 * @author S. Livezey
 */
public class XMLPrettyPrinter {
    ...
    /**
     * Constructor that provides a <code>PrettyPrintLineBreakProcessor</code> to be utilized during
     * XML formatting.
     * 
     * @param lineBreakProcessor
     *            the line break processor instance
     */
    ...
    /**
     * Produces formatted XML output using the given document content, sending it to the specified
     * output stream.
     * 
     * @param document
     *            the DOM document that defines the XML document's content
     * @param out
     *            the output stream that will receive the formatted content
     */
    ...
    /**
     * Writer that intercepts the XML output produced by the pretty-printer class, replacing
     * line-break tokens with actual line breaks in the underlying output stream.
     * 
     * @author S. Livezey
     */
    ...
    /**
     * Initializes the document builder instance used to construct documents and DOM serializer
     * components.
     */
    static {
        try {
