...
public interface LibrarySaveHandler<T> {
    ...
    /**
     * Performs a preliminary validation of the library before actually attempting to store content using the
     * 'saveLibraryContent()' method. Unlike the components of the compiler's <code>validate</code> package, this method
     * only ensures that the required prerequisites for marshalling file content have been fulfilled (e.g. conformance
     * with the library XML schema).
     * 
     ...
     * @throws IllegalArgumentException thrown if the handler does not support the protocol or location specified by the
     *         URL provided
     */
    ...
    /**
     * Assigns the flag value that determines whether this save handler will create a backup file during processing of
     * the 'saveLibraryContent()' method.
     * 
