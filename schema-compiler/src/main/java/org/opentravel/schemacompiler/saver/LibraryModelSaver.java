...
public final class LibraryModelSaver {
    ...
    /**
     * Assigns the <code>LibrarySaveHandler</code> instance that will be used to persist the JAXB
     * library instances.
     * 
     * @param saveHandler
     ...
     * 
     * @param model
     *            the model containing the libraries to save
    ...
    public ValidationFindings saveAllLibraries(TLModel model) throws LibrarySaveException {
        ...
        SymbolResolver symbolResolver = new TL2JaxbLibrarySymbolResolver(
                SymbolTableFactory.newSymbolTableFromModel(model));
    ...
    /**
     * Saves all of the libraries in the given list to their original URL location. Any user-defined
     * libraries that are read-only will be skipped by this method without error.
     * 
     * @param libraryList
     *            the list of libraries to save
     * @throws LibrarySaveException
     *             thrown if a problem occurs during the save operation
     */
    public ValidationFindings saveLibraries(List<TLLibrary> libraryList)
            throws LibrarySaveException {
        ValidationFindings findings = new ValidationFindings();
    ...
    /**
     * Saves the given library to its original URL location. If the given library is read-only, an
     * exception will be thrown by this method.
     * 
     * @param library
     *            the library instance to save
     * @throws LibrarySaveException
     *             thrown if a problem occurs during the save operation
     */
     ...
     *            the transformation context to utilize during save processing
     * @throws LibrarySaveException
     *             thrown if a problem occurs during the save operation
     */
	...
	private <T> ValidationFindings saveLibrary(TLLibrary library,
        ...
        if (library == null) {
            ...
            throw new LibrarySaveException("Unable to save to the requested URL: "
     ...
     * 
     * @param library
     *            the library for which to calculate a CRC
     * @return Long
     */
