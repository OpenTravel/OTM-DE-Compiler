...
public class ServiceAssemblyManager {
	 ...
	 * 
	 * <p>NOTE:  The assembly returned from this method does have its 'assemblyUrl'
	 * field configured using the 'assemblyFile' parameter, but the assembly itself
	 * is not saved to the file system by this method.
	 * 
	 ...
	 */
	public ServiceAssembly loadAssembly(File assemblyFile, ValidationFindings findings)
			throws LibraryLoaderException {
		return fileUtils.loadAssemblyFile( assemblyFile, findings);
	}
	
	/**
	 * Saves the given assembly.  The file location is specified by its 'assemblyUrl' field;
	 * non-file URL's will result in an error.
	 * 
	...
	/**
	 * Validates the contents of the given assembly.  The scope of this validation does
	 * not include the contents of the libraries in the various releases.  Instead, it
	 * primarily looks for inconsistencies in commit dates when different BETA releases
	 * reference the same library file.
	 * 
	...
	public ValidationFindings validateAssembly(ServiceAssembly assembly) {
		ValidatorFactory factory = ValidatorFactory.getInstance(
				ValidatorFactory.ASSEMBLY_RULE_SET_ID,
	...
	/**
	 * Loads the provider model for the given assembly which only includes the provider
	 * API releases in the resulting model.  If the given assembly contains errors, this
	 * method will return null and report the errors in the validation findings provided.
	 * 
	...
	/**
	 * Loads the consumer model for the given assembly which only includes the consumer
	 * API releases in the resulting model.  If the given assembly contains errors, this
	 * method will return null and report the errors in the validation findings provided.
	 * 
	...
	/**
	 * Loads the implementation model for the given assembly which only includes all API
	 * releases (provider and consumer) in the resulting model.  If the given assembly
	 * contains errors, this method will return null and report the errors in the validation
	 * findings provided.
	 * 
	 * @param assembly  the service assembly for which to load the consumer model
	 * @param findings  the validation findings that will be used to collect any errors or
	...
	/**
	 * Loads a model that consists of the assembly items provided.  This method assumes the
	...
	private TLModel loadModel(List<ServiceAssemblyItem> assemblyItems, ValidationFindings findings)
		...
		for (ServiceAssemblyItem saItem : assemblyItems) {
			try {
				...
				log.warn("Unable to load release from remote repository: " +
			...
			for (ReleaseMember member : release.getAllMembers()) {
				LibraryInputSource<InputStream> inputSource =
						repositoryManager.getHistoricalContentSource(
								member.getRepositoryItem(), member.getEffectiveDate() );
	...
	/**
	 * Returns all of the user-defined libraries from the model that are associated with
	 * the given release.
	 * 
