...
public class ReleaseFileUtils implements AbstractFileUtils {
	...
	public void saveReleaseFile(Release release, boolean createBackup) throws LibrarySaveException {
		File releaseFile = URLUtils.isFileURL( release.getReleaseUrl() ) ? URLUtils.toFile( release.getReleaseUrl() )
				: null;
	...
	public ReleaseMemberType unmarshalReleaseMemberContent(String memberContent) throws IOException {
		try {
			Unmarshaller unmarshaller = jaxbContext.createUnmarshaller();
			JAXBElement<ReleaseMemberType> memberElement = (JAXBElement<ReleaseMemberType>) unmarshaller
				.unmarshal( new StringReader( memberContent ) );
	...
	private ReleaseType transformToJaxbRelease(Release release) {
		...
		Map<String,String> optionProps = (compilerOptions == null) ? new ReleaseCompileOptions().toProperties()
				: compilerOptions.toProperties();
	...
	static {
		try {
			...
			InputStream schemaStream = SchemaDeclarations.OTA2_RELEASE_SCHEMA
