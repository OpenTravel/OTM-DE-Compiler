...
/**
 * Performs the auto-creation of 'nsid.txt' files in the repository folders during the
 * initialization process of a <code>RepositoryManager</code>.
 * 
 * NOTE: This class is intended as a temporary startup utility since some existing repositories may
 * not yet have created the required 'nsid.txt' files. Once all existing repositories (local and
 * remote) have been migrated, this class can be safely removed.
 * 
 * @author S. Livezey
 */
public class NamespaceIdFileGenerator {
     ...
     *            the repository manager that owns the folder structure to process
     */
    ...
    /**
     * Constructs a list of all base namespaces that are declared in the meta-data files of
     * <code>RepositoryItem</code> records that are stored in the repository.
     * 
     * @param nsFolder
     *            the namespace folder to search for base namespace declarations
     * @param results
     *            the resulting list of base namespaces being compiled
     */
    private void findBaseNamespaces(File nsFolder, List<String> results) throws RepositoryException {
        if ((nsFolder == null) || !nsFolder.exists())
            return;
        ...
        for (File folderMember : nsFolder.listFiles()) {
            ...
            if (folderMember.isDirectory()) {
                ...
                try {
                    LibraryInfoType libraryMetadata = (LibraryInfoType) repositoryManager
                            .getFileManager().loadLibraryMetadata(folderMember);
