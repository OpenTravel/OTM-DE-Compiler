 ...
 * sources as required by the OTM release specification. All other module loader functions are delegated to an
 * underlying module loader instance.
 * 
 * <p>
 * During the loading process the list of referenced libraries is also refreshed. Any references that are no longer
 * required are removed from the release, and any new references are applied using the release's default effective date.
 */
public class ReleaseLibraryModuleLoader implements LibraryModuleLoader<InputStream> {
     ...
     */
    public ReleaseLibraryModuleLoader(ReleaseManager releaseManager, LibraryModuleLoader<InputStream> delegateLoader) {
        this.releaseManager = releaseManager;
        this.delegate = delegateLoader;
    }

    /**
     * Updates the list of referenced items and their associated effective dates for the given release. The new list of
     * referenced items is obtained from the non-principal dependencies that were loaded by this module loader.
     */
    ...
    /**
     * Builds an input source for the library at the specified URL. If effective dates for the library are specified by
     * the release manager, a historical URL will be assigned in the input source that is returned.
     * 
    ...
    private LibraryInputSource<InputStream> buildInputSource(URL libraryUrl) {
        ...
        try {
            ...
            log.warn( "Unexpected exception while loading historical content: " + libraryFile.getName(), e );
        }
        return inputSource;
    }
