...
public class RepositoryAvailabilityChecker {
    ...
    /**
     * Constructor that provides the repository manager that will be used to establish remote connections.
     * 
    ...
    /**
     * Returns a new <code>RepositoryAvailabilityChecker</code> instance for the given repository manager.
     * 
    ...
    /**
     * Checks availability of the remote repository for the given item and return false if that repository is
     * unavailalbe.
     * 
    ...
    /**
     * Pings the repository with the specified ID. If a successful ping has already been done within the recheck
     * interval window, this method will return true without actually pinging the remote service.
     * 
     * <p>
     * If the repository is considered to be available, this method will return true.
     * 
    ...
    /**
     * Checks availability of all repositories upon which the given release has dependencies and returns false if any of
     * those repositories are unavailalbe.
     * 
    ...
    /**
     * Checks availability of all repositories upon which the given model has dependencies and returns false if any of
     * those repositories are unavailalbe.
     * 
    ...
    /**
     * If the library is repository-managed, adds the ID of the controlling repository to the list of ID's provided.
     * 
    ...
    /**
     * Performs a ping of all known remote repositories. If the 'failOnSingleFailure' flag is true, this method will
     * return false if even a single remote repository is unavailable. If false, all repositories must be unavailable
     * for this method to return false.
     * 
    ...
    private boolean pingAllRepositories(Collection<String> repositoryIds, boolean failOnSingleFailure) {
        ExecutorService executor =
            Executors.newFixedThreadPool( Math.max( repositoryManager.listRemoteRepositories().size(), 1 ) );
        try {
            ...
            for (final String repositoryId : repositoryIds) {
                resultList.add( executor.submit( new Callable<Boolean>() {
