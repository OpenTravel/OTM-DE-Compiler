...
public class ReleaseManager {
	...
	public ReleaseItem publishRelease(Repository repository) throws RepositoryException {
		...
		try {
			...
			if (findings.hasFinding( FindingType.ERROR )) {
				throw new RepositoryException( "Unable to publish the release because it contains validation errors: "
						+ releaseFile.getName() );
			}
	...
	private File getReleaseFile() {
		...
		if (release != null) {
			releaseFile = URLUtils.isFileURL( release.getReleaseUrl() ) ? URLUtils.toFile( release.getReleaseUrl() )
					: null;
		}
		return releaseFile;
	}
