...
public class RepositoryNamespaceUtils {
	 ...
	 * 
	 * @param url  the namespace URI to normalize
	 * @return URL
	 */
	 ...
	 * of the associated repository, this method will return null.  If the namespace is not part of one
	 * of the repository's base namespace, an <code>IllegalArgumentException</code> will be thrown.
	 * 
	 ...
	 * @throws IllegalArgumentException  thrown if the given namespace URI is not part of the base namespace
	 *									 hierarchy from the associated repository
	 */
	public static String getParentNamespace(String ns, Repository repository) throws RepositoryException {
		...
		if (isRootNS) {
			...
			parentNS = parentNS.substring(0, parentNS.lastIndexOf('/')); // not a root NS, so always guranteed to have a seconds path separator
			
		} else {
			throw new IllegalArgumentException("The namespace is not part of the repository's root namespace hierarchy: " + ns);
		}
		return parentNS;
	}
