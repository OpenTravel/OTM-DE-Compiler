...
/**
 * Extends the base class by adding support for XML and JSON schemas, as well as EXAMPLE output generation.
 * 
 * @author S. Livezey
 */
public abstract class AbstractSchemaCompilerTask extends AbstractCompilerTask implements SchemaCompilerTaskOptions {
    ...
    /**
     * Compiles the XML schema files for the given model using the context, filename builder, and code generation filter
     * provided.
     * 
     ...
     * @throws SchemaCompilerException thrown if an error occurs during schema generation
     */
    protected void compileXmlSchemas(Collection<TLLibrary> userDefinedLibraries, Collection<XSDLibrary> legacySchemas,
        CodeGenerationContext context, CodeGenerationFilenameBuilder<?> filenameBuilder, CodeGenerationFilter filter)
        ...
        if (legacySchemaFilter == null) {
            DependencyFilterBuilder filterBuilder =
                new DependencyFilterBuilder().setIncludeExtendedLegacySchemas( true );
        ...
        generateBuiltIns( model, context, CodeGeneratorFactory.XSD_TARGET_FORMAT, filter, filenameBuilder,
            importLocations );
    ...
    private void generateXsdForLibrary(TLLibrary library, TLModel model, CodeGenerationContext context,
        ...
        if ((filter != null) && (xsdGenerator instanceof AbstractJaxbCodeGenerator)) {
            ...
            for (SchemaDeclaration schemaDeclaration : generator.getCompileTimeDependencies()) {
                if (schemaDeclaration.getFilename( CodeGeneratorFactory.XSD_TARGET_FORMAT ).endsWith( ".xsd" )) {
                    continue;
                }
                AbstractLibrary dependentLib =
                    model.getLibrary( schemaDeclaration.getNamespace(), schemaDeclaration.getName() );
    ...
    private void generateLegacySchemas(Collection<XSDLibrary> legacySchemas, CodeGenerationFilter legacySchemaFilter,
        ...
        CodeGenerator<XSDLibrary> legacyExtensionGenerator =
            newCodeGenerator( CodeGeneratorFactory.EXT_XSD_TARGET_FORMAT, XSDLibrary.class,
    ...
    private void generateBuiltIns(TLModel model, CodeGenerationContext context, String targetFormat,
        ...
        ImportSchemaLocations importLocations) throws CodeGenerationException, ValidationException {
        if (model != null) {
            CodeGenerator<BuiltInLibrary> xsdGenerator = newCodeGenerator( targetFormat, BuiltInLibrary.class,
    ...
    /**
     * Performs a dependency analysis of all the libraries that are to be included in the generated schema output. For
     * each included library, the name and location of the generated schemas are pre-calculated for the purpose of
     * creating consistent import declarations across all of the generated schemas.
     * 
     ...
     */
    @SuppressWarnings("unchecked")
    private ImportSchemaLocations analyzeImportDependencies(TLModel model, CodeGenerationContext context,
        CodeGenerationFilenameBuilder<?> filenameBuilder, CodeGenerationFilter filter) {
        if (filenameBuilder == null) {
            filenameBuilder = new LibraryFilenameBuilder<>();
        }
        CodeGenerationFilenameBuilder<TLLibrary> tlFilenameBuilder =
            (CodeGenerationFilenameBuilder<TLLibrary>) filenameBuilder;
        CodeGenerationFilenameBuilder<XSDLibrary> xsdFilenameBuilder =
            (CodeGenerationFilenameBuilder<XSDLibrary>) filenameBuilder;
        CodeGenerationFilenameBuilder<XSDLibrary> extFilenameBuilder =
            new LegacySchemaExtensionFilenameBuilder<>( xsdFilenameBuilder );
    ...
    /**
     * Searches the implied schema dependencies of the given built-in and adds any required libraries to the list.
     * 
     * @param builtIn the built-in library to analyze
     * @param builtInDependencies the set of schema dependencies being populated
     */
     ...
     * @throws SchemaCompilerException thrown if an error occurs during schema generation
     */
    protected void compileJsonSchemas(Collection<TLLibrary> userDefinedLibraries, Collection<XSDLibrary> legacySchemas,
        CodeGenerationContext context, CodeGenerationFilenameBuilder<?> filenameBuilder, CodeGenerationFilter filter)
        throws SchemaCompilerException {
        ...
        // Generate output for all built-in libraries
        generateBuiltIns( model, context, CodeGeneratorFactory.JSON_SCHEMA_TARGET_FORMAT, filter, filenameBuilder,
            null );
    }
    ...
    private void generateJsonForLibrary(TLLibrary library, TLModel model, CodeGenerationContext context,
        CodeGenerationFilter filter, CodeGenerationFilenameBuilder<?> filenameBuilder)
        throws CodeGenerationException, ValidationException {
        CodeGenerator<TLLibrary> jsonSchemaGenerator = newCodeGenerator( CodeGeneratorFactory.JSON_SCHEMA_TARGET_FORMAT,
            TLLibrary.class, (CodeGenerationFilenameBuilder<TLLibrary>) filenameBuilder, filter );
