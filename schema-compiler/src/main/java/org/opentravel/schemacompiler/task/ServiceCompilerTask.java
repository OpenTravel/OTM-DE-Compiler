...
/**
 * Compiler task used to generate WSDL documents for the services defined in a model, as well as the
 * trimmed schema (XSD) files that contain the entities upon which those services depend.
 * 
 * @author S. Livezey
 */
public class ServiceCompilerTask extends AbstractSchemaCompilerTask implements ServiceCompilerTaskOptions {
	...
	protected void generateOutput(Collection<TLLibrary> userDefinedLibraries, Collection<XSDLibrary> legacySchemas) throws SchemaCompilerException {
		...
		for (TLLibrary library : userDefinedLibraries) {
			...
			if ((service != null) && isLatestServiceVersion(service)) {
				...
				serviceContext.setValue(CodeGenerationContext.CK_OUTPUT_FOLDER, getServiceOutputFolder(service, modelContext));
				...
				if (isGenerateExamples()) {
					generateExampleArtifacts(userDefinedLibraries, serviceContext, new LibraryTrimmedFilenameBuilder(service),
	 ...
	 */
	protected String getServiceOutputFolder(TLService service, CodeGenerationContext context) throws SchemaCompilerException {
	...
	/**
	 * @see org.opentravel.schemacompiler.task.AbstractSchemaCompilerTask#getExampleOutputFolder(org.opentravel.schemacompiler.model.LibraryMember, org.opentravel.schemacompiler.codegen.CodeGenerationContext)
	 */
	@Override
	...
	/**
	 * @see org.opentravel.schemacompiler.task.AbstractSchemaCompilerTask#getSchemaRelativeFolderPath(org.opentravel.schemacompiler.model.LibraryMember, org.opentravel.schemacompiler.codegen.CodeGenerationContext)
	 */
	@Override
