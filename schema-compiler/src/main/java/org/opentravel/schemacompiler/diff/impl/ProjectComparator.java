...
public class ProjectComparator extends BaseComparator {
    ...
    public ProjectChangeSet compareProjects(Project oldProject, Project newProject) {
        ...
        if (valueChanged( oldProject.getName(), newProject.getName() )) {
            changeItems.add( new ProjectChangeItem( changeSet, ProjectChangeType.NAME_CHANGED, oldProject.getName(),
                newProject.getName() ) );
        }
        ...
        for (QName newName : newLibraryNames) {
            changeItems.add(
                new ProjectChangeItem( changeSet, ProjectChangeType.LIBRARY_ADDED, newLibraries.get( newName ) ) );
        }

        // Any old names left over represent libraries that were removed
        for (QName oldName : oldLibraryNames) {
            changeItems.add(
                new ProjectChangeItem( changeSet, ProjectChangeType.LIBRARY_DELETED, oldLibraries.get( oldName ) ) );
        }
    ...
    private void findModifiedSameVersionLibraries(Map<QName,TLLibrary> oldLibraries, Map<QName,TLLibrary> newLibraries,
        ...
        while (iterator.hasNext()) {
            ...
            if (newLibraries.containsKey( libraryName )) {
                ...
                pendingChangeSets.add( new ChangeSetItem( ProjectChangeType.LIBRARY_CHANGED, oldLibrary, newLibrary ) );
        ...
        while (iterator.hasNext()) {
            ...
            if (!newVersionMatches.isEmpty()) {
                ...
                if (closestVersionMatch != null) {
                    ...
                    pendingChangeSets
                        .add( new ChangeSetItem( ProjectChangeType.LIBRARY_VERSION_CHANGED, oldLibrary, newLibrary ) );
    ...
    /**
     * Constructs a map of QNames for each user-defined library in the project using the namespace and library name as
     * the key.
     * 
