...
public class LibraryComparator extends BaseComparator {
    ...
    public LibraryChangeSet compareLibraries(TLLibrary oldLibrary, TLLibrary newLibrary) {
        ...
        if (valueChanged( oldLibrary.getName(), newLibrary.getName() )) {
            changeItems.add( new LibraryChangeItem( changeSet, LibraryChangeType.NAME_CHANGED, oldLibrary.getName(),
                newLibrary.getName() ) );
        }
        if (!getCompareOptions().isSuppressLibraryPropertyChanges()) {
            ...
            if (valueChanged( oldStatus, newStatus )) {
                changeItems
                    .add( new LibraryChangeItem( changeSet, LibraryChangeType.STATUS_CHANGED, oldStatus, newStatus ) );
            }
        }
    ...
    private void compareLibraryMembers(Map<String,NamedEntity> oldEntities, Map<String,NamedEntity> newEntities,
        ...
        for (String newName : newEntityNames) {
            if (!oldEntities.containsKey( newName )) {
                changeItems.add(
                    new LibraryChangeItem( changeSet, LibraryChangeType.MEMBER_ADDED, newEntities.get( newName ) ) );
            }
        }
        ...
        for (String oldName : oldEntityNames) {
            if (!newEntities.containsKey( oldName )) {
                changeItems.add(
                    new LibraryChangeItem( changeSet, LibraryChangeType.MEMBER_DELETED, oldEntities.get( oldName ) ) );
            }
        }
        ...
        for (String entityName : oldEntityNames) {
            if (newEntities.containsKey( entityName )) {
                ...
                EntityChangeSet entityChangeSet =
                    new EntityComparator( getCompareOptions(), getNamespaceMappings() ).compareEntities(
    ...
    private void compareResourceMembers(TLLibrary oldLibrary, TLLibrary newLibrary, LibraryChangeSet changeSet,
        ...
        for (String newName : newResourceNames) {
            if (!oldResources.containsKey( newName )) {
                changeItems.add(
                    new LibraryChangeItem( changeSet, LibraryChangeType.RESOURCE_ADDED, newResources.get( newName ) ) );
            }
        }
        ...
        for (String oldName : oldResourceNames) {
            if (!newResources.containsKey( oldName )) {
                changeItems.add( new LibraryChangeItem( changeSet, LibraryChangeType.RESOURCE_DELETED,
                    oldResources.get( oldName ) ) );
            }
        }
        ...
        for (String resourceName : oldResourceNames) {
            if (newResources.containsKey( resourceName )) {
                ...
                ResourceChangeSet resourceChangeSet =
                    new ResourceComparator( getCompareOptions(), getNamespaceMappings() ).compareResources( oldResource,
                        newResource );
    ...
    /**
     * Constructs a map of all named entities in the library that associates the entitys' local names with the entities
     * themselves.
     * 
    ...
    /**
     * Constructs a map of all resources in the library that associates the their local names with the resources
     * themselves.
     * 
