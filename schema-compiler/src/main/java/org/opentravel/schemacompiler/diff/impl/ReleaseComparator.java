...
public class ReleaseComparator extends BaseComparator {
    ...
    public ReleaseChangeSet compareReleases(ReleaseManager oldReleaseManager, ReleaseManager newReleaseManager) {
        ...
        if (valueChanged( oldRelease.getName(), newRelease.getName() )) {
            changeItems.add( new ReleaseChangeItem( changeSet, ReleaseChangeType.NAME_CHANGED, oldRelease.getName(),
                newRelease.getName() ) );
        }
        ...
        if (valueChanged( oldStatus, newStatus )) {
            changeItems
                .add( new ReleaseChangeItem( changeSet, ReleaseChangeType.STATUS_CHANGED, oldStatus, newStatus ) );
        }
        ...
        findModifiedSameVersionLibraries( oldLibraries, newLibraries, oldLibraryNames, newLibraryNames,
            pendingChangeSets );

        findModifiedUpversionedLibraries( oldLibraries, newLibraries, oldLibraryNames, newLibraryNames,
            pendingChangeSets );

        // Any new names left over represent libraries that were added
        for (QName newName : newLibraryNames) {
            changeItems.add(
                new ReleaseChangeItem( changeSet, ReleaseChangeType.LIBRARY_ADDED, newLibraries.get( newName ) ) );
        }

        // Any old names left over represent libraries that were removed
        for (QName oldName : oldLibraryNames) {
            changeItems.add(
                new ReleaseChangeItem( changeSet, ReleaseChangeType.LIBRARY_DELETED, oldLibraries.get( oldName ) ) );
        }

        // Process all of the pending change sets that were identified during library
        // comparisons.
        for (ChangeSetItem item : pendingChangeSets) {
            LibraryChangeSet libraryChangeSet = new LibraryComparator( getCompareOptions(), getNamespaceMappings() )
    ...
    /**
     * Identify libraries that were modified at the same version. As we identify matching libraries, remove the names
     * that were processed so they will not be considered in subsequent checks.
     * 
     ...
     */
    private void findModifiedSameVersionLibraries(Map<QName,TLLibrary> oldLibraries, Map<QName,TLLibrary> newLibraries,
        SortedSet<QName> oldLibraryNames, SortedSet<QName> newLibraryNames, List<ChangeSetItem> pendingChangeSets) {
        ...
        while (iterator.hasNext()) {
            ...
            if (newLibraries.containsKey( libraryName )) {
                ...
                pendingChangeSets.add( new ChangeSetItem( ReleaseChangeType.LIBRARY_CHANGED, oldLibrary, newLibrary ) );
    ...
    /**
     * Identify libraries that were modified at a different version. As we identify matching libraries, remove the names
     * that were processed so they will not be considered in subsequent steps.
     * 
     * @param oldLibraries the libraries from the old-version project
     * @param newLibraries the libraries from the new-version project
     * @param oldLibraryNames the set of qualified names for the old-version libraries
     ...
     * @param pendingChangeSets the list of pending change sets being constructed
     */
    private void findModifiedUpversionedLibraries(Map<QName,TLLibrary> oldLibraries, Map<QName,TLLibrary> newLibraries,
        SortedSet<QName> oldLibraryNames, SortedSet<QName> newLibraryNames, List<ChangeSetItem> pendingChangeSets) {
        Iterator<QName> iterator;
        iterator = oldLibraryNames.iterator();
        ...
        while (iterator.hasNext()) {
            ...
            List<QName> newVersionMatches =
                ModelCompareUtils.findMatchingVersions( libraryName, newLibraryNames, versionScheme );

            if (!newVersionMatches.isEmpty()) {
                QName closestVersionMatch =
                    ModelCompareUtils.findClosestVersion( libraryName, newVersionMatches, versionScheme );
                ...
                if (closestVersionMatch != null) {
                    ...
                    pendingChangeSets
                        .add( new ChangeSetItem( ReleaseChangeType.LIBRARY_VERSION_CHANGED, oldLibrary, newLibrary ) );
    ...
    /**
     * Constructs a map of QNames for each user-defined library in the release using the namespace and library name as
     * the key.
     * 
