...
public abstract class BaseComparator {
    ...
    /**
     * Adds a mapping the forces the namespace of a old version libraries to be evaluated as if it were assigned to the
    ...
    /**
     * Returns true if the old value and new value are different. Under this comparison, empty strings and null values
     * evaluate as being equivalent.
     * 
     * @param oldValue the old value to compare
     * @param newValue the new value to compare
     * @return boolean
    ...
    /**
     * Evaluates whether the given lists of documentation have been modified from the old version to the new. This
     * method presumes that the two lists have already been determined to be of equal size.
     * 
    ...
    private boolean valueChangedSameSize(List<TLDocumentationItem> oldItems, List<TLDocumentationItem> newItems) {
        ...
        for (int i = 0; i < oldItems.size(); i++) {
            ...
            String oldContext = (oldItem instanceof TLAdditionalDocumentationItem)
                ? ((TLAdditionalDocumentationItem) oldItem).getContext()
                : null;
            String newContext = (oldItem instanceof TLAdditionalDocumentationItem)
                ? ((TLAdditionalDocumentationItem) newItem).getContext()
                : null;
     ...
     * @param versionScheme the version scheme used by both the old and new versions
     * @return boolean
     */
    ...
    /**
     * Returns the extended entity of the one provided or null if the given entity does not extend another one.
     * 
