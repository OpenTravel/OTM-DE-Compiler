...
public class TLLibraryTransformer extends BaseTransformer<TLLibrary,Library,SymbolResolverTransformerContext> {
		...
		for (TLNamespaceImport modelImport : source.getNamespaceImports()) {
			...
			if ((modelImport.getFileHints() != null) && (modelImport.getFileHints().size() > 0)) {
				...
				for (String fileHint : modelImport.getFileHints()) {
					if (fileHint != null) {
						if (fileHints.length() > 0) fileHints.append(' ');
	...
	/**
	 * Extracts the list of include paths from the given list of <code>TLInclude</code> entities and returns
	 * them as a simple list of strings.
	 * 
	...
	/**
	 * Initializes the JAXB-to-TLModel class mappings required to obtain an <code>ObjectTransformer</code>
	 * for arbitrary JAXB objects.
	 */
	static {
		try {
