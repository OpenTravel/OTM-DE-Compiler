...
/**
 * Handles the transformation of objects from the <code>TLChoiceObject</code> type to the
 * <code>ChoiceObject</code> type.
 *
 * @author S. Livezey
 */
public class TLChoiceObjectTransformer extends TLComplexTypeTransformer<TLChoiceObject,ChoiceObject> {
	...
	public ChoiceObject transform(TLChoiceObject source) {
        ...
        for (TLContextualFacet choiceFacet : source.getChoiceFacets()) {
        	choiceObject.getChoice().add(facetContextualTransformer.transform(choiceFacet));
        }
