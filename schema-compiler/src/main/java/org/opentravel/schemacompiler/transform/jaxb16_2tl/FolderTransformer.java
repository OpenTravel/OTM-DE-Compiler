...
/**
 * Handles the transformation of objects from the <code>Folder</code> type to the
 * <code>TLFolder</code> type.
 */
public class FolderTransformer extends ComplexTypeTransformer<Folder, TLFolder> {
	...
	public TLFolder transform(Folder source) {
		TLLibrary targetLibrary = (TLLibrary)
				context.getContextCacheEntry( LibraryTransformer.TARGET_LIBRARY_CONTEXT_ID );
	...
	/**
	 * Constructs a new library folder using the JAXB source folder provided.  If the
	 * new folder's name is invalid, a dummy name will be assigned to the resulting folder.
	 * 
