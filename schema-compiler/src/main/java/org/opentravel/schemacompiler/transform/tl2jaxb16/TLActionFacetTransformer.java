...
/**
 * Handles the transformation of objects from the <code>TLActionFacet</code> type to the <code>FacetAction</code> type.
 *
 * @author S. Livezey
 */
public class TLActionFacetTransformer extends TLComplexTypeTransformer<TLActionFacet,FacetAction> {
    ...
    public FacetAction transform(TLActionFacet source) {
        ...
        if (basePayload != null) {
            facet.setBasePayload(
                context.getSymbolResolver().buildEntityName( basePayload.getNamespace(), basePayload.getLocalName() ) );
        }
