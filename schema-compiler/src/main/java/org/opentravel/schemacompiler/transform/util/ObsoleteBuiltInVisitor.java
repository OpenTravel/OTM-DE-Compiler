...
/**
 * Visitor that identifies any attribute or element references to the <code>OTA_SimpleTypes</code> library that have not
 * yet been resolved. If any such references are discovered, the visitor also contains a method to load the legacy
 * library from the OpenTravel repository and assign all of the missing references.
 */
public class ObsoleteBuiltInVisitor extends ModelElementVisitorAdapter {
    ...
    private static List<String> obsoleteTypeNames = Arrays.asList( "String_AlphaNumeric", "String_Long", "String",
        ...
        "String_Text", "Code_Airline_IATA", "Code_Airline_ICAO", "Code_Airport_IATA", "Code_Airport_ICAO",
        "Code_Agency_IATA_Number", "Code_Country", "Code_Country_ISO_3", "FlightNumber_Suffix", "DateTime",
        "DateTime_WithTimeZone", "Code_Language", "Code_Language_ISO_3", "DateTime_Local", "Code_Currency",
        "Code_Railway_Station_IATA", "Enum_DayOfWeek", "Enum_MonthOfYear", "FlightNumber_With_Suffix",
        "Code_Agency_IATA" );
    ...
    /**
     * Inspects the given unresolved reference to determine if it is one of the types from the obsolete built-in
     * library. If such a reference is found, a new <code>UnresolvedObsoleteReference</code> is created for this
     * visitor.
     * 
     ...
     */
    private void checkUnresolvedReference(AbstractLibrary owningLibrary, ModelElement modelElement,
        String referenceName) {
        if ((owningLibrary != null) && (modelElement != null) && (referenceName != null)
    ...
    /**
     * If any references to the unresolved built-in library were discovered, this method attempts to load the managed
     * library containing those types from the OpenTravel repository. Once loaded, all of the unresolved references are
     * assigned.
     */
    ...
    /**
     * Attempts to load the obsolete built-in library from the OpenTravel repository. If the load is successful, the
     * managed library is returned; if unsuccessful this method will return null.
     * 
    ...
    /**
     * Encapsulates all information required to resolve and assign a type reference to an obsolete built-in type that is
     * now managed in the OpenTravel repository.
     */
    private static class UnresolvedObsoleteReference {
        ...
        /**
         * Constructor that specifies the OTM model element to be resolved as well as the local name of the entity from
         * the obsolete built-in that should be assigned.
         * 
