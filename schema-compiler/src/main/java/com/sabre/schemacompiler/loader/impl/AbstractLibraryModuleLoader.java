...
/**
 * Base class for <code>LibraryModuleLoader</code> implementations that obtain their
 * content from an <code>InputStream</code> source.
 * 
 * @author S. Livezey
 */
public abstract class AbstractLibraryModuleLoader implements LibraryModuleLoader<InputStream>, LoaderValidationMessageKeys {
	
	protected static final Logger log = LoggerFactory.getLogger(LibrarySchema1_3_ModuleLoader.class);
	...
	/**
	 * @see com.sabre.schemacompiler.loader.LibraryModuleLoader#newInputSource(java.net.URL)
	 */
	...
	/**
	 * @see com.sabre.schemacompiler.loader.LibraryModuleLoader#isLibraryInputSource(com.sabre.schemacompiler.loader.LibraryInputSource)
	 */
	@Override
	...
	/**
	 * Loads the content of the JAXB content from the specified input source.  If a validation
	 * schema is provided, it will be used to validate the XML content of the file.
	 * 
	 ...
	 * @param jaxbContext  the JAXB context to use for unmarshalling the XML library content
	 * @param validationSchema  the validation schema to apply to the XML library content (may be null)
	 * @return Object
	 * @throws LibraryLoaderException  thrown if a system-level exception occurs
	 * @throws JAXBException  thrown if the XML content is not readable
	 */
	protected Object loadLibrary(LibraryInputSource<InputStream> inputSource, ValidationFindings validationFindings,
			JAXBContext jaxbContext, javax.xml.validation.Schema validationSchema) throws LibraryLoaderException, JAXBException {
		...
		try {
			if (is != null) {
				...
				JAXBElement<?> documentElement = (JAXBElement<?>) unmarshaller.unmarshal(inputSource
						.getLibraryContent());
				...
				validationFindings.addFinding(FindingType.WARNING, new URLValidationSource(libraryUrl),
						...
						(libraryUrl == null) ? "[MISSING URL]" : URLUtils.getShortRepresentation(libraryUrl));
			}
		} finally {
			try {
				if (is != null) is.close();
	...
	/**
	 * @see com.sabre.schemacompiler.loader.LibraryModuleLoader#loadSchema(com.sabre.schemacompiler.loader.LibraryInputSource,
	 *      com.sabre.schemacompiler.validate.ValidationFindings)
	 */
	@Override
	public LibraryModuleInfo<Schema> loadSchema(LibraryInputSource<InputStream> inputSource, ValidationFindings validationFindings)
		...
		try {
			...
			try {
				if (is != null) {
					XMLStreamReader xmlsReader = XMLInputFactory.newInstance().createXMLStreamReader(
							inputSource.getLibraryContent());
					Map<String, String> namespacePrefixMappings = new HashMap<String, String>();
					Unmarshaller unmarshaller = jaxbContext.createUnmarshaller();

					unmarshaller.setSchema(schemaValidationSchema);
					schema = (Schema) unmarshaller.unmarshal(new PrefixMappingXMLStreamReader(xmlsReader,
					...
					validationFindings.addFinding(FindingType.WARNING, new URLValidationSource(schemaUrl),
							WARNING_SCHEMA_NOT_FOUND,
							(schemaUrl == null) ? "[MISSING URL]" : URLUtils.getShortRepresentation(schemaUrl));
				}
			} finally {
				try {
					if (is != null) is.close();
			...
			String urlString = (schemaUrl == null) ? "[MISSING URL]" : URLUtils.getShortRepresentation(schemaUrl);

			validationFindings.addFinding(FindingType.ERROR, new URLValidationSource(schemaUrl),
					ERROR_UNREADABLE_SCHEMA_CONTENT, urlString, ExceptionUtils.getExceptionClass(e).getSimpleName(),
	...
	private LibraryModuleInfo<Schema> buildModuleInfo(Schema schema) {
		...
		return new LibraryModuleInfo<Schema>(schema, null, schema.getTargetNamespace(), null, includes, imports);
	}
	...
	private class PrefixMappingXMLStreamReader extends StreamReaderDelegate {
		...
		public PrefixMappingXMLStreamReader(XMLStreamReader reader, Map<String, String> namespacePrefixMappings) {
			super(reader);
