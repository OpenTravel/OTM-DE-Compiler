...
import com.sabre.schemacompiler.repository.impl.NamespaceIdFileGenerator;
...
import com.sabre.schemacompiler.repository.impl.RepositoryItemImpl;
import com.sabre.schemacompiler.repository.impl.RepositoryItemVersionedWrapper;
import com.sabre.schemacompiler.repository.impl.RepositoryUtils;
...
import com.sabre.schemacompiler.xml.XMLGregorianCalendarConverter;

/**
 * Manager that coordinates the actions, lookups, and relationships between OTA2.0 repositories and the
 * items that are managed within those repositories.
 * 
 * @author S. Livezey
 */
public final class RepositoryManager implements Repository {
	...
	/**
	 * Constructor that specifies the root location of the repository to manage and the file
	 * manager to use when interacting with the local file system.
	 * 
	 * @param repositoryLocation  the file system location of the repository to manage
	...
	/**
	 * Returns the default <code>RepositoryManager</code> instance to manage the repository at the default
	...
	/**
	 * Returns the location of the OTA2.0 repository that is managed by this <code>RepositoryManager</code>
	 * instance.
	 * 
	 * @return File
	 */
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#getManager()
	 */
	@Override
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#getId()
	 */
	@Override
	public String getId() {
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#getDisplayName()
	 */
	@Override
	 ...
	 * @param displayName  the display name for the local repository
	 * @throws RepositoryException  thrown if the local repository's configuration settings cannot be updated
	 */
	 ...
	 * 
	 * @param repositoryId  the ID of the new repository to create
	 * @param displayName  the display name for the new repository
	...
	public void createLocalRepository(String repositoryId, String displayName) throws RepositoryException {
		File repositoryMetadataFile = new File(fileManager.getRepositoryLocation(), RepositoryFileManager.REPOSITORY_METADATA_FILENAME);
	 ...
	 * 
	 * @param id  the ID of the repository to return
	 * @return Repository
	 */
	 ...
	 * repositories.  This information is added to the global OTA2.0 configuration that applies
	 * to all projects and models that can be accessed by the local user.
	 * 
	 * <p>NOTE: The remote repository must be available for this method to function properly.  If
	 * the repository's web service is not accessible, this method will throw an exception.
	 * 
	...
	public RemoteRepository addRemoteRepository(String endpointUrl) throws RepositoryException {
		...
		if (localRepositoryMetadata.getRemoteRepositories() != null) {
			if (localRepositoryMetadata.getID().equals( newRepositoryID )) {
				throw new RepositoryException("The remote repository has the same ID as that of the local repository instance.");
			}
			for (RemoteRepositoryType existingRepository : localRepositoryMetadata.getRemoteRepositories().getRemoteRepository()) {
				if (existingRepository.getID().equals( newRepositoryID )) {
					throw new RepositoryException("A remote repository is already defined with ID: " + newRepositoryID);
				}
				if (existingRepository.getEndpointUrl().equals( endpointUrl )) {
					throw new RepositoryException("A remote repository with the specified endpoint URL already exists: " + newRepositoryID);
				}
			}
		} else {
	 ...
	 * 
	 * @throws RepositoryException  thrown if the local configuration settings cannot be updated
	 */
	...
	/**
	 * Assigns a policy value for the given repository that indicates how often items that are cached in the
	 * local repository should be checked for updates.
	 * 
	 * @param repository  the repository for which the credentials apply
	 * @param refreshPolicy  the refresh policy value to apply
	 * @throws RepositoryException  thrown if the local repository's configuration settings cannot be updated
	 */
	public void setRefreshPolicy(RemoteRepository repository, RefreshPolicy refreshPolicy) throws RepositoryException {
		if (!remoteRepositories.contains(repository)) {
			throw new IllegalArgumentException("The specified repository is not accessible from the local workspace.");
		}
	 ...
	 * 
	 * @param repository  the remote repository for which the credentials apply
	 * @param userId  the user ID credential for the repository
	 * @param password  the plain-text password credential for the repository
	 * @throws RepositoryException  thrown if the local repository's configuration settings cannot be updated
	 */
	public void setCredentials(Repository repository, String userId, String password) throws RepositoryException {
		if (repository == this) {
			throw new IllegalArgumentException("Security credentials are not required for local repository access.");
		}
		if (!remoteRepositories.contains(repository)) {
			throw new IllegalArgumentException("The specified repository is not accessible from the local workspace.");
		}
	...
	/**
	 * Since the <code>RepositoryManager</code> represents the root repository, it will always return
	 * a single namespace URI that is based on the current user's ID.  Only remote repositories can manage
	 * namespaces other than this default URI.
	 * 
	 * @see com.sabre.schemacompiler.repository.Repository#listRootNamespaces()
	 */
	@Override
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#listNamespaceChildren(java.lang.String)
	 */
	@Override
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#listBaseNamespaces()
	 */
	@Override
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#listAllNamespaces()
	 */
	@Override
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#listItems(java.lang.String, boolean, boolean)
	 */
	@Override
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#search(java.lang.String, boolean, boolean)
	 */
	@Override
	public List<RepositoryItem> search(String freeTextQuery, boolean latestVersionsOnly,
		...
		for (RemoteRepository repository : remoteRepositories) {
			try {
				List<RepositoryItem> itemList = repository.search(freeTextQuery, latestVersionsOnly, includeDraftVersions);
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#getVersionHistory(com.sabre.schemacompiler.repository.RepositoryItem)
	 */
	@Override
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#getRepositoryItem(java.lang.String, java.lang.String, java.lang.String)
	 */
	@Override
	public RepositoryItem getRepositoryItem(String baseNamespace, String filename, String versionIdentifier) throws RepositoryException {
		String baseNS = RepositoryNamespaceUtils.normalizeUri( baseNamespace );
		File itemContent = fileManager.getLibraryContentLocation(baseNS, filename, versionIdentifier);
		
		for (RemoteRepositoryClient repository : remoteRepositories) {
			if (itemContent.exists()) break;
			try {
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#getRepositoryItem(java.lang.String)
	 */
	@Override
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#getRepositoryItem(java.lang.String, java.lang.String)
	 */
	@Override
	public RepositoryItem getRepositoryItem(String itemUri, String itemNamespace) throws RepositoryException, URISyntaxException {
		URI uri = RepositoryUtils.toRepositoryItemUri( itemUri );
		Repository repository = getRepository( uri.getAuthority() );
		RepositoryItem item;
		...
		if (repository == this) {
			...
			if (itemNamespace == null) {
				throw new RepositoryException("Unable to identify the repository item because its namespace was not specified.");
			}
			try {
				...
				LibraryInfoType libraryMetadata = fileManager.loadLibraryMetadata(baseNS, uriParts[2], versionIdentifier);
	 ...
	 * 
	 * @see com.sabre.schemacompiler.repository.RemoteRepository#getUserAuthorization(java.lang.String)
	 */
	@Override
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#createRootNamespace(java.lang.String)
	 */
	@Override
		...
		try {
			...
			try {
				...
				if ((nsUrl.getProtocol() == null) || (nsUrl.getAuthority() == null)) {
					throw new MalformedURLException(); // URLs without protocols or authorities are not valid for the repository
				}
			...
			if (nsidFile.exists()) {
				throw new RepositoryException("The root namespace cannot be created because it conflicts with an existing one.");
			}
			
			// Validation Check - Check to see if the new root is a parent or child of an existing root namespace
			...
			for (String existingRootNS : listRootNamespaces()) {
				...
				while ((existingRootNSFolder != null) && !repositoryBaseFolder.equals(existingRootNSFolder.getAbsolutePath())) {
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#deleteRootNamespace(java.lang.String)
	 */
	@Override
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#createNamespace(java.lang.String)
	 */
	@Override
	public void createNamespace(String baseNamespace) throws RepositoryException {
		...
		try {
			fileManager.startChangeSet();
			File nsidFile = new File( fileManager.getNamespaceFolder(baseNS, null), RepositoryFileManager.NAMESPACE_ID_FILENAME );
			
			if (nsidFile.exists()) {
				throw new RepositoryException("The namespace cannot be created because it conflicts with an existing one.");
			}
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#deleteNamespace(java.lang.String)
	 */
	@Override
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#publish(java.io.InputStream, java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String, com.sabre.schemacompiler.model.TLLibraryStatus)
	 */
	@Override
	public RepositoryItem publish(InputStream unmanagedContent, String filename, String libraryName, String namespace,
		...
		try {
			...
			// Close all streams
			try {
				if (unmanagedContent != null) unmanagedContent.close();
			} catch (Throwable t) {}
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#commit(com.sabre.schemacompiler.repository.RepositoryItem)
	 */
	@Override
	 ...
	 * @throws RepositoryException  thrown if the file content cannot be committed or is not yet locked by the current user
	 */
	public void commit(RepositoryItem item, InputStream wipContent) throws RepositoryException {
		String baseNS = RepositoryNamespaceUtils.normalizeUri( item.getBaseNamespace() );
		LibraryInfoType libraryMetadata = fileManager.loadLibraryMetadata(baseNS, item.getFilename(), item.getVersion());
		
		if ( ((libraryMetadata.getState() != RepositoryState.MANAGED_LOCKED) && (item.getState() != RepositoryItemState.MANAGED_WIP))
				|| (item.getLockedByUser() == null) || !item.getLockedByUser().equals(libraryMetadata.getLockedBy()) ) {
			throw new RepositoryException("Unable to commit - only work-in-process items can be committed to the repository.");
		}
		
		boolean success = false;
		try {
			log.info("Committing updated content '" + item.getFilename() + "' to namespace '" + item.getBaseNamespace() + "'");
			fileManager.startChangeSet();
			
			// Overwrite the existing content with the WIP data
			File contentFile = fileManager.getLibraryContentLocation(item.getBaseNamespace(), item.getFilename(), item.getVersion());
			...
			try {
				if (wipContent != null) wipContent.close();
			} catch (Throwable t) {}
	 ...
	 * 
	 * @param item  the repository item to be reverted
	 * @throws RepositoryException  thrown if the file content cannot be reverted
	 */
	public void revert(RepositoryItem item) throws RepositoryException {
		...
		try {
			...
			File repositoryContentFile = fileManager.getLibraryContentLocation(item.getBaseNamespace(), item.getFilename(), item.getVersion());
			File wipContentFile = fileManager.getLibraryWIPContentLocation(item.getBaseNamespace(), item.getFilename());
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#lock(com.sabre.schemacompiler.repository.RepositoryItem)
	 */
	@Override
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#unlock(com.sabre.schemacompiler.repository.RepositoryItem, boolean)
	 */
	@Override
	 ...
	 * 'wipContent' stream is provided, the work-in-process conent of the will be committed to the remote
	 * repository before the existing lock is released.  If the stream parameter is null, any changes in
	 ...
	 * @throws RepositoryException  thrown if the file content cannot be unlocked or is not yet locked by the current user
	 */
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#promote(com.sabre.schemacompiler.repository.RepositoryItem)
	 */
	@Override
		...
		if (item.getRepository() == this) {
			...
			try {
				...
				if (!(item instanceof ProjectItem)) {
					// Only required for non-ProjectItems; ProjectItem derives the status value from its library content
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#demote(com.sabre.schemacompiler.repository.RepositoryItem)
	 */
	@Override
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#recalculateCrc(com.sabre.schemacompiler.repository.RepositoryItem)
	 */
	@Override
	public void recalculateCrc(RepositoryItem item) throws RepositoryException {
		...
		if (libraryMetadata.getStatus() != LibraryStatus.FINAL) {
			throw new RepositoryException("Unable to demote - CRC values can only be recalculated for user-defined libraries in FINAL status.");
		}
		...
		if (item.getRepository() == this) {
			...
			try {
				...
				if (libraryContent != null) {
					...
					fileManager.addToChangeSet( contentFile );
					libraryContent.setStatus( TLLibraryStatus.FINAL ); // just in case the library and its meta-data are out of sync
					modelSaver.getSaveHandler().setCreateBackupFile( false );
					modelSaver.saveLibrary( libraryContent );
				}
				...
				throw new RepositoryException("Unable to recalculate the CRC for repository item: " + item.getFilename(), e);
	...
	/**
	 * @see com.sabre.schemacompiler.repository.Repository#delete(com.sabre.schemacompiler.repository.RepositoryItem)
	 */
	@Override
	public void delete(RepositoryItem item) throws RepositoryException {
		if (item.getRepository() == this) {
			...
			try {
				...
				File metadataFile = fileManager.getLibraryMetadataLocation(baseNS, item.getFilename(), item.getVersion());
				File contentFile = fileManager.getLibraryContentLocation(baseNS, item.getFilename(), item.getVersion());
				...
				if (!metadataFile.delete()) {
					throw new RepositoryException("Unable to delete library meta-data file: " + metadataFile.getAbsolutePath());
				}
				if (!contentFile.delete()) {
					throw new RepositoryException("Unable to delete library content file: " + contentFile.getAbsolutePath());
				}
	 ...
	 * 
	 * @param item  the repository item whose location is to be returned
	 * @return URL
	 * @throws RepositoryException  thrown if the content is not available
	 */
	public URL getContentLocation(RepositoryItem item) throws RepositoryException {
		...
		if (item.getState() == RepositoryItemState.MANAGED_WIP) {
			...
			contentLocation = URLUtils.toURL( fileManager.getLibraryContentLocation( baseNS, item.getFilename(), item.getVersion()) );
		}
		return contentLocation;
	}

	/**
	 * If the given <code>RepositoryItem</code> is owned by a remote repository, the local repository's
	 * copy is updated with the latest available content.  If the item is owned by the local repository,
	 * this method has no effect.  This update is performed regardless of the update policy for the
	 * repository that owns and manages the item.
	 * 
	 ...
	 * 
	 * NOTE: Although the repository metadata will be saved by this method, the active change
	 * set will not be committed.  It is the responsibility of the calling method to commit or
	 * roll back the change set depending on the success of the end-to-end transaction.
	 * 
	 * @throws RepositoryException  thrown if the local repository's configuration settings cannot be updated
	 */
	public void saveLocalRepositoryMetadata() throws RepositoryException {
		...
		// Get the list of root namespaces from the existing file if the list has not yet been initialized.  They
		// are not accessible from local repository fields because a default namespace is always published for the
		// local repository.
		...
		try {
			...
			// If an error occurred, refresh all local settings to make sure we are still in-sync with the
			// local file system
			try {
				if (!success) {
					refreshLocalRepositoryInfo( true );
	...
	/**
	 * Initializes the settings for the local OTA2.0 repository.  If a local repository does
	 * not yet exist, one is created automatically.
	 * 
	 * @throws RepositoryException  thrown if the local repository cannot be accessed
	 */
	...
	/**
	 * Checks the last-updated date of the local repository's meta-data and reloads it if
	 * required.
	 * 
	 * @param forceRefresh  indicates whether the metadata should be refreshed regardless of the file's last-upated timestamp
	 */
	private void refreshLocalRepositoryInfo(boolean forceRefresh) {
		try {
			...
			if (actualLastUpdated == null) {
				...
				for (RemoteRepositoryType newRepository : repositoryInfo.getRemoteRepositories().getRemoteRepository()) {
	 ...
	 * 
	 * @param jaxbRemoteRepository  the JAXB object containing the remote repository configuration settings
	 * @param remoteRepositoryImpl  the schema compiler's remote repository object instance
	 */
	private void copyRemoteRepositorySettings(RemoteRepositoryType jaxbRemoteRepository, RemoteRepositoryClient remoteRepository) {
	 ...
	 * exceptions occur during the load, the file will be assumed to be a non-OTM file.  In such, cases
	...
	private TLLibrary loadOtmLibraryContent(File libraryFile) {
		...
		try {
			if ((libraryFile != null) && libraryFile.exists() && !libraryFile.getName().toLowerCase().endsWith(".xsd")) {
