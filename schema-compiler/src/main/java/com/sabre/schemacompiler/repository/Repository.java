...
public interface Repository {
	...
	/**
	 * Returns a list of each <code>RepositoryItem</code> assigned to the specified base
	 * namespace.  If multiple versions of a <code>RepositoryItem</code> are present, only
	 * the latest version will be returned when the 'latestVersionsOnly' flag is true.  If the
	 * 'includeDraftVersions' flag is false, only <code>FINAL</code> versions will be considered
	 * durng the search.
	 * 
	 ...
	 * @param latestVersionsOnly  flag indicating whether the results should include all matching versions or just the latest version of each library
	 * @param includeDraftVersions  flag indicating whether versions in <code>DRAFT</code> status should be considered by the search
	 * @return List<RepositoryItem>
	 * @throws RepositoryException  thrown if the remote repository cannot be accessed
	 */
	 ...
	 * @param includeDraftVersions  flag indicating whether versions in <code>DRAFT</code> status should be included in the resulting list
	 * @return List<RepositoryItem>
	 * @throws RepositoryException  thrown if the remote repository cannot be accessed
	 */
	...
	/**
	 * Returns the meta-data record for a repository item using the URI provided.  This method requires
	 * that the URI contains the encoded namespace value for the repository item (normally optional per
	 * the OTM repository URI specification).
	 * 
	 ...
	 * specifies a namespace value (optional per the OTM repository URI specification), that namespace will be
	 * ignored in favor of the 'itemNamespace' value provided.
	 * 
	 * <p>The format of the 'itemUri' is as follows:
	 * <pre>otm://&lt;repository-id&gt;[/&lt;item-namespace&gt;]/&lt;otm-filename&gt;[#&lt;version-scheme-id&gt;]</pre>
	 * <ul>
	 *   <li><u>repository-id</u> - the ID of the repository to which the item is published (required)</li>
	 *   <li><u>item-namespace</u> - the repository-managed namespace to which the item is assigned (optional); since this component is a URI, this value must be URL encoded to include it within the repository URI path</li>
	 *   <li><u>otm-filename</u> - the filename assigned to the item in the OTM repository</li>
	 *   <li><u>version-scheme-id</u> - the version scheme assigned to the item; if the default value of "OTA2" is assigned, this component of the URI path is optional</li>
	 * </ul>
	 * 
	 * @param itemUri  the fully-qualified URI of the repository item (encoded namespace value is required)
	...
	/**
	 * Creates the given root namespace within the repository.  If the namespace URI provided
	 * is nested under an existing root namespace, an exception will be thrown.
	 * 
	 * @param rootNamespace  the base namespace to be created
	 * @throws RepositoryException  thrown if the repository cannot be accessed or the namespace is
	 *								is nested under an existing root namespace
	 */
	public void createRootNamespace(String rootNamespace) throws RepositoryException;
	
	/**
	 * Deletes the specified root namespace from the repository.  This method will only
	 * succeed if the root namespace is empty.
	 * 
	 ...
	 * @throws RepositoryException  thrown if the repository cannot be accessed or the namespace is
	 *								not the child of an existing root namespace
	 */
	 ...
	 * @paramnamespace  the full namespace (including version-specific path information) with which the file content is to be associated
	 * @param versionIdentifier  the version identifier of the repository item
	 ...
	 * 'commitWIP' parameter is true, the work-in-process conent of the will be committed to the remote
	 * repository before the existing lock is released.  If the flag is false, any changes in the item's
	 * WIP content will be discarded.
	 * 
	 * @param item  the repository item to lock
	 * @param commitWIP  flag indicating that the current WIP content changes should be committed prior to releasing the lock
	...
	/**
	 * Demotes a <code>RepositoryItem</code> from <code>FINAL</code> status to <code>DRAFT</code>.  This operation
	 * can only be performed if the local user has administrative permissions to modify the requested item.
	 * 
	...
	/**
	 * Recalculates the CRC of a <code>RepositoryItem</code> that is in the <code>FINAL</code> status.
	 * 
	...
	/**
	 * Deletes a <code>RepositoryItem</code> from the repository.  This operation can only be performed if the local
	 * user has administrative permissions to modify the requested item.
	 * 
