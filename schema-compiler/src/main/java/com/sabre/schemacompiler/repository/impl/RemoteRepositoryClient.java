...
import com.sabre.schemacompiler.repository.RepositoryUnavailableException;
...
public class RemoteRepositoryClient implements RemoteRepository {
	...
	private boolean useProxy() {
		SystemDefaultRoutePlanner planner = new SystemDefaultRoutePlanner(null, ProxySelector.getDefault());
		// TODO Auto-generated method stub
		return false;
	}

	private HttpResponse executeWithAuthentication(HttpUriRequest request) throws ClientProtocolException, IOException {
	...
	/**
	 * @see com.sabre.schemacompiler.repository.RemoteRepository#getRefreshPolicy()
	 */
	@Override
	...
	/**
	 * @see com.sabre.schemacompiler.repository.RemoteRepository#getEndpointUrl()
	 */
	...
	/**
	 * @see com.sabre.schemacompiler.repository.RemoteRepository#refreshRepositoryMetadata()
	 */
	@Override
	...
	public List<String> listNamespaceChildren(String baseNamespace) throws RepositoryException {
		try {
			...
			JAXBElement<NamespaceListType> jaxbElement = (JAXBElement<NamespaceListType>)
	...
	public List<RepositoryItem> search(String freeTextQuery, boolean latestVersionsOnly,
			boolean includeDraftVersions) throws RepositoryException {
		try {
			HttpGet request = newGetRequest( SEARCH_ENDPOINT,
					new HttpGetParam("query", freeTextQuery),
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<RepositoryItem> getVersionHistory(RepositoryItem item)
	...
	public RepositoryItem getRepositoryItem(String baseNamespace, String filename, String versionIdentifier) throws RepositoryException {
		...
		LibraryInfoType libraryInfo = manager.getFileManager().loadLibraryMetadata(baseNS, filename, versionIdentifier);
	...
	public RepositoryItem getRepositoryItem(String itemUri, String itemNamespace) throws RepositoryException, URISyntaxException {
		...
		if ((uri.getAuthority() == null) || !uri.getAuthority().equals(id)) {
			throw new RepositoryException("Unable to retrieve the requested item becuase it does not belong to this repository.");
		}
		...
		try {
			...
			LibraryInfoType libraryMetadata = manager.getFileManager().loadLibraryMetadata(baseNS, uriParts[2], versionIdentifier);
	...
	public RepositoryPermission getUserAuthorization(String baseNamespace) throws RepositoryException {
		try {
			...
			Unmarshaller unmarshaller = RepositoryFileManager.getSharedJaxbContext().createUnmarshaller();
			JAXBElement<RepositoryPermissionType> jaxbElement = (JAXBElement<RepositoryPermissionType>)
					unmarshaller.unmarshal( response.getEntity().getContent() );
	...
	public void createRootNamespace(String rootNamespace) throws RepositoryException {
		try {
			...
			HttpGet request = newGetRequest( CREATE_ROOT_NAMESPACE_ENDPOINT, new HttpGetParam("rootNamespace", rootNS) );
	...
	public void deleteRootNamespace(String rootNamespace) throws RepositoryException {
		try {
			String rootNS = RepositoryNamespaceUtils.normalizeUri( rootNamespace );
			HttpGet request = newGetRequest( DELETE_ROOT_NAMESPACE_ENDPOINT, new HttpGetParam("rootNamespace", rootNS) );
	...
	public RepositoryItem publish(InputStream unmanagedContent, String filename, String libraryName, String namespace,
		...
		try {
			...
			mpEntity.addBinaryBody("fileContent", toByteArray(unmanagedContent), ContentType.DEFAULT_BINARY, filename);
	...
	public void lock(RepositoryItem item) throws RepositoryException {
		...
		try {
			...
			Unmarshaller unmarshaller = RepositoryFileManager.getSharedJaxbContext().createUnmarshaller();
			JAXBElement<LibraryInfoType> jaxbElement = (JAXBElement<LibraryInfoType>)
					unmarshaller.unmarshal( response.getEntity().getContent() );
	...
	public void unlock(RepositoryItem item, boolean commitWIP) throws RepositoryException {
		...
		try {
			...
			// Force a re-download of the updated content to make sure the local copy is synchronized
	...
	public void promote(RepositoryItem item) throws RepositoryException {
		try {
			...
			// Update the local cache with the content that was just modified in the remote repository
			downloadContent(item, true);
	...
	/**
	 * @see com.sabre.schemacompiler.repository.RemoteRepository#downloadContent(com.sabre.schemacompiler.repository.RepositoryItem, boolean)
	 */
	@Override
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void downloadContent(String baseNamespace, String filename, String versionIdentifier, boolean forceUpdate) throws RepositoryException {
		String baseNS = RepositoryNamespaceUtils.normalizeUri( baseNamespace );
		...
		try {
			...
			refreshRequired = forceUpdate ||
					(contentMetadata.getStatus() == LibraryStatus.DRAFT) || // always update draft items
					(refreshPolicy == RefreshPolicy.ALWAYS);
			
			if (!refreshRequired && (refreshPolicy == RefreshPolicy.DAILY)) {
				Date lastUpdated = XMLGregorianCalendarConverter.toJavaDate( contentMetadata.getLastUpdated() );
				refreshRequired = !dateOnlyFormat.format( lastUpdated ).equals( dateOnlyFormat.format( new Date() ) );
			}
		} catch (RepositoryException e) {
			refreshRequired = true;
		}
		
		// If the item was previously downloaded, make sure it originated from this remote repository
		...
		// If a refresh is required, download the item's metadata and content from the remote web service
		if (refreshRequired) {
			File repositoryContentFile = manager.getFileManager().getLibraryContentLocation(baseNS, filename, versionIdentifier);
			boolean success = false;
			
			try {
				log.info("Downloading content from repository '" + id + "' - " + baseNS + "; " + filename + "; " + versionIdentifier);
				manager.getFileManager().startChangeSet();
				...
				Unmarshaller unmarshaller = RepositoryFileManager.getSharedJaxbContext().createUnmarshaller();
				JAXBElement<LibraryInfoType> jaxbElement = (JAXBElement<LibraryInfoType>)
						unmarshaller.unmarshal( metadataResponse.getEntity().getContent() );
				...
				// If the remote repository is inaccessible, it is only an error if we are downloading the
				// files for the first time.
				File repositoryMetadataFile = manager.getFileManager().getLibraryMetadataLocation(baseNS, filename, versionIdentifier);
	...
	/**
	 * Configures the 'Authorization' header on the given HTTP request using the current user ID
	 * and encrypted password that is configured for this repository.
	 */
	...
	/**
	 * If the response status indicates an error condition and a message is provided, the text
	...
	/**
	 * Encapsulates a single name/value pair that should be included as a URL parameter
	 * on an HTTP GET request.
	 */
