...
import com.sabre.schemacompiler.validate.ValidationBuilder;
...
public abstract class TLValidatorBase<T extends Validatable> implements Validator<T> {
	...
	public static final String TLMODEL_PREFIX = "com.sabre.schemacompiler.";
	...
	public void setValidationContext(ValidationContext context) {
		if (context instanceof TLModelValidationContext) {
			...
			throw new IllegalArgumentException("The validation context must be an instance of TLModelValidationContext.");
		}
	}
	...
	/**
	 * Called by the {@link #validate(Validatable)} method to perform the validation of the target object's
	 * field values.  The default implementation of this method returns null.
	 * 
	...
	/**
	 * Called by the {@link #validate(Validatable)} method to perform the validation of the target object's
	 * child components (if any).  The default implementation of this method returns null.
	 * 
	 * @param target  the target object to validate
	 * @return ValidationFindings
	 */
	...
	protected void setContextLibrary(TLLibrary library) {
		if (context != null) {
			...
			if (symbolResolver != null) {
				symbolResolver.setPrefixResolver(
						(library == null) ? null : new LibraryPrefixResolver(library) );
				symbolResolver.setAnonymousEntityFilter(
						(library == null) ? null : new ChameleonFilter(library) );
			}
		}
	}
	...
	/**
	 * If the given owner provides example values, this method will issue a warning if the assigned type
	 * is 'ota:Empty'.
	 * 
	 ...
	 */
	protected void checkEmptyValueType(TLExampleOwner exampleOwner, NamedEntity assignedType, String propertyName, ValidationBuilder<?> builder) {
		if ((exampleOwner != null) && (exampleOwner.getExamples().size() > 0)) {
			...
			boolean isEmptyType = (assignedType == null) ||
					(emptyElement.getSchemaDeclaration().getNamespace().equals(assignedType.getNamespace())
							&& emptyElement.getLocalName().equals(assignedType.getLocalName()));
	 ...
	 * 
	 * <p>NOTE: This method is designed to be used for validation purposes.  It is less strict
	 * than the method provided by the <code>MinorVersionHelper</code> because it does not require
	 * the resulting object to be a member of the same major version chain.  It only requires that
	 * both objects reside within the same base namespace.
	 * 
	 ...
	 * 
	 * @param versionedEntity  the versioned entity for which to return version extensions
	 * @return Collection<V>
	 */
	...
	/**
	 * If the given entity is a minor version extension of another entity in the same base namespace,
	 * this method will return true if the extended entity is in a later version than the one passed
	 * to this method.
	 * 
	 * @param versionedEntity  the versioned entity to analyze
	 * @return boolean
	...
	/**
	 * Checks to determine if one or more other entities entities in the model will be assigned the same
	 * name and namespace in the resulting schema(s) that are generated by the compiler.  If a duplicate XSD
	 * type and/or global element name is detected, a validation error will be added to the builder provided. 
	 * 
	...
	/**
	 * Returns the cached <code>SchemaNameValidationRegistry</code> instance.  If a cached object does not exist, one is created
	 * automatically.
	 * 
	...
	/**
	 * Checks to determine if another entity is assigned to the given entity's major-version namespace
	 * that is not a previous or later version of the owning library.  The only duplicate names that are
	 * allowed in a major-version namespace are for minor versions of <code>Versioned</code> objects.
	 * 
	 * @param entity  the named entity to analyze
	 * @param builder  the validation builder that will receive any validation errors that are detected
	 */
		...
		if (matchingEntities.size() > 1) {
			...
			for (NamedEntity matchingEntity : matchingEntities) {
				...
				if ((entity instanceof Versioned) && (matchingEntity instanceof Versioned)
					...
					// If both entities are versioned, we do NOT have a naming conflict if they are members of the same family
				...
				if (conflictingEntity != null) {
					// NOTE: This validation check only reports errors for duplicate names in DIFFERENT namespaces
					// of a major-version chain.  Duplicate symbols that occur in the same namespace are reported
					// by different validation checks.
					if (!conflictingEntity.getNamespace().equals(entity.getNamespace())) {
						builder.addFinding(FindingType.ERROR, "name", ERROR_DUPLICATE_MAJOR_VERSION_SYMBOL, localName);
					}
					break;
				}
			}
	...
	/**
	 * Returns the collection of all later and earlier minor versions of the given entity.  At a minimum,
	 * the resulting collection will contain the original entity.  To improve performance, the minor version
	 * families are cached in the validation context.
	 * 
	...
	private <V extends Versioned> Collection<V> getMinorVersionFamily(V versionedEntity) {
		Map<Versioned,Collection<V>> minorVersionFamilyMappings =
				(Map<Versioned,Collection<V>>) getContextCacheEntry("minorVersionFamilyMappings");
		...
		if (minorVersionFamily == null) {
			...
			if ((versionedEntity.getBaseNamespace() != null) && (versionedEntity.getOwningModel() != null)) {
				for (TLLibrary library : versionedEntity.getOwningModel().getUserDefinedLibraries()) {
					...
					// Find all of the entities from this library of the same type as our original versioned entity
					...
					// Determine whether the given entity is part of the original entity's minor version family
					for (V versionedMember : versionedMembers) {
						if ((versionedMember == versionedEntity) || priorEntityVersions.contains(versionedMember)) {
							// The member is a previous version of our original versioned entity (or the original entity itself)
	...
	private String getMajorVersionNamespace(TLLibrary library) {
		...
		if (majorVersionNamespace == null) {
			try {
				...
				VersionScheme versionScheme = factory.getVersionScheme( factory.getDefaultVersionScheme() );
				
				majorVersionNamespace = versionScheme.getMajorVersionNamespace( libraryNamespace );
				
			} catch (VersionSchemeException e) {
				// Use default naming in case of a URI that does not match the default version scheme
