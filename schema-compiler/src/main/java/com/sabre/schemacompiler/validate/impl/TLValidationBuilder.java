...
public final class TLValidationBuilder extends ValidationBuilder<TLValidationBuilder> {
	...
	/**
	 * @see com.sabre.schemacompiler.validate.ValidationBuilder#getThis()
	 */
	@Override
	...
	/**
	 * @see com.sabre.schemacompiler.validate.ValidationBuilder#setProperty(java.lang.String, java.lang.Object)
	 */
	@Override
	...
	/**
	 * Assigns the property value (presumed to be from the target object) to be validated as a reference
	 * to another named model entity.
	 * 
	 * @param propertyName  the name of the entity reference property
	 * @param propertyValue  the value of the entity reference
	 * @param expectedEntityType  the expected entity 
	 * @return TLValidationBuilder
	 */
	public TLValidationBuilder setEntityReferenceProperty(String propertyName, NamedEntity propertyValue, String plainTextEntityName) {
	...
	/**
	 * Assigns the version scheme to be used during validation checks.  If the given identifier is not
	 * valid, this method will assign a null value for the version scheme implementation.
	 * 
	...
	/**
	 * @see com.sabre.schemacompiler.validate.ValidationBuilder#assertNotNull()
	 */
	@Override
	...
	/**
	 * Asserts that the current entity reference property being evaluated is a sub-class/interface of
	 * at least one of the expected types provided.
	 * 
	 * <p>Null values checked by this method <i>will not</i> produce an error.
	 * 
	...
	public TLValidationBuilder assertValidEntityReference(Class<?>... expectedEntityTypes) {
		...
		if (value != null) {
			String entityName = (value == null) ? null :
...
// NOTE: Temporarily removed until integrity-checker implementation is complete; will re-evaluate
...
//			}
	...
	/**
	 * Adds a validation finding if the string value does not represent a valid (recognizable) version
	 * scheme identifier.
	 * 
	 * @return TLValidationBuilder
	 */
	...
	/**
	 * Adds a validation finding if the string value does not represent a valid (recognizable) version
	 * scheme identifier.  If a valid version scheme has not been specified prior to this call, the validation
		...
		if (isNamedEntityProperty) {
			...
			if (value != null) {
				...
				if (value instanceof TLDocumentationOwner) {
					...
					// Entities defined in built-in libraries that contain the keyword "Deprecated" will always
					// be considered to be deprecated.  YES - this is a kludge, but we have no other (easy) way
					// to deprecate legacy (XSD) built-in schema entities. :)
					if (valueLibrary instanceof BuiltInLibrary) {
						isDeprecated = (valueLibrary.getName() != null) && (valueLibrary.getName().indexOf("Deprecated") >= 0);
					}
				}
				if (isDeprecated) {
		...
		if ((modelElementTypes == null) || (modelElementTypes.length == 0)) {
			...
			try {
				displayName = SchemaCompilerApplicationContext.getContext().getMessage(
						modelElementTypes[0].getSimpleName() + ".displayName", null, Locale.getDefault());
			...
			for (Class<?> elementType : modelElementTypes) {
				...
				try {
					String elementName = SchemaCompilerApplicationContext.getContext().getMessage(
							elementType.getSimpleName() + ".displayName", null, Locale.getDefault());
