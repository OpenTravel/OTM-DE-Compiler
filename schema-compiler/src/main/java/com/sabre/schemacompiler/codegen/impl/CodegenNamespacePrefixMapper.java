...
/**
 * JAXB namespace prefix mapper that obtains its mappings from the namespace import
 * declarations of a <code>TLLibrary</code> module.
 * 
 * @author S. Livezey
 */
public class CodegenNamespacePrefixMapper extends NamespacePrefixMapper {
	 ...
	 * @param useWsdlMappings  flag indicating whether the prefix mappings for WSDL documents
	 * 						   (versus XML schema documents) should be declared
	...
	private void addLibraryNamespaceMappings(AbstractLibrary library, CodeGenerationFilter filter) {
		...
		for (AbstractLibrary lib : library.getOwningModel().getAllLibraries()) {
			...
			// Skip libraries that are in the target or the XML schema namespaces; also skip chameleon
			...
			// Add a prefix mapping for this library's namespace if one is required and does not already exist
	...
	/**
	 * Adds namespace/prefix mappings for compile-time dependencies that have been registered
	...
	private void addWsdlMappings() {
		...
		// If a WSDL bindings component is defined in the application context, allow it to declare any additional
