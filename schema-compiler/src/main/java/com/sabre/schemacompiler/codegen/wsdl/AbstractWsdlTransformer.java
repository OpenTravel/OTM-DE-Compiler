...
import com.sabre.schemacompiler.codegen.impl.AbstractCodegenTransformer;
 ...
 * WSDL code generation subsystem.
 * 
...
public abstract class AbstractWsdlTransformer<S,T> extends AbstractCodegenTransformer<S,T> {
	...
	public AbstractWsdlTransformer() {
		...
		if (appContext.containsBean(SchemaCompilerApplicationContext.CODE_GENERATION_WSDL_BINDINGS)) {
			wsdlBindings = (CodeGenerationWsdlBindings) appContext.getBean(
					SchemaCompilerApplicationContext.CODE_GENERATION_WSDL_BINDINGS);
		}
	}
	
	/**
	 * Returns the <code>CodeGenerationWsdlBindings</code> component that will be used to generate binding
	 * elements for the WSDL document.
	 *
	...
	/**
	 * Creates the types section of the WSDL document with imports for each of the namespaces and schemas
	 * required to define the RAS service messages.
	 * 
	...
	protected TTypes createTypes(LibraryMember source, CodeGenerationFilenameBuilder<LibraryMember> filenameBuilder) {
		...
		schemaAnnotation.getAppinfoOrDocumentation().add( XsdCodegenUtils.getServiceAppInfo(source, context.getCodegenContext()) );
		...
		if (source instanceof TLService) {
			for (TLService dependentService : getDependentServices( (TLService) source )) {
				if (dependentService.getNamespace().equals(targetNamespace)) {
					schema.getIncludeOrImportOrRedefine().add( newInclude(filenameBuilder.buildFilename(dependentService, "xsd")) );
					
				} else {
					schema.getIncludeOrImportOrRedefine().add(
							newImport(dependentService.getNamespace(), filenameBuilder.buildFilename(dependentService, "xsd")) );
				}
	 ...
	 * 
	 * <p>NOTE: This method assumed that the given service is the latest version of the owning library
	...
	/**
	 * Returns a list of dependent services (including the one provided) that must be included or imported
	 * due to inherited operations from previous versions of the service.
	 * 
	...
	/**
	 * Creates the WSDL bindings and service elements using the information provided.  This operation delegates
	 * the actual content creation to the 'codeGenerationWsdlBindings' component defined in the compiler's application
	 * context file.  If a service endpoint URL is not specified in the code generation context, the service
	...
	protected void addBindingAndService(TDefinitions definitions, TPortType portType, List<TMessage> wsdlMessages,
		...
		if ((wsdlBindings != null) && (serviceEndpointUrl != null) && (serviceEndpointUrl.length() > 0)) {
			...
			if (binding != null) {
				TService service = wsdlBindings.createService(binding, definitions.getTargetNamespace(), serviceEndpointUrl);
