...
/**
 * Performs the translation from <code>TLCoreObject</code> objects to the JAXB nodes used
...
public class TLCoreObjectCodegenTransformer extends AbstractXsdTransformer<TLCoreObject,CodegenArtifacts> {
	...
	public CodegenArtifacts transform(TLCoreObject source) {
		...
		otherArtifacts.addAllArtifacts(
				delegateFactory.getDelegate(source.getSimpleListFacet()).generateElements()
		...
		otherArtifacts.addAllArtifacts( delegateFactory.getDelegate(source.getSimpleListFacet()).generateArtifacts() );
	...
	private TopLevelComplexType createRoleEnumerationComplexType(TLCoreObject source) {
		...
		extension.setBase(new QName(source.getNamespace(), source.getRoleEnumeration().getLocalName() + "_Open"));
	 ...
	 * 
	 * @param source  the core object being transformed
	 * @param openEnumeration  indicates whether to generate the open or closed enumeration
	 *						   variant of the simple type
	...
	private TopLevelSimpleType createRoleEnumerationSimpleType(TLCoreObject source, boolean openEnumeration) {
		...
		if (openEnumeration) {
			...
			rolesEnum.setRestriction(restriction);
			restriction.setBase(new QName(source.getNamespace(), source.getRoleEnumeration().getLocalName() + "_Open"));
		}
		...
		for (TLRole role : PropertyCodegenUtils.getInheritedRoles( source )) {
			...
			if ((role.getDocumentation() != null) && !role.getDocumentation().isEmpty()) {
				ObjectTransformer<TLDocumentation,Annotation,CodeGenerationTransformerContext> docTransformer =
						getTransformerFactory().getTransformer(role.getDocumentation(), Annotation.class);
