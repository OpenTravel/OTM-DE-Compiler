...
public class AliasCodegenUtils {
	...
	public static TLAlias getOwnerAlias(TLAlias facetAlias) {
		...
		if (facetAlias.getOwningEntity() instanceof TLFacet) {
			...
			String aliasSuffix = "_" + sourceFacet.getFacetType().getIdentityName(
	...
	/**
	 * Returns the corresponding alias from the sibling facet of the specified type.  A "sibling" facet is
	 * a facet with the same facet owner as the source facet.
	 * 
	 * @param facetAlias  the source facet alias
	...
	public static TLAlias getSiblingAlias(TLAlias facetAlias, TLFacetType siblingFacetType) {
		...
		if (facetAlias.getOwningEntity() instanceof TLFacet) {
			...
			if (facetAlias.getName().endsWith(aliasSuffix)) {
				...
				TLFacet siblingFacet = FacetCodegenUtils.getFacetOfType(sourceFacet.getOwningEntity(), siblingFacetType,
						sourceFacet.getContext(), sourceFacet.getLabel());
				...
				// Next, find the corresponding alias on the sibling facet; if one cannot be located,
				// create a ghost alias (yes, this means we can have ghost aliases for ghost facets)
				String siblingSuffix = "_" + siblingFacet.getFacetType().getIdentityName(
						sourceFacet.getContext(), sourceFacet.getLabel());
				...
				// If a corresponding alias could not be located on the sibling facet, we must assume this
				// this to be an inherited alias; therefore we must return a ghost
	 ...
	 * @return TLAlias
	 */
	public static TLAlias getFacetAlias(TLAlias ownerAlias, TLFacetType facetType, String facetContext, String facetLabel) {
		TLAlias facetAlias = null;
		
		if (ownerAlias.getOwningEntity() instanceof TLFacetOwner) {
			TLFacet facet = FacetCodegenUtils.getFacetOfType((TLFacetOwner) ownerAlias.getOwningEntity(), facetType, facetContext, facetLabel);
			String derivedAlias = ownerAlias.getName() + "_" + facetType.getIdentityName(facetContext, facetLabel);
