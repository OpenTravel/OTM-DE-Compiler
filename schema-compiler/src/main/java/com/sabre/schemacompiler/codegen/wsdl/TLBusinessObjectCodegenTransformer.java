 ...
 * @author S. Livezey
 */
public class TLBusinessObjectCodegenTransformer extends AbstractWsdlTransformer<TLBusinessObject,JAXBElement<?>> {
	...
	public JAXBElement<?> transform(TLBusinessObject source) {
		...
		TMessage createRQMessage = createMessage(source.getDetailFacet(), RASOperationType.CREATE, true);
		TMessage createRSMessage = createMessage(source.getIdFacet(), RASOperationType.CREATE, false);
		TMessage updateRQMessage = createMessage(source.getSummaryFacet(), RASOperationType.UPDATE, true);
		TMessage updateRSMessage = createMessage(source.getSummaryFacet(), RASOperationType.UPDATE, false);
		...
		portType.getOperation().add(
				createPortTypeOperation(source.getIdFacet(), RASOperationType.GET, getRQMessage, getRSMessage, messageList) );
		
		messageList.add( createRQMessage );
		messageList.add( createRSMessage );
		portType.getOperation().add(
				createPortTypeOperation(source.getIdFacet(), RASOperationType.CREATE, createRQMessage, createRSMessage, messageList) );
		
		messageList.add( updateRQMessage );
		messageList.add( updateRSMessage );
		portType.getOperation().add(
				createPortTypeOperation(source.getIdFacet(), RASOperationType.UPDATE, updateRQMessage, updateRSMessage, messageList) );
		
		messageList.add( deleteRQMessage );
		portType.getOperation().add(
				createPortTypeOperation(source.getIdFacet(), RASOperationType.DELETE, deleteRQMessage, null, messageList) );
		...
		for (TLFacet queryFacet : source.getQueryFacets()) {
			...
			portType.getOperation().add(
					createPortTypeOperation(queryFacet, RASOperationType.FIND, findRQMessage, findRSMessage, messageList) );
		}
	 ...
	 * 
	 * @param sourceFacet  the business object facet for which the message is being created
	 ...
	 * @return ComplexType
	 */
	private TMessage createMessage(TLFacet sourceFacet, RASOperationType operationType, boolean isRequest) {
	 ...
	 * the operation definition, they will be added to the 'faultMessages' list that is passed in the
	 * method parameters.
	 * 
	 * @param sourceFacet  the business object facet for which the port-type operation is being created
	 * @param operationType  the type of operation for which the type will be created
	 * @param requestMessage  the request message for the operation
	 * @param responseMessage  the response message for the operation (may be null)
	 ...
	 * @param isRequest  flag indicating whether the fault(s) are being created for a request or a response message
	 * @param targetNamespace  the target namespace of the WSDL document
	 */
	private void addFaults(TOperation op, RASOperationType operationType, List<TMessage> faultMessages,
			boolean isRequest, String targetNamespace) {
		if (wsdlBindings != null) {
			OperationType wsdlOperationType = (operationType == RASOperationType.DELETE) ? OperationType.ONE_WAY : OperationType.REQUEST_RESPONSE;
			TLFacetType operationFacetType = isRequest ? TLFacetType.REQUEST : TLFacetType.RESPONSE;
			List<TMessage> faults = wsdlBindings.getFaultMessages(op, wsdlOperationType, operationFacetType);
