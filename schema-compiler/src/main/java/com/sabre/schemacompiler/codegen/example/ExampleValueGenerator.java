...
import com.sabre.schemacompiler.model.TLAbstractEnumeration;
...
public class ExampleValueGenerator {
	...
	private static enum ExampleSearchMode { PREFERRED_EXAMPLE, ANY_EXAMPLE, LEGACY_VALUE };
	...
	public static ExampleValueGenerator getInstance(String preferredContext) {
		...
		ExampleValueGenerator generator = (ExampleValueGenerator) appContext.getBean(SchemaCompilerApplicationContext.EXAMPLE_GENERATOR);
	...
	/**
	 * Assigns the list of <code>LegacyTypeExampleProviders</code> that will be used to produce examples
	 * for legacy simple types.
	 * 
	 ...
	 * 
	 * <p>NOTE: Values will only be returned for elements that are assigned as simple types.
	 * 
	...
	/**
	 * Searches for an example value for the given parent type using the indicated search mode.  If
	 * a suitable example cannot be identified, this method will return null.
	 * 
	...
	private String getExampleValue(NamedEntity parentType, ExampleSearchMode searchMode) {
		...
		if (parentType instanceof TLSimple) {
			...
			exampleValue = getExampleValue( ((TLRole) parentType).getRoleEnumeration().getOwningEntity().getSimpleFacet(), searchMode);
	...
	/**
	 * Attempts to return an example value for the given <code>TLSimple</code> entity using
	 * the specified search mode.
	 * 
	 * @param simple  the simple type for which to return an example value
	 * @param searchMode  the mode to use when attempting to lookup example values
	 * @return String
	...
	/**
	 * Attempts to return an example value for the given <code>TLSimpleFacet</code> entity using
	 * the specified search mode.
	 * 
	 * @param simpleFacet  the simple facet for which to return an example value
	 * @param searchMode  the mode to use when attempting to lookup example values
	...
	/**
	 * Attempts to return an example value for the given <code>TLListFacet</code> entity using
	 * the specified search mode.
	 * 
	 * <p>NOTE: This method will only return an example value if the underlying item facet of the
	 * list facet is a <code>TLSimpleFacet</code> instance.
	 * 
	...
	private String getExampleValue(TLListFacet listFacet, ExampleSearchMode searchMode) {
		...
		if (listFacet.getItemFacet() instanceof TLSimpleFacet) {
			...
			for (int i = 0; i < 3; i++) {
				...
				if (ex != null) {
					if (exampleList.length() > 0) exampleList.append(" ");
	...
	private String getExampleValue(String simpleTypeName, String namespace, SimpleType xsdSimple, TLModel model) {
		...
		if ((exampleValue == null) && (xsdSimple != null)) {
			if (xsdSimple.getRestriction() != null) {
				...
				if (parentType != null) {
					exampleValue = getExampleValue(parentType.getName(), parentTypeName.getNamespaceURI(), parentType, model);
				}
	...
	private SimpleType findSimpleType(QName simpleTypeName, TLModel model) {
		...
		for (AbstractLibrary library : model.getLibrariesForNamespace(simpleTypeName.getNamespaceURI())) {
			for (LibraryMember member : library.getNamedMembers()) {
				if ((member instanceof XSDSimpleType) && member.getLocalName().equals(simpleTypeName.getLocalPart())) {
	...
	/**
	 * Returns an example value from one of the examples that is explicitly defined for the
	 * given example owner.  If an example is not defined for the preferred context, this method
	 * will return null.
	 * 
	 ...
	 * Returns an example value from one of the examples that is explicitly defined for the
	 * given example owner.  If an example is not defined for the preferred context, this method
	 * will an example from the first available context that is defined.  If no examples have
	 * been defined, this method will return null.
	 * 
	 * @param exampleOwner  the example owner for which to retrieve an example value
	 * @return String
	 */
	 ...
	 * the given entity is null, this method will assume that a match to the empty element and return
	 * true.
	 * 
