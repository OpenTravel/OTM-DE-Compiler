...
public abstract class TLFacetCodegenDelegate extends FacetCodegenDelegate<TLFacet> {
	...
	public FacetCodegenElements generateElements() {
		...
		if (hasContent()) {
			...
			for (TLAlias facetAlias : sourceFacet.getAliases()) {
				...
				if (ownerAlias != null) {
					if (doSubstitutionGroupElement) {
						codegenElements.addSubstitutionGroupElement(ownerAlias, createSubstitutionGroupElement(ownerAlias));
					}
					codegenElements.addFacetElement(ownerAlias, createElement(facetAlias));
					
					if (doNonSubstitutableElement) {
						codegenElements.addFacetElement(ownerAlias, createNonSubstitutableElement(facetAlias));
					}
				}
			}
		}
	...
	public boolean hasContent() {
		return super.hasContent() || !getAttributes().isEmpty() || !getIndicators().isEmpty() || !getElements().isEmpty();
	}

	/**
	 * @see com.sabre.schemacompiler.codegen.xsd.facet.FacetCodegenDelegate#hasExtensionPoint()
	 */
	@Override
	 ...
	 * cases (business/core object extension), the facet returned by this method may have a different
	 * owner than that of the source facet.
	 * 
	 * @return TLFacet
	 */
	protected TLFacet getBaseFacet() {
		...
		// If the source facet does not have a base facet from its local owner, we need to look at the
		// business object/core/operation extension and select the corresponding facet from the extended
		// owner as the base
		if (baseFacet == null) {
			InheritedFacetSearch facetSearch = new InheritedFacetSearch(getSourceFacet()) {
				public boolean isMatchingCandidate(TLFacet candidateFacet) {
					TLFacetCodegenDelegate candidateFacetDelegate = (TLFacetCodegenDelegate)
							new FacetCodegenDelegateFactory(transformerContext).getDelegate(candidateFacet);
	 ...
	 * method simply returns the type name of the value returned by the 'getLocalBaseFacet()' method.
	 * Sub-classes may override if the facet's schema type should extend from a type that is not
	 * a member of the compiler's model.
	 * 
	 * @return QName
	 */
	protected QName getLocalBaseFacetTypeName() {
		...
		return (baseFacet == null) ? null : new QName(baseFacet.getNamespace(), XsdCodegenUtils.getGlobalTypeName(baseFacet));
	}
	 ...
	 * 
	 * <p>NOTE: By default, this method returns false.  Sub-classes should override to implement
	...
	/**
	 * Returns the element that will serve as the head of the substitution group for the source facet's
	 * owning entity.
	 * 
		...
		if (owner instanceof TLDocumentationOwner) {
			...
			if (documentation != null) {
				ObjectTransformer<TLDocumentation,Annotation,CodeGenerationTransformerContext> docTransformer =
						getTransformerFactory().getTransformer(documentation, Annotation.class);
	...
	/**
	 * Returns true if the source facet should have a non-substitutable facet in addition to the substitutable
	 * one that is created by default.
	 * 
	 * <p>NOTE: By default, this method returns false.  Sub-classes should override to implement
	 * facet-specific logic indicating when the reference element should be created.
	 * 
	...
	/**
	 * Returns a non-substitutable element for the source facet.  Although it is termed "non-substitutable",
	 * all global elements are members of the root "...SubGrp" element at a minimum.
	 * 
	...
	protected Element createNonSubstitutableElement(TLAlias facetAlias) {
		TLAlias ownerAlias = (facetAlias == null) ? null : AliasCodegenUtils.getOwnerAlias(facetAlias);
		TLFacet sourceFacet = getSourceFacet();
		...
		if (ownerAlias == null) {
			element.setSubstitutionGroup( new QName(sourceFacet.getNamespace(),
					XsdCodegenUtils.getSubstitutionGroupElementName(sourceFacet.getOwningEntity()).getLocalPart()) );
		} else {
			element.setSubstitutionGroup( new QName(sourceFacet.getNamespace(),
					XsdCodegenUtils.getSubstitutionGroupElementName(ownerAlias).getLocalPart()) );
		}
		return element;
	}
	 ...
	 * 
	 * <p>NOTE: By default, this method returns the results of the 'getElementName()' that is used to construct
	 * names for the substitutable elements.  Sub-classes that return true from the 'hasNonSubstitutableElement()'
	 * method should override this method to supply a unique element name for the non-substitutable element.
	 * 
	...
	/**
	 * @see com.sabre.schemacompiler.codegen.xsd.facet.FacetCodegenDelegate#getElementName(com.sabre.schemacompiler.model.TLAlias)
	 */
	@Override
	...
	/**
	 * If the source facet should support an extension point element, this method will return the qualified
	 * name of the global extension point element to use in the type's definition.  If extensions are not
	 * supported for the facet, this method sould return null.
	 * 
	 * @return QName
	 */
	 ...
	 * 
	 * @see com.sabre.schemacompiler.codegen.xsd.facet.FacetCodegenDelegate#getSubstitutionGroup(com.sabre.schemacompiler.model.TLAlias)
	 */
	@Override
	protected QName getSubstitutionGroup(TLAlias facetAlias) {
		...
		if (baseFacet != null) {
			if (facetAlias != null) {
				subGrp = XsdCodegenUtils.getSubstitutableElementName( AliasCodegenUtils.getSiblingAlias(facetAlias, baseFacet.getFacetType()) );
			} else {
				subGrp = XsdCodegenUtils.getSubstitutableElementName( baseFacet );
			}
		} else {
			if (facetAlias != null) {
				subGrp = XsdCodegenUtils.getSubstitutionGroupElementName( AliasCodegenUtils.getOwnerAlias(facetAlias) );
			} else {
				subGrp = XsdCodegenUtils.getSubstitutionGroupElementName( getSourceFacet().getOwningEntity() );
			}
		}
		return subGrp;
	}
	...
	/**
	 * Returns the list of attributes to be generated for the source facet.  By default, this
	 * method returns the attributes declared by the source facet and inherited from like-typed
	...
	/**
	 * Returns the list of indicators to be generated for the source facet.  By default, this
	 * method returns the indicators declared by the source facet and inherited from like-typed
	 * facets of the extended facet owner(s).  Sub-classes may override to alter (or add to)
	 * the contents of the resulting list.
	 * 
	...
	protected Annotation createJaxbDocumentation(TLDocumentationOwner entity) {
		...
		if (entity.getDocumentation() != null) {
			ObjectTransformer<TLDocumentation,Annotation,CodeGenerationTransformerContext> docTransformer =
				getTransformerFactory().getTransformer(entity.getDocumentation(), Annotation.class);
	...
	/**
	 * Returns the specified facet type from the core object, unless that facet is empty.  In those cases,
	...
	/**
	 * Returns the specified facet type from the business object, unless that facet is empty.  In those cases,
	 * the facet hierarchy will be traversed upwards until a non-empty candidate is found.
	 * 
	 * @param coreObject  the core object for which to return a facet instance
	 * @param facetType  the preferred type of facet to return
	...
	/**
	 * Returns the specified facet type from the facet hierarchy provided, unless that facet is empty.
	 * In those cases, the facet hierarchy will be traversed upwards until a non-empty candidate is found.
	 * 
	 * @param facetHierarchy  the facet hierarchy for the core/business object
	 * @param facetType  the preferred type of facet to return
	 * @return TLPropertyType
	 */
	private TLPropertyType getPreferredFacet(Stack<TLAbstractFacet> facetHierarchy, TLFacetType facetType) {
	...
	protected abstract class InheritedFacetSearch {
		...
		/**
		 * Searches the source facet's inheritance hierarchy, and returns the corresponding facet from
		 * the next-higher owner if the test condition is met.
		 * 
		 * @return TLFacet
		 */
			...
			while ((inheritedFacet == null) && (extendedOwner != null)) {
				...
				if (firstCandidate == null) {
					if (candidateFacet == null) {
						// We should never need to do this for facet types such as ID's that are the root of their
						// substitution group, and are guranteed (by validation rules) to exist.  For facet types
						// such as Request, Response, Query, etc., the root of the substitution group may need to
						// be presumed to exist if it is inherited from an extended facet owner.  In these cases,
						// we will create a "ghost facet" to represent the missing base facet during code generation.
