...
import com.sabre.schemacompiler.codegen.wsdl.CodeGenerationWsdlBindings;
...
public class DOMExampleVisitor extends AbstractExampleVisitor {
	...
	public DOMExampleVisitor(String preferredContext) {
		...
		try {
			...
			if (appContext.containsBean(SchemaCompilerApplicationContext.CODE_GENERATION_WSDL_BINDINGS)) {
				this.wsdlBindings = (CodeGenerationWsdlBindings) appContext.getBean(
	...
	/**
	 * @see com.sabre.schemacompiler.codegen.example.ExampleVisitor#getBoundNamespaces()
	 */
	@Override
	public Collection<String> getBoundNamespaces() {
		...
		/*
		 * Commented out these lines due to false validation errors on substitution group elements that
		 * result when multiple namespace mappings are included in the 'xsi:schemaLocation' attribute
		 * of an example XML document.
		 * 
	...
	/**
	 * @see com.sabre.schemacompiler.codegen.example.AbstractExampleVisitor#endFacet(com.sabre.schemacompiler.model.TLFacet)
	 */
	@Override
	...
	public void startListFacet(TLListFacet listFacet, TLRole role) {
		...
		// If this is a list facet for a normal TLFacet, push the underlying item facet onto the stack
	...
	/**
	 * @see com.sabre.schemacompiler.codegen.example.AbstractExampleVisitor#endListFacet(com.sabre.schemacompiler.model.TLListFacet, com.sabre.schemacompiler.model.TLRole)
	 */
	@Override
	...
	public void endAttribute(TLAttribute attribute) {
		...
		if (attributeType instanceof TLOpenEnumeration) {
			context.getDomElement().setAttribute( context.getModelAttribute().getName() + "Extension",
	...
	/**
	 * @see com.sabre.schemacompiler.codegen.example.AbstractExampleVisitor#startIndicatorAttribute(com.sabre.schemacompiler.model.TLIndicator)
	 */
	@Override
	...
	/**
	 * @see com.sabre.schemacompiler.codegen.example.AbstractExampleVisitor#startIndicatorElement(com.sabre.schemacompiler.model.TLIndicator)
	 */
	@Override
	...
	/**
	 * @see com.sabre.schemacompiler.codegen.example.AbstractExampleVisitor#startRoleEnumeration(com.sabre.schemacompiler.model.TLRoleEnumeration)
	 */
	@Override
	...
	/**
	 * @see com.sabre.schemacompiler.codegen.example.AbstractExampleVisitor#startExtensionPoint(com.sabre.schemacompiler.model.TLFacet)
	 */
	@Override
	public void startExtensionPoint(TLFacet facet) {
		...
		if ((extensionElement != null) && (owningDomElement != null)) {
			...
			context.setDomElement( createXmlElement( extensionElement.getSchemaDeclaration().getNamespace(),
					extensionElement.getLocalName(), extensionElement.getSchemaDeclaration().getDefaultPrefix() ) );
			owningDomElement.appendChild( context.getDomElement() );
		}
	}

	/**
	 * @see com.sabre.schemacompiler.codegen.example.AbstractExampleVisitor#endExtensionPoint(com.sabre.schemacompiler.model.TLFacet)
	 */
	@Override
	public void endExtensionPoint(TLFacet facet) {
		...
		if ((extensionElement != null) && (domElement != null)
				...
				&& domElement.getNamespaceURI().equals(extensionElement.getSchemaDeclaration().getNamespace())) {
			context = contextStack.pop();
		}
	}
	...
	/**
	 * @see com.sabre.schemacompiler.codegen.example.AbstractExampleVisitor#endExtensionPointFacet(com.sabre.schemacompiler.model.TLExtensionPointFacet)
	 */
	@Override
	 ...
	 * assigned as a child of the 'currentElement' for this visitor, and replaces that 'currentElement'
	 * as the DOM element that will receive attributes and child elements within the current context.
	 * 
	 * @param elementType  specifies the type of element to use when the current context is
	 *					   null (i.e. when the new element will be the root of the XML document)
	 */
	private void createComplexElement(NamedEntity elementType) {
		...
		if ((context.getModelElement() != null)
				&& ((elementType instanceof TLAttributeType) || (elementType.equals(context.getModelElement().getType())))) {
			newElement = createPropertyElement(context.getModelElement(), (TLPropertyType) elementType);
		} else {
			newElement = createXmlElement(elementType.getNamespace(), getContextElementName(elementType, false), elementType);
		}
	...
	private void createSimpleElement(NamedEntity elementType) {
		if (context.getModelAttribute() != null) {
			...
			if (contextStack.isEmpty() && (context.getDomElement() == null)) {
				Element rootElement = createXmlElement(elementType.getNamespace(), elementType.getLocalName(), elementType);
				...
				if (context.getDomElement() == null) {
					createComplexElement(elementType); // constructs a new DOM element with no content
				}
				if (context.getModelElement().isReference()) {
					context.getDomElement().setTextContent( generateExampleValue(elementType) );
					
				} else {
					context.getDomElement().setTextContent( generateExampleValue(context.getModelElement()) );
				}
			}
		}
	}
	...
	private Element createPropertyElement(TLProperty property, TLPropertyType propertyType) {
		...
		if (!PropertyCodegenUtils.hasGlobalElement(propertyType)) {
			...
			if (elementName == null) {
				...
				// The element may be inherited, so to obtain the proper namespace we need to use the
				...
				if (facetStack.isEmpty()) {
					...
					if (facetStack.peek() == propertyType) {
						// If the top of the facet stack is our property type, we need to go up one more level
	 ...
	 * 
	 * @param elementType  specifies the type of element to use when the current context is
	 *					   null or the given entity does not have a pre-defined global element
	...
	private String getContextElementName(NamedEntity elementType, boolean isReferenceProperty) {
		...
		if (!XsdCodegenUtils.isSimpleCoreObject(elementType)) {
			if (context.getModelElement() != null) {
				...
				if ((elementType instanceof TLBusinessObject) || (elementType instanceof TLCoreObject)) {
					useSubstitutableElementName = true;
				}
			}
		}
		...
		if (context.getModelAlias() != null) {
			if (!isReferenceProperty && useSubstitutableElementName) {
				...
				elementQName = PropertyCodegenUtils.getDefaultXmlElementName(context.getModelAlias(), isReferenceProperty);
			}
		} else {
	...
	/**
	 * Constructs a DOM element using the information provided.  In addition to the element
	 * itself, any namespace mappings that are required for the element's prefix are also added
	 * to the document's root.
	 * 
	...
	/**
	 * Adds an example role value for the given core object and each of the extended objects
	 * that it inherits role attributes from.
	 * 
		...
		while (coreObject != null) {
			if (coreObject.getRoleEnumeration().getRoles().size() > 0) {
				...
				context.getDomElement().setAttribute(
						attrName, exampleValueGenerator.getExampleRoleValue(coreObject));
			}
	...
	private void addOperationPayloadContent(TLFacet operationFacet) {
		...
		if ((domElement != null) && (wsdlBindings != null)) {
			wsdlBindings.addPayloadExampleContent( domElement, operationFacet );
		}
	}
	
	/**
	 * Adds a comment as a child of the current DOM element indicating that the content
	 * is based on a legacy type that cannot be generated by this visitor component.
	 * 
	...
	/**
	 * @see com.sabre.schemacompiler.codegen.example.AbstractExampleVisitor#generateExampleValue(java.lang.Object)
	 */
	...
	/**
	 * Encapsulates the data-generation context within the current element/property being
	 * visited.
	 *
	 * @author S. Livezey
	 */
	private class ExampleContext {
		 ...
		 * 
		 * @param domElement  the DOM element to assign
		 */
	...
	/**
	 * Handles the deferred assignment of 'IDREF' and 'IDREFS' values as a post-processing step of the
	...
	private class IdReferenceAssignment {
		 ...
		 */
		public IdReferenceAssignment(NamedEntity referencedEntity, int referenceCount, String attributeName) {
		...
		private QName getReferenceElementName() {
			...
			if (referencedEntity == null) {
				...
				// Iterate through the list and select the first name that appears in the ID registry
		...
		private void addEntityNames(NamedEntity entityRef, List<QName> entityNames) {
			...
			// If the entity is a Core or Business Object (or a Core/BO alias), we can include the names of
				...
				if (owner instanceof TLBusinessObject) {
					...
					for (TLFacet customFacet : entity.getCustomFacets()) {
						TLAlias customAlias = AliasCodegenUtils.getFacetAlias(entityAlias, TLFacetType.CUSTOM,
								customFacet.getContext(), customFacet.getLabel());
	...
	static {
		try {
			...
			extensionPointTypeMap.put( TLFacetType.SUMMARY, SchemaDependency.getExtensionPointSummaryElement() );
			extensionPointTypeMap.put( TLFacetType.DETAIL, SchemaDependency.getExtensionPointDetailElement() );
			extensionPointTypeMap.put( TLFacetType.CUSTOM, SchemaDependency.getExtensionPointCustomElement() );
			extensionPointTypeMap.put( TLFacetType.QUERY, SchemaDependency.getExtensionPointQueryElement() );
