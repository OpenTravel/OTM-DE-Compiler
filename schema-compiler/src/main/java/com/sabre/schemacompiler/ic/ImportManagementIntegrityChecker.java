 ...
 * @author S. Livezey
 */
public abstract class ImportManagementIntegrityChecker<E extends ModelEvent<S>,S> extends AbstractIntegrityChecker<E,S> {
	
	/**
	 * Verifies the imports and includes of the originating library, ensuring that imports or includes
	 * are defined for all referenced libraries.  Any existing imports/includes that are no longer required
	 ...
	 * are deleted by this method.
	 * 
	 * <p>NOTE: The 'libraryUrlOverride' map that is passed to this method allows the caller to override
	 * the current location of zero or more libraries in the current model.  This is sometimes necessary
	 * when the locations of these libraries are about to change (e.g. just before one or more libraries
	 * are published to a repository).
	 * 
	 * @param originatingLibrary  the originating library whose imports and includes will be verified
	 * @param libraryUrlOverrides  map that allows the caller to override the current URL location of various libraries
	 */
	public static void verifyReferencedLibraries(TLLibrary originatingLibrary, Map<AbstractLibrary,URL> libraryUrlOverrides) {
	...
	/**
	 * Verifies that the owning library defines a namespace import (and file hint) for the
	 * specified reference library.  If one does not exist, it is added automatically.
	 * 
	...
	private static void verifyLibraryImport(TLLibrary owningLibrary, AbstractLibrary referencedLibrary,
		...
		if (referencedNamespace != null) {
			...
			if (importReference == null) { // Add a new namespace for this import
				importReference = new TLNamespaceImport(getUniquePrefix(owningLibrary, referencedLibrary), referencedNamespace);
	...
	/**
	 * Verifies that the owning library defines an include (and file hint) for the
	 * specified reference library.  If one does not exist, it is added automatically.
	 * 
	...
	/**
	 * Removes all namespace imports and file hints that do not reference one of the given namespaces
	 * and libraries.
	 * 
	 * @param owningLibrary  the owning library whose imports are to be processed
	 * @param importedFileHints  the map of imported namespaces to the relative URL's of the libararies
	 *							 assigned to those namespaces
	 */
	private static void removeUnneededImports(TLLibrary owningLibrary, Map<String,List<String>> importedFileHints) {
		List<TLNamespaceImport> importList = new ArrayList<TLNamespaceImport>( owningLibrary.getNamespaceImports() );
	...
	/**
	 * Constructs a URL that can be used by the originating library to reference the target library as an
	 * import or include location.  If the referenced library is repository-managed, the resulting URL will
	 * be an OTM repository URI.  If the referenced library is unmanaged, the URL will be a relative file
	 * path.
	 * 
	...
	private static class ReferencedLibraryVisitor extends ModelElementVisitorAdapter {
		...
		/**
		 * Ensures that the given library has been added to the list (as long as it is not the owning
		 * library instance).
		 * 
