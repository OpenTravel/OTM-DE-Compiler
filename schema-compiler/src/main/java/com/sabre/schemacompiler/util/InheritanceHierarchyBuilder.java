...
/**
 * Visitor that scans a model in order to construct a map that links each member of an
 * inheritance hierarchy with a list of all hierarchy members.
 * 
 * @author S. Livezey
 */
public class InheritanceHierarchyBuilder {
	...
	/**
	 * Constructs a map that associates each individual member of an inheritance hierarchy with a collection
	 * that contains all members of that hierarchy tree.  The list of hierarchy members are sorted from the
	 * top-most hierarchy base member to the lowest (siblings are sorted in alphabetical order by local name).
	 * 
	...
	public Map<NamedEntity,List<NamedEntity>> buildHierarchyInfo() {
		...
		// First, collect all of the extension relationships from the model into an easily-readable map
		...
		if (isIsloatedHierarchyMember(member)) {
			...
			while (baseFacetOwner != null) {
				TLFacet baseFacet = FacetCodegenUtils.getFacetOfType(baseFacetOwner, memberFacet.getFacetType(),
						memberFacet.getContext(), memberFacet.getLabel());
	 ...
	 * hierarchy.  For example, query facets of business objects should not be considered substitutable
	 * with the business object's ID facet, but it may be considered a substitutable with query facets
	 * of the same name from an extended business object.
	 * 
	 * <p>NOTE: Under the current modeling scheme, isloated hierarchies are guranteed to be wholly
	...
	private class ExtensionVisitor extends ModelElementVisitorAdapter {
		...
		public boolean visitExtension(TLExtension extension) {
			...
			if ((extendedEntity != null) && (baseEntity != null) && baseEntity.getClass().equals(extendedEntity.getClass())) {
	...
	private class HierarchyComparator implements Comparator<NamedEntity> {
		...
		public int compare(NamedEntity member1, NamedEntity member2) {
			...
			if (member1Level.equals(member2Level)) {
				String member1Name = (member1 == null) ? "" : ((NamedEntity) member1).getLocalName();
				String member2Name = (member2 == null) ? "" : ((NamedEntity) member2).getLocalName();
