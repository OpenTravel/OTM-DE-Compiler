...
import com.sabre.schemacompiler.ic.ContextReferrerVisitor;
...
public class ContextUtils {
	...
	/**
	 * When a model element is moved from one library to another, it is necessary to scan its context ID references
	 * and rename them to valid values in the target library.  This mapping is accomplished by examining the application
	 ...
	 * @param targetLibrary
	 */
	public static void translateContextIdReferences(LibraryElement entity, TLLibrary sourceLibrary, TLLibrary targetLibrary) {
	 ...
	 * function may assign the application context of the imported object (typically its target namespace)
	 * as the context ID.  This method will scan all context references in the given library element
	 * and replace all application context values with the context ID values that are resolved from
	 * the owning library.
	 * 
	...
	public static void resolveApplicationContexts(LibraryElement entity) {
		if (entity.getOwningLibrary() instanceof TLLibrary) {
			...
			for (TLContextReferrer cr : visitor.getContextReferrers()) {
				if ((cr.getContext() != null) && owningLibrary.getContext( cr.getContext() ) == null) { // only process invalid context values
