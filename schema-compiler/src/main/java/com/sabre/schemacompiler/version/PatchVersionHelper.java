...
/**
 * Helper methods used to construct new patch versions of <code>TLLibrary</code> instances
...
public final class PatchVersionHelper extends AbstractVersionHelper {
	...
	/**
	 * @see com.sabre.schemacompiler.version.AbstractVersionHelper#getLaterPatchVersions(com.sabre.schemacompiler.model.TLLibrary)
	 */
	@Override
	...
	/**
	 * @see com.sabre.schemacompiler.version.AbstractVersionHelper#getPatchedVersion(com.sabre.schemacompiler.model.TLExtensionPointFacet)
	 */
	@Override
	...
	/**
	 * Returns the list of all extension point facets that reference the given versioned entity
	 * from a library whose version is a patch of the given entity.
	 * 
	...
	public List<TLExtensionPointFacet> getLaterPatchVersions(Versioned versionedEntity) throws VersionSchemeException {
		...
		if (versionedEntity instanceof LibraryElement) {
			...
			if (versionedMember.getOwningLibrary() instanceof TLLibrary) {
				List<TLLibrary> patchVersionLibraries = getLaterPatchVersions((TLLibrary) versionedMember.getOwningLibrary());
				
				if (!patchVersionLibraries.isEmpty()) {
					// Start by identifying the possible facets to which an extension point facet can refer
					...
					// Search the extension points to determine if any of them reference a facet of our original entity
					for (TLLibrary patchVersionLib : patchVersionLibraries) {
						for (TLExtensionPointFacet xpFacet : patchVersionLib.getExtensionPointFacetTypes()) {
							NamedEntity xpExtendedEntity = (xpFacet.getExtension() == null) ? null : xpFacet.getExtension().getExtendsEntity();
	...
	/**
	 * Returns the libraries in the current model that are eligible to accept new patches for
	 * the given versioned entity facet.
	 * 
	 ...
	 */
	public List<TLLibrary> getEligiblePatchVersionTargets(TLFacet versionedEntityFacet) throws VersionSchemeException {
	 ...
	 * @throws VersionSchemeException
	 */
	public TLLibrary getPreferredPatchVersionTarget(TLFacet versionedEntityFacet) throws VersionSchemeException {
	...
	/**
	 * Creates a new patch version of the given library.  The library that is passed to this method must be a
	 * major or minor version intance.  The resulting patch number of the resulting patch library will be one
	 * greater than the latest known patch for the library that is passed to this method.
	 * 
	...
	/**
	 * Creates a new <code>TLExtensionPointFacet</code> that is a patch for the given versioned entity.  The contents
	 * of the patch returned by this method will be empty.
	 * 
	...
	public TLExtensionPointFacet createNewPatch(TLFacet facetToPatch, TLLibrary targetLibraryVersion)
		...
		if (patchLibraries.indexOf(targetLibraryVersion) < 0) {
			throw new VersionSchemeException("The target library cannot contain a patch for the versioned entity provided.");
		}
		if (isReadOnly(targetLibraryVersion)) {
			throw new VersionSchemeException("Unable to create the requested patch because the target library is read-only.");
		}
