 ...
 * @author S. Livezey
 */
public class TestValueWithAttributesTransformers extends Abstract_1_3_TestTransformers {
	
	@Test
	public void testValueWithAttributesTransformer() throws Exception {
		TLValueWithAttributes type = getValueWithAttributesType(PACKAGE_2_NAMESPACE, "library_1_p2", "SampleValueWithAttributes");
		...
		assertEquals("SampleValueWithAttributes-equivalent", type.getEquivalents().get(0).getDescription());
		...
		assertEquals(LibraryTransformer.DEFAULT_CONTEXT_ID, type.getAttributes().get(2).getExamples().get(0).getContext());
		...
		assertEquals("attr4-equivalent", type.getAttributes().get(3).getEquivalents().get(0).getDescription());
		...
		assertEquals(XMLConstants.W3C_XML_SCHEMA_NS_URI, type.getAttributes().get(3).getType().getNamespace());
	...
	public void testTLValueWithAttributesTransformer() throws Exception {
		ValueWithAttributes type = transformValueWithAttributesType(PACKAGE_2_NAMESPACE, "library_1_p2", "SampleValueWithAttributes");
		...
		assertEquals(LibraryTransformer.DEFAULT_CONTEXT_ID, type.getAttribute().get(2).getExample().get(0).getContext());
		...
		assertEquals("attr4-equivalent", type.getAttribute().get(3).getEquivalent().get(0).getValue());
	...
	private TLValueWithAttributes getValueWithAttributesType(String namespace, String libraryName, String typeName) throws Exception {
		TLLibrary library = getLibrary(namespace, libraryName);
		
		return (library == null) ? null : library.getValueWithAttributesType(typeName);
	}
	
	private ValueWithAttributes transformValueWithAttributesType(String namespace, String libraryName, String typeName) throws Exception {
		TLValueWithAttributes origType = getValueWithAttributesType(namespace, libraryName, typeName);
		...
		ObjectTransformer<TLValueWithAttributes,ValueWithAttributes,SymbolResolverTransformerContext> transformer =
				factory.getTransformer(origType, ValueWithAttributes.class);
