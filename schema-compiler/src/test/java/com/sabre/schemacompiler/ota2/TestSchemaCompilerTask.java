...
import com.sabre.schemacompiler.ota2.OTA2CompilerConstants;
...
import com.sabre.schemacompiler.task.TaskFactory;
...
public class TestSchemaCompilerTask {
	...
	public void testSTL2_ContextComponents() throws Exception {
		...
		// extensions
		assertTrue(SchemaCompilerApplicationContext.getContext().containsBean(
				"ota2CommonSchema"));
		assertTrue(SchemaCompilerApplicationContext.getContext().containsBean(
				"ota2MessageSchema"));
		assertTrue(SchemaCompilerApplicationContext.getContext().containsBean(
	...
	public void testSTL2_SchemaCompilerTask() throws Exception {
		...
		compilerTask
				.setServiceEndpointUrl("http://www.OpenTravel.org/services");
	...
	public void testSTL2_BuiltInDependencies() throws Exception {
		...
		for (ValidationFinding f : findings.getAllFindingsAsList()) {
			if (LoaderValidationMessageKeys.WARNING_UNRESOLVED_LIBRARY_NAMESPACE
					.equals(f.getMessageKey())) {
	...
	// @Test
	public void testSTL2_SchemaCompilerTask_manualProjectTest()
			throws Exception {
		File sourceFile = new File(System.getProperty("user.dir"),
				"../../../temp/schemas/test/ThirdLibrary.otm");
		File projectFile = new File(sourceFile.getParentFile(),
				"/test_project.otp");
		...
		projectManager.addUnmanagedProjectItems(
				Arrays.asList(new File[] { sourceFile }), project, findings);

		SchemaCompilerTestUtils.printFindings(findings, FindingType.ERROR);
		assertFalse(findings.hasFinding(FindingType.ERROR));

		for (TLLibrary library : projectManager.getModel()
	...
	private File getTestOutput(String filename) {
		return new File(System.getProperty("user.dir")
				+ "/target/codegen-output/" + filename);
	}
