...
/**
 * Base class for all model-diff tests that pre-loads a model containing known
 * differences to be used for verification.
 */
public abstract class AbstractDiffTest {
    
    protected static final QName TLIBRARY_DIFF1_V1   = new QName( "http://www.OpenTravel.org/ns/OTA2/SchemaCompiler/test-package-diff1/v1", "library_diff1" );
    protected static final QName TLIBRARY_DIFF1_V1_1 = new QName( "http://www.OpenTravel.org/ns/OTA2/SchemaCompiler/test-package-diff1/v1_1", "library_diff1" );
    protected static final QName TLIBRARY_DIFF1_V2   = new QName( "http://www.OpenTravel.org/ns/OTA2/SchemaCompiler/test-package-diff2/v2", "library_diff2" );
    ...
    protected static TLModel loadModel(String libraryPath) throws Exception {
        LibraryInputSource<InputStream> libraryInput = new LibraryStreamInputSource(
                new File( SchemaCompilerTestUtils.getBaseLibraryLocation() + libraryPath ) );
    ...
    protected <T> T getMember(QName libraryId, String docPath, Class<T> expectedType) {
        ...
        if ((member != null) && !expectedType.isAssignableFrom( member.getClass() )) {
            throw new ClassCastException( "Unexpected member type '" +
                    member.getClass().getSimpleName() + "' for path: " + docPath );
        }
