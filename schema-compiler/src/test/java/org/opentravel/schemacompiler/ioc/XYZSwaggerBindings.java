...
/**
 * Provides an example swagger binding extension for testing of the swagger
 * code generator.
 */
public class XYZSwaggerBindings implements CodeGenerationSwaggerBindings {
	...
	protected static List<SwaggerParameter> initGlobalParameters() {
		...
		paramList.add( newParameter( "TEST-PARAM-HEADER",
				SwaggerParamType.HEADER, false, JsonType.JSON_STRING, "Description of the test header parameter." ) );
		return paramList;
	}
	 ...
	 */
	protected static SwaggerParameter newParameter(String name, SwaggerParamType in, boolean required,
			JsonType type, String documentation) {
	...
	private static List<SwaggerHeader> initGlobalResponseHeaders() {
		...
		headerList.add( newHeader( "TEST-RESPONSE-HEADER", JsonType.JSON_STRING, "Description of test response header" ) );
		return headerList;
	}
	 ...
	 */
	private static SwaggerResponse newResponse(int statusCode, JsonSchema schema,
			List<SwaggerHeader> responseHeaders, String documentation) {
		JsonDocumentation jsonDoc = (documentation == null) ? null : new JsonDocumentation( documentation );
	...
	/**
	 * Initializes the list of global parameters for all API's generated with the
	 * TVP binding style.
	 */
	static {
		try {
