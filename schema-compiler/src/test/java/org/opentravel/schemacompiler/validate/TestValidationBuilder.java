...
public class TestValidationBuilder {
	...
	public void initBuilder() throws Exception {
		...
		testSource = sourceBO;
		builder = new TestVB().setTargetObject( testSource )
	...
	public void testAssertNoDuplicates() throws Exception {
		builder.setProperty( TEST_PROPERTY, Arrays.asList( sourceBO, duplicateBO ) ).assertNoDuplicates( e -> ((TLBusinessObject) e).getName() );
		assertHasFinding( ValidationBuilder.ERROR_DUPLICATE_ELEMENT );
		builder.setProperty( TEST_PROPERTY, Arrays.asList( sourceBO ) ).assertNoDuplicates( e -> ((TLBusinessObject) e).getName() );
		assertNoFinding();
	}
