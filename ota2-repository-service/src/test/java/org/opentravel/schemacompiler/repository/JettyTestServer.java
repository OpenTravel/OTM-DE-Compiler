...
/**
 * Encapsulates the configuration and run-time environment of an OTA2.0
 * repository that is launched as an embedded Jetty application to facilitate
 * testing.
 * 
 * @author S. Livezey
 */
public class JettyTestServer {
	...
	/**
	 * Constructor that specifies the configuration of the Jetty server to be
	 * used for testing.
	 * 
	...
	public JettyTestServer(int port, File snapshotLocation, Class<?> testClass) {
		...
		if ((repositorySnapshotLocation != null) && !repositorySnapshotLocation.exists()) {
			throw new IllegalArgumentException("Repository Snapshot Not Found: " + repositorySnapshotLocation.getAbsolutePath());
		}
		if (!repositoryRuntimeLocation.exists() && !repositoryRuntimeLocation.mkdirs()) {
			throw new IllegalArgumentException("Unable to create run-rime repository folder: " + repositoryRuntimeLocation.getAbsolutePath());
		}
		System.setProperty("test.class", testClass.getSimpleName());
		System.setProperty("ota2.repository.config", System.getProperty("user.dir") + "/target/test-classes/ota2-repository-config.xml");
		RepositoryComponentFactory.resetDefault();
	}
	
	/**
	 * Initializes the run-time repository from the snapshot and launches the
	 * Jetty server.
	 * 
	...
	public synchronized void start() throws Exception {
		...
		context.addServlet( new ServletHolder( new RepositoryServlet( new PackagesResourceConfig(
				new String[] { "org.opentravel.schemacompiler.repository", "org.opentravel.schemacompiler.providers" }) ) ),
				"/service/*");
	...
	/**
	 * Pings the Jetty service  with a meta-data request that forces the initialization
	 * of the repository web service.
	 */
	...
	/**
	 * Initializes the run-time OTA2.0 repository by deleting any existing files
	 * and copying all of the files in the snapshot folder location.
	 * 
