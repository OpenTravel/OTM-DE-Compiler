...
/**
 * Test that verifies that a refresh operation from a remote repository will properly synchronize
 * a user's local repository and workspace.
 */
public class RepositoryRefreshTest extends RepositoryMultiUserTestBase {
	...
	private void user2_task1_updateLibrary() throws Exception {
		if (DEBUG) System.out.println("User 2: Updating model and committing changes");
	...
	private void user1_task2_refreshModel() throws Exception {
		if (DEBUG) System.out.println("User 1: Refreshing model");
		ProjectItem item = findProjectItem( "Version_Test_1_0_0.otm" );
