...
public abstract class TestRepositoryFunctions extends RepositoryTestBase {
	...
	public void test_02_LockLibrary() throws Exception {
		...
		List<RepositoryItem> itemList = testRepository.get().listItems( projectItem.getBaseNamespace(),
				TLLibraryStatus.DRAFT, true );
	...
	public void test_12_PublishRelease() throws Exception {
		...
		if (this.getClass().getName().contains( "Remote" )) { // only test when running non-local
			try {
				RepositoryItem principalLib = releaseItem.getContent().getPrincipalMembers().get( 0 )
					.getRepositoryItem();
	...
	public void test_16_UnpublishRelease() throws Exception {
		...
		List<RepositorySearchResult> searchResults = repositoryManager.get().search( "TestRelease",
