...
/**
 * Abstract base class for tests that verify concurrency conditions that can exist when multiple users access a
 * repository at the same time. This class is designed such that each sub-class should only implement a single test
 * case.
 */
public class RepositoryMultiUserTestBase {
    ...
    protected static void startTestServer(String repositorySnapshotFolder, int port, Class<?> testClass)
        throws Exception {
        ...
        jettyServer =
            new JettyTestServer( port, repositorySnapshot, testClass, RepositoryTestBase.defaultRepositoryConfig );
        jettyServer.start();
    }
    ...
    private void setupCurrentThread(String userId, String password) throws Exception {
        File localRepository = new File( System.getProperty( "user.dir" ),
            "/target/test-workspace/" + this.getClass().getSimpleName() + "/" + userId + "/local-repository" );
        ...
        wipFolder.set( new File( System.getProperty( "user.dir" ),
            "/target/test-workspace/" + this.getClass().getSimpleName() + "/" + userId + "/wip" ) );
