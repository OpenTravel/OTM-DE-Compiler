...
/**
 * Creates an SVN server daemon process that can be started and stopped programmatically.
 */
public class SvnserveProcess {

    private static final String SVNSERVE_CONFIG_TEMPLATE_RESOURCE = "/svn-config/svnserve.config.template";
    ...
    public static SvnserveProcess run(TestOptions testOptions, File repositoryRoot, Map<String,String> loginToPassword)
        throws SVNException {
        ...
        final SvnserveProcess svnserveProcess =
            new SvnserveProcess( svnserveCommand, port, repositoryRoot, tempDirectory, loginToPassword );
    ...
    /**
     * Returns true if the local system has an SVN client installed.
     * 
     * @return boolean
     */
    ...
    private SvnserveProcess(String svnserveCommand, int port, File repositoryRoot, File tempDirectory,
        Map<String,String> loginToPassword) {
    ...
    private File generateConfigFile() {
        ...
        if (inputStream == null) {
            throw new RuntimeException(
                "Unable to load resource: " + SVNSERVE_CONFIG_TEMPLATE_RESOURCE + ", check classpath" );
        }
        ...
        configContents = configContents.replaceAll( "%password.db.file%",
            TestUtil.convertSlashesToDirect( getOrCreatePasswordDbFile().getAbsolutePath() ) );
        configContents = configContents.replaceAll( "%authzfile%",
            TestUtil.convertSlashesToDirect( getOrCreateAuthzFile().getAbsolutePath() ) );
