...
/**
 * Verifies the operation of the OTA2.0 repository's indexing service and
 * free-text search utilities.
 * 
 * @author S. Livezey
 */
public class TestFreeTextSearchService {
	...
	public void testIndexAllRepositoryItems() throws Exception {
		...
		// Re-start the service and re-run the search to make sure we can reopen the index after closing it
	...
	protected FreeTextSearchService initSearchService(String testName) throws Exception {
		File repositorySnapshot = new File(System.getProperty("user.dir"), "/src/test/resources/repo-snapshots/versions-repository");
		File testRepository = new File(System.getProperty("user.dir"),
				"/target/test-workspace/" + TestFreeTextSearchService.class.getSimpleName() + "/" + testName + "/test-repository");
		File indexFolder = new File(System.getProperty("user.dir"),
				"/target/test-workspace/" + TestFreeTextSearchService.class.getSimpleName() + "/" + testName + "/index-test");
