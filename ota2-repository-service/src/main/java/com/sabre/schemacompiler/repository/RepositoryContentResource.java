...
import com.sabre.schemacompiler.lock.LockableResource;
import com.sabre.schemacompiler.lock.RepositoryLockManager;
...
public class RepositoryContentResource {
	...
	public RepositoryContentResource() throws RepositoryException {
		...
		try {
			FreeTextSearchService.initializeSingleton( componentFactory.getSearchIndexLocation(), repositoryManager );
	...
	public JAXBElement<NamespaceListType> listAllNamespaces(
		...
		List<String> nsList = repositoryManager.getFileManager().findAllNamespaces( repositoryMetadata.getRootNamespace() );
	...
	public JAXBElement<NamespaceListType> listBaseNamespaces(
		...
		UserPrincipal user = securityManager.getUser(authorizationHeader);
		RepositoryInfoType repositoryMetadata = repositoryMetadataResource.getResource();
		List<String> nsList = repositoryManager.getFileManager().findAllBaseNamespaces( repositoryMetadata.getRootNamespace() );
		NamespaceListType namespaceList = new NamespaceListType();
	...
	/**
	 * Returns a list of the latest version of each <code>RepositoryItem</code>'s meta-data record in
	 * the repository.
	 * 
	...
	/**
	 * Returns a list of the latest version of each <code>RepositoryItem</code>'s that match
	 * the free-text search criteria provided.
	 * 
	...
	@Path("search")
	@Produces(MediaType.TEXT_XML)
	public JAXBElement<LibraryInfoListType> search(
			@QueryParam("query") String freeTextQuery,
	...
	public Response createRootNamespace(
			@QueryParam("rootNamespace") String rootNamespace,
			@HeaderParam("Authorization") String authorizationHeader)
					throws RepositoryException {
		...
		if (securityManager.isAdministrator(user)) {
			...
			throw new RepositorySecurityException("The user does not have permission to create the requested namespace.");
		}
	}
	...
	public Response deleteRootNamespace(
			@QueryParam("rootNamespace") String rootNamespace,
			@HeaderParam("Authorization") String authorizationHeader)
					throws RepositoryException {
		...
		if (securityManager.isAdministrator(user)) {
			...
			throw new RepositorySecurityException("The user does not have permission to delete the requested namespace.");
		}
	}
	 ...
	 * @param namespace  the full namespace (with version-specific path information) to which the library content will be published
	...
	public Response publishContent(
		...
		// Attempt to obtain a lock on the resource we are about to create.  If we cannot calculate the base namespace
		// for the item, proceed without the lock.  In those cases, the attempt will most likely result in an error anyway.
		...
		try {
			...
			lockedResource = RepositoryLockManager.getInstance().acquireWriteLock( baseNamespace, contentDetail.getFileName() );
		...
		try {
			...
			if (securityManager.isAuthorized(user, namespace, RepositoryPermission.WRITE)) {
				...
				RepositoryItem publishedItem = repositoryManager.publish(contentStream, contentDetail.getFileName(),
						libraryName, namespace, versionIdentifier, versionScheme, TLLibraryStatus.valueOf( initialStatus.toUpperCase() ));
	...
	public Response commitContent(
		...
		try {
			...
			if (securityManager.isAuthorized(user, itemMetadata.getBaseNamespace(), RepositoryPermission.WRITE)) {
				repositoryManager.getFileManager().setCurrentUserId( user.getUserId() );
				...
				throw new RepositorySecurityException("The user does not have permission to access the requested resource.");
			}
	...
	public JAXBElement<LibraryInfoType> lockRepositoryItem(
		...
		try {
			...
			if (securityManager.isAuthorized(user, itemMetadata.getBaseNamespace(), RepositoryPermission.WRITE)) {
				...
				// NOTE: No need to re-index the item because locking does not change its content - only the state of its meta-data
	...
	public JAXBElement<LibraryInfoType> demoteRepositoryItem(
		...
		try {
			...
			if (securityManager.isAdministrator(user)) {
				...
				throw new RepositorySecurityException("The user does not have permission to demote an item in the repository - administration access required.");
			}
	...
	public JAXBElement<LibraryInfoType> recalculateRepositoryItemCrc(
		...
		try {
			...
			if (securityManager.isAdministrator(user)) {
				...
				throw new RepositorySecurityException("The user does not have permission to recalculate the CRC of an item in the repository - administration access required.");
			}
	...
	@Consumes(MediaType.TEXT_XML)
	@Produces(MediaType.TEXT_XML)
	public Response deleteRepositoryItem(
			JAXBElement<RepositoryItemIdentityType> identityElement,
			@HeaderParam("Authorization") String authorizationHeader)
					throws RepositoryException {
	...
	public JAXBElement<LibraryInfoType> getRepositoryItemMetadata(
		...
		try {
			...
			if (securityManager.isAuthorized(user, itemMetadata.getBaseNamespace(), getMinimumReadPermission(itemMetadata))) {
				return objectFactory.createLibraryInfo( itemMetadata );
	...
	public JAXBElement<RepositoryPermissionType> getUserAuthorization(
		...
		UserPrincipal user = securityManager.getUser(authorizationHeader);
		RepositoryPermission securityAuthorization = securityManager.getAuthorization(user, baseNamespace);
		...
		if (securityAuthorization != null) {
			repositoryAuthorization =
					org.opentravel.ns.ota2.repositoryinfo_v01_00.RepositoryPermission.fromValue(
							securityAuthorization.value() );
		} else {
			repositoryAuthorization =
					org.opentravel.ns.ota2.repositoryinfo_v01_00.RepositoryPermission.NONE;
		}
	 ...
	 */
	protected boolean isReadable(RepositoryItem item, UserPrincipal user, Map<String,Map<TLLibraryStatus,Boolean>> accessibleItemCache)
	...
	/**
	 * Returns the permission level that is required to read the meta-data or content of the indicated
	 * repository item.
	 * 
	 * @param itemMetadata  the meta-data for the repository item to be accessed
	...
	private void indexRepositoryItem(RepositoryItem item, boolean deleteIndex) {
		...
		if ((service != null) && (item != null)) {
			try {
				...
				log.warn("Error submitting repository item for indexing: "
						+ item.getFilename() + " [" + item.getNamespace() + "]", t);
			}
		}
	}
