...
/**
 * Encapsulates the list of all permissions for a single namespace. This class also supports to and from the JAXB
 * representation of namespace permissions to facilitate storage into the local file system.
 * 
 * @author S. Livezey
 */
public class NamespacePermissions {

    private static List<RepositoryPermission> prioritizedPermissions =
        Arrays.asList( RepositoryPermission.READ_FINAL, RepositoryPermission.READ_DRAFT, RepositoryPermission.WRITE );
    ...
    /**
     * Constructor that configures the permissions using data from the JAXB namespace authorizations file.
     * 
     * @param namespace the namespace for which permissions are being viewed or configured
    ...
    /**
     * Scans the list of permissions for this namespace and ensures that entries exist for all of the groups in list
     * provided (and the anonymous user as an implied group). If new permission entries are created for a group, the
     * grant and deny permissions will be null.
     * 
    ...
    /**
     * Converts this collection of namespace permissions into its JAXB equivalent, allowing them to be saved to the
     * repository's file system. Any principals who are not explicitly granted or denied any permissions are omitted
     * from the resulting JAXB authorizations.
     * 
    ...
    /**
     * Uses the given JAXB permission declarations to populate the list of principals and the map of their corresponding
     * permissions.
     * 
    ...
    /**
     * Resolves the two permissions by returning the one that takes precedent (e.g. Read-Draft has priority over
     * Read-Final).
     * 
