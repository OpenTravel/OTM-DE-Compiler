...
/**
 * Wraps a <code>TLFacet</code> to provide additional information required during JSP page rendering.
 * 
 * @author S. Livezey
 */
public class FacetIdentityWrapper {
    ...
    public FacetIdentityWrapper(TLFacet facet) {
        if (facet instanceof TLContextualFacet) {
            ...
            if ((ownerName != null) && !ownerName.getNamespaceURI().equals( cFacet.getFacetNamespace() )) {
    ...
    /**
     * Returns the namespace in which the facet was declared if different from the owning entity's namespace.
     *
     * @return String
     */
