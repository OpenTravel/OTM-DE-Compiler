...
/**
 * Implements the logic required to index a single repository item for
 * free-text searching.
 * 
 * @author S. Livezey
 */
public class IndexingTask extends AbstractFreeTextSearchTask {
	 ...
	 * 
	 * @param indexWriter  the index writer to which all indexed documents are to be published
	 ...
	 * 
	 * @param itemToIndex  the item to be indexed
	 ...
	 * 
	 * @param itemToDelete  the item whose index is to be deleted
	 * @return TaskInfo
	 */
	 ...
	 * 
	 * @param task  the indexing task that specifies the repository item to be indexed
	 */
	public void indexRepositoryItem(TaskInfo task) {
		try {
			...
			if (log.isInfoEnabled()) {
				log.info("Indexing repository item '" + item.getFilename() + "' [" + item.getNamespace() + "].");
			}
	...
	public void deleteRepositoryItemIndex(TaskInfo task) {
		try {
			...
			if (log.isInfoEnabled()) {
				log.info("Deleting repository item index '" + item.getFilename() + "' [" + item.getNamespace() + "].");
			}
	 ...
	 * the union of all minor and patch versions for the item.  If no minor or patch version exist for
	 * the item, any existing head version index is deleted.
	 * 
	 * @param item  the item whose head version is to be indexed
	 * @throws RepositoryException  thrown if the items version history cannot be retrieved
	 * @throws VersionSchemeException  thrown if the item's version scheme is not recognized
	 * @throws IOException  thrown if the search index cannot be updated
	 */
	private void indexHeadVersion(RepositoryItem item) throws RepositoryException, VersionSchemeException, IOException {
		...
		if (!headVersions.isEmpty()) {
			buildIndex( headVersions.get(0), IndexVersionType.HEAD, buildIndexingTempFile(headVersions) );
		...
		if (!headFinalVersions.isEmpty()) {
			buildIndex( headFinalVersions.get(0), IndexVersionType.HEAD_FINAL, buildIndexingTempFile(headFinalVersions) );
	...
	/**
	 * Creates a Lucene index document using the information and file content provided.  After the indexing is
	 * complete, the temporary file content is deleted from the file system.
	 * 
	 * @param item  the repository item to be indexed
	 * @param versionType  the type of version-index to be created
	 * @param indexingTempFile  the temporary file content to be indexed for searching
	 * @throws IOException  thrown if the index document cannot be created
	 */
	private void buildIndex(RepositoryItem item, IndexVersionType versionType, File indexingTempFile) throws IOException {
		try {
			...
			indexDoc.add( new StringField(BASE_NAMESPACE_FIELD, item.getBaseNamespace(), Field.Store.YES) );
			...
			indexDoc.add( new TextField(CONTENT_FIELD, new BufferedReader(new FileReader(indexingTempFile))) );
			
			indexWriter.updateDocument(new Term(IDENTITY_FIELD, indexIdentity), indexDoc);
			indexWriter.commit();
			
		} finally {
			if (indexingTempFile != null) indexingTempFile.delete();
		}
	}
	 ...
	 * no action.
	 * 
	 * @param item  the repository item to be deleted from the index
	 * @param versionType  the type of version-index to be deleted from the index
	 */
	 ...
	 * 
	 * @param item  the repository item for which to return an identity string
	 * @param versionType  the type of version-index to be created for the repository item
	 * @return String
	 */
	private String getIndexIdentity(RepositoryItem item, IndexVersionType versionType) {
		...
		identity.append( (versionType == IndexVersionType.STANDARD) ? item.getVersion() : versionType.toString() );
		return identity.toString();
	}
	
	/**
	 * Returns a list of the versions of the repository item that should be included in the HEAD (or HEAD-FINAL)
	 * indexing document.  If the given repository item is not a member of the latest major version chain for
	 * the item, this method will return an empty list.
	 * 
	 * If the 'includeDraftVersions' flag is true, all versions will be included in the resulting list of
	 * repository items.  If false, all versions up to and including the latest final version will be included.
	 * 
	 * @param item  the item for which head versions should be returned
	 * @param includeDraftVersions  indicates whether draft versions are to be included in the indexed content
	 * @return List<RepositoryItem>
	 */
	private List<RepositoryItem> getHeadVersionsToIndex(RepositoryItem item, boolean includeDraftVersions)
			throws VersionSchemeException, RepositoryException {
		VersionScheme versionScheme = VersionSchemeFactory.getInstance().getVersionScheme( item.getVersionScheme() );
		...
		if (!allVersions.isEmpty()) {
			String latestMajorVersion = versionScheme.getMajorVersion( allVersions.get(0).getVersion() );
			
			// Only submit content for indexing if the version we are indexing is in the latest major
			// version chain.  If it is not, then the HEAD (or HEAD-FINAL) version indexing could not have
			...
			if (itemMajorVersion.equals(latestMajorVersion)) {
				...
				for (RepositoryItem itemVersion : allVersions) {
					String currentMajorVersion = versionScheme.getMajorVersion( itemVersion.getVersion() );
					
					// If we are not including draft items, skip over each version 
					canIndex |= (itemVersion.getStatus() == TLLibraryStatus.FINAL);
					
					// If we have gone back in the history past the current major version, we can stop gathering
	 ...
	 * 
	 * @param itemToIndex  the repository item whose content is to be indexed
	 * @return File
	 */
	private File buildIndexingTempFile(RepositoryItem itemToIndex) throws RepositoryException, IOException {
	 ...
	 * 
	 * @param itemsToIndex  the list of repository items whose content is to be included in the indexed document
	 * @return File
	 * @throws RepositoryException  thrown if the content file for one or more of the repository items cannot be identified
	 * @throws IOException  thrown if the temporary file cannot be created or written to
	 */
	private File buildIndexingTempFile(List<RepositoryItem> itemsToIndex) throws RepositoryException, IOException {
	...
	public static class TaskInfo {
		...
		/**
		 * Constructor that specifies the item to be indexed, and also indicates whether the head version
		 * of this item will also be indexed.
		 * 
		 * @param itemToIndex  the item to be indexed
		 * @param indexHeadVersion  flag indicating whether the head version of the repository item should also be indexed
		 * @param deleteIndex  flag indicating that the item's index should be deleted
		 */
