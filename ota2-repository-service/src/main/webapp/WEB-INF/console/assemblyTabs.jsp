...
<c:url var="viewUrl" value="/console/assemblyView.html">
	<c:param name="baseNamespace" value="${item.baseNamespace}" />
	<c:param name="filename" value="${item.filename}" />
...
<table style="border-collapse:collapse;width:100%;float:left;">
	<tr>
		<td>
			<h2 style="padding-bottom: 0;">Service Assembly: ${item.libraryName} <small>(${item.version})</small></h2>
		...
		<td style="white-space:nowrap;width:10%;text-align:left;">
			<c:if test="${sessionScope.isAdminAuthorized}">
				...
				[ <a href="${deleteItemUrl}">Delete this Assembly</a> ]<br/>
			</c:if>
...
<ul class="tab">
	...
	<li><a href="${viewUrl}" class="${tabStyle}">Member Releases</a></li>
</ul>
<p/><br/>
