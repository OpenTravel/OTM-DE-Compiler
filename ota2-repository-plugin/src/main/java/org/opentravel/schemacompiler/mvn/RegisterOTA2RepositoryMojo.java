...
/**
 * Registers or updates the registration for an OTA2.0 repository during an automated build process. If the system
 * property 'otm.repository.forceUpdate' is set to true, the existing registration will be deleted and recreated using
 * the information passed to this plugin.
 */
...
public class RegisterOTA2RepositoryMojo extends AbstractMojo {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        try {
            ...
            if (repository == null) {
                ...
                if (!remoteRepo.getId().equals( repositoryId )) {
                    throw new MojoExecutionException(
                        String.format( "The remote repository was added but its ID does not match the required value "
            ...
            throw new MojoExecutionException( "Error registering OTA2.0 repository: " + repositoryId, e );
        }
    }

    /**
     * Returns true if the environment variable has been set to force the update of the OTM repository registration.
     * This may be required when updating user credentials or the base URL of the repository has changed.
     * 
     * @return boolean
     */
