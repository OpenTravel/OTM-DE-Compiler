...
public class SubscriptionManager {
    ...
    /**
     * Constructor that specifies the repository manager for which this instance handles subscriptions and
     * notifications.
     * 
    ...
    /**
     * Creates a new single-version <code>SubscriptionTarget</code> using information from the
     ...
     * @return SubscriptionTarget
     */
    public static SubscriptionTarget getSubscriptionTarget(String baseNamespace, String libraryName, String version) {
    ...
    /**
     * Retrieves the list of events to which the user is subscribed for all versions of the specified repository item.
     * 
     * @param subscriptionTarget the target for which to retrieve the user's subscribed events
     * @param userId the ID of the user whose subscriptions are to be retrieved
     * @return List&lt;SubscriptionEventType&gt;
     * @throws RepositoryException thrown if an error occurs while accessing the repository
     */
    public List<SubscriptionEventType> getAllVersionsSubscriptions(SubscriptionTarget subscriptionTarget, String userId)
        throws RepositoryException {
        List<SubscriptionEventType> eventTypes = new ArrayList<>();

        if ((userId != null) && !userId.equals( UserPrincipal.ANONYMOUS_USER_ID )) {
            SubscriptionList subscriptions = getAllVersionsSubscriptions( subscriptionTarget.getBaseNamespace(),
                subscriptionTarget.getLibraryName() );
     ...
     * @throws RepositoryException thrown if an error occurs while retrieving the subscription list
     */
    private SubscriptionList getAllVersionsSubscriptions(String baseNamespace, String libraryName)
        throws RepositoryException {
    ...
    public List<SubscriptionEventType> getSingleVersionSubscriptions(SubscriptionTarget subscriptionTarget,
        ...
        if ((userId != null) && !userId.equals( UserPrincipal.ANONYMOUS_USER_ID )) {
            SubscriptionList subscriptions = getSingleVersionSubscriptions( subscriptionTarget.getBaseNamespace(),
                subscriptionTarget.getLibraryName(), subscriptionTarget.getVersion() );
     ...
     * @throws RepositoryException thrown if an error occurs while retrieving the subscription list
     */
    private SubscriptionList getSingleVersionSubscriptions(String baseNamespace, String libraryName, String version)
        throws RepositoryException {
     ...
     * 
     * @param subscriptionTarget the target for which to update the user's subscribed events
     * @param userId the ID of the user whose subscriptions are to be updated
     * @param eventTypes the types of events for which the user wishes to be notified
     * @throws RepositoryException thrown if an error occurs while updating the subscription list
     */
    public void updateAllVersionsSubscriptions(SubscriptionTarget subscriptionTarget, String userId,
        ...
        updateSubscriptions(
            getAllVersionsSubscriptions( subscriptionTarget.getBaseNamespace(), subscriptionTarget.getLibraryName() ),
            userId, eventTypes );
    }
    ...
    public void updateSingleVersionSubscriptions(SubscriptionTarget subscriptionTarget, String userId,
        List<SubscriptionEventType> eventTypes) throws RepositoryException {
        updateSubscriptions( getSingleVersionSubscriptions( subscriptionTarget.getBaseNamespace(),
            subscriptionTarget.getLibraryName(), subscriptionTarget.getVersion() ), userId, eventTypes );
    }
     ...
     * @throws RepositoryException thrown if an error occurs while updating the subscription list
     */
    private void updateSubscriptions(SubscriptionList subscriptions, String userId,
        List<SubscriptionEventType> eventTypes) throws RepositoryException {
        if (eventTypes == null) {
    ...
    /**
     * Notifies all subscribed users of an action that was performed on the specified namespace. The only valid actions
     * for this method are <code>NS_CREATED</code> and <code>NS_DELETED</code>.
     * 
    ...
    /**
     * Notifies all subscribed users of an action that was performed on the given repository item. All actions are
     * considered valid for this method <i>except</i> <code>NS_CREATED</code> and <code>NS_DELETED</code>.
     * 
    ...
    /**
     * Returns the delay period in milliseconds between when a repository event occurs and the email notification job is
     * processed.
     *
     * @return long
     */
    ...
    /**
     * Assigns the delay period in milliseconds between when a repository event occurs and the email notification job is
     * processed.
     *
    ...
    /**
     * Returns the base URL that should be used when creating links to the OTM repository in email notification
     * messages.
     *
     * @return String
     */
    ...
    /**
     * Assigns the base URL that should be used when creating links to the OTM repository in email notification
     * messages.
     *
    ...
    /**
     * Executes the loop that listens for notification events and periodically checks to see if a listener shutdown has
     * been requested.
     */
    ...
    /**
     * Processes the given <code></code> by sending an email message to all user accounts which are configured with a
     * valid email address.
     * 
    ...
    private void processNotificationJob(NotificationJob job) {
        try {
            ...
            log.info( "Processing notification job: " + job.getAction() + " (" + recipients.size() + " recipients)" );
            ...
            if (!recipients.isEmpty()) {
                ...
                InternetAddress fromAddress =
                    new InternetAddress( smtpConfig.getSenderAddress(), smtpConfig.getSenderName() );
    ...
    private Session getMailSession(final String smtpUser, final String smtpPassword) {
        return smtpConfig.isAuthEnable()
            ? Session.getInstance( smtpConfig.getSmtpProps(), new javax.mail.Authenticator() {
                @Override
    ...
    /**
     * Returns the list of all users who should be notified of the specified action that was performed on the given
     * repository item.
     * 
     ...
     */
    private List<String> getNotificationList(RepositoryItem item, RepositoryActionType action)
        throws RepositoryException {
        SubscriptionList singleVersionSubscriptions =
            getSingleVersionSubscriptions( item.getBaseNamespace(), item.getLibraryName(), item.getVersion() );
        SubscriptionList allVersionSubscriptions =
            getAllVersionsSubscriptions( item.getBaseNamespace(), item.getLibraryName() );
    ...
    /**
     * Returns the list of all users who should be notified of the specified action that was performed in the specified
     * namespace.
     * 
     ...
     * @throws RepositoryException thrown if an error occurs while retrieving the list of users to be notified
     */
    private List<String> getNotificationList(String affectedNamespace, RepositoryActionType action)
        throws RepositoryException {
    ...
    /**
     * Returns the list of <code>InternetAddress</code> objects for all users who are configured with a valid user
     * account. User's who have not specified an email address will be omitted from the resulting list.
     * 
            ...
            try {
                ...
                if ((user != null) && (user.getEmailAddress() != null) && (user.getEmailAddress().length() > 0)) {
                    ...
                    if (user.getFirstName() != null) {
                        fullName = new StringBuilder().append( user.getFirstName() ).append( " " ).append( fullName )
                            .toString();
                    }
    ...
    /**
     * Contstructs the subject line for an email notification using information from the given job.
     * 
    ...
    private String buildMessageSubject(NotificationJob job) {
        return MessageFormat.format( SUBJECT_FORMAT, job.getAction().getDisplayLabel( Locale.getDefault() ),
            manager.getDisplayName() );
    }

    /**
     * Contstructs the message body for an email notification using information from the given job.
     * 
     * @param job the email notification job for which to generate the email message body
     * @return String
     */
    private String buildMessageBody(NotificationJob job) {
        ...
        if ((job.getAction() == RepositoryActionType.UNLOCK) && REMARK_UNLOCK_REVERT.equals( job.getRemarks() )) {
            job.remarks = null;
    ...
    private class NotificationJob implements Delayed {
        ...
        /**
         * Returns the free-text remarks that were submitted as part of the notification content.
         *
         * @return String
         */
