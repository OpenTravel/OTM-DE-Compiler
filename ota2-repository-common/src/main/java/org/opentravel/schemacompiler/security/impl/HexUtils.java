...
/**
 * Utility methods useful in dealing used to convert byte arrays to and from strings of hexadecimal
 * digits.
 * 
 * @author S. Livezey
 */
public class HexUtils {
     ...
     *                if an invalid hexadecimal digit is found, or the input string contains an odd
	...
	private static void appendNextDigit(String digits, int digitsPos, ByteArrayOutputStream baos) {
		...
		if ((digitsPos + 1) >= digits.length()) {
		    throw new IllegalArgumentException(
		            "Invalid hexadecimal string - odd numbered length.");
		}
