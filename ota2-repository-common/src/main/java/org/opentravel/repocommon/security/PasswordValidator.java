...
/**
 * Handles the validation of user credentials against the encrypted password provided by a remote provider.
 * 
 * @author S. Livezey
 */
public class PasswordValidator {
    ...
    /**
     * Constructs a password validator that utilizes the default encoding for password and credential strings.
     * 
    ...
    /**
     * Constructs a password validator that utilizes the specified encoding for password and credential strings.
     * 
     * @param digestAlgorithm the message digest algorithm to employ when encrypting passwords
    ...
    public boolean isValidPassword(String credentials, String userPassword) {
        ...
        if (messageDigest != null) {
            if (userPassword.startsWith( "{MD5}" ) || userPassword.startsWith( "{SHA}" )) {
                ...
                isValid = PasswordHelper.encrypt( credentials ).equalsIgnoreCase( userPassword );
            }
        } else {
            isValid = PasswordHelper.encrypt( credentials ).equals( userPassword );
        }
        return isValid;
    }
    ...
    protected String digest(String credentials) {
        ...
        synchronized (messageDigest) {
            try {
                return (HexUtils.convert( digestBytes( credentials ) ));
     ...
     * @return String
     */
    protected String digestBase64(String credentials) {
        // If no MessageDigest instance is specified, return unchanged
        if (messageDigest == null) {
            return credentials;
        }
        ...
        synchronized (messageDigest) {
            try {
                return Base64.encode( digestBytes( credentials ) );
     ...
     * @return String
     */
    protected byte[] digestBytes(String credentials) {
        synchronized (messageDigest) {
            messageDigest.reset();
