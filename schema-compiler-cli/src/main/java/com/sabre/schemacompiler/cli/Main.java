...
public class Main {
			...
			if (validateCommandLine(commandLineArgs)) {
				...
				if (bindingStyle != null) {
					if (CompilerExtensionRegistry.getAvailableExtensionIds().contains(bindingStyle)) {
						...
						System.out.println( MessageFormat.format(messageBundle.getString("invalidBindingStyle"), bindingStyle) );
						validBinding = false;
					}
				}
				...
				if (validBinding && libraryFile.exists()) {
					...
					throw new IOException( MessageFormat.format(messageBundle.getString("fileNotFound"), filename) );
				}
			} else {
				displayHelp();
			}
	...
	protected Options getCommandLineOptions() {
		...
		options.addOption("M", "exampleMaxDetails", true, messageBundle.getString("exampleMaxDetails"));
	...
	protected void displayHelp() {
		new HelpFormatter().printHelp(SCRIPT_SYNTAX, messageBundle.getString("helpHeader"), getCommandLineOptions(), null);
	}
